...
description: "了解如何轻松安装和设置 Coolify，自托管您的应用、数据库和服务 → 立即了解更多"
...
Coolify 内置支持部署静态站点、Node.js、Python、PHP、Rust 和 Docker 容器。它还提供热门的一键模板，比如 WordPress、Ghost、Grafana、Appwrite、Nextcloud 和 PostgreSQL 数据库。它自动处理 Let's Encrypt SSL 证书和通过 Traefik 的反向代理路由。

## 准备工作

虽然 Coolify 核心轻量，但根据部署的应用数量、Docker 构建和同时运行的数据库，资源使用可能大幅增加。我们推荐以下硬件配置来在你的 VPS/独服上托管 Coolify。
...
:::tip[SSH 密钥认证]
我们**强烈建议**使用 SSH 密钥替代密码作为认证方式。你可以通过我们的 [SSH 密钥设置](vserver-linux-sshkey.md) 教程了解具体操作。
:::
...
Coolify 提供了快速自动化安装脚本。为了安全起见，我们用环境变量预设 root 管理员账号，避免安装过程中被未授权访问。
...
```
:::note[开源]
安装脚本源码可在 [官方 Coolify 仓库](https://github.com/coollabsio/coolify/blob/v4.x/scripts/install.sh) 查看
:::

脚本会自动安装 Docker，配置所需网络，并设置 Coolify 容器。根据服务器硬件，过程可能需要几分钟。

![img](https://screensaver01.zap-hosting.com/index.php/s/Zx5DXzEH2MmrkrX/preview)

### 访问 Coolify

安装完成后，打开浏览器访问控制面板。输入服务器 IP 地址加端口 8000：`http://[your_server_ip]:8000`
...
登录 Coolify 控制面板后，第一步是绑定自定义域名。通过 IP 地址访问 Coolify (`http://[your_server_ip]:8000`) 不安全。

绑定域名后，Coolify 会自动为你的控制面板和未来所有应用生成免费 SSL 证书。

进入你的域名服务商的 DNS 管理页面，添加两个指向服务器的 `A` 记录。如果你的域名在 ZAP-Hosting 注册，可以参考我们的 [域名记录](domain-records.md) 指南轻松添加：
...
:::info[DNS 路由]
* `@` 记录让 Coolify 使用你的根域名
* `*`（通配符）记录让 Coolify 自动路由所有新子域（如 `api.[your_domain]`），无需为每个项目单独添加 DNS 记录
:::

### 配置实例域名

为了给 Coolify 实例启用 SSL 证书，你需要给它绑定一个域名。点击左侧边栏的 **设置**，找到 **实例域名** 字段，输入你的域名，带上 `https://` 前缀（例如 `https://coolify.[your_domain]`）。点击 **保存** 完成设置。

![img](https://screensaver01.zap-hosting.com/index.php/s/jYW63sF9k5sZez3/preview)

## 部署你的第一个应用

接下来，访问 **项目** 标签页，开始部署你的第一个应用！
...
干得漂亮！你已经成功在服务器上安装了 Coolify，可以马上开始部署项目。我们还推荐你看看以下资源，能帮你更好地配置服务器：
