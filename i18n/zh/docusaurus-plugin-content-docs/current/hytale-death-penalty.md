...
在 Hytale 服务器上，**死亡惩罚**决定了玩家死亡时会受到怎样的影响。这个设置控制玩家死亡后，进度、物品、耐久度或其他游戏后果的影响程度。
...
## 可用的死亡惩罚设置
死亡惩罚设置控制玩家死亡时对背包物品和物品耐久度的影响。这些选项让你决定物品是保留、部分丢失还是全部掉落，以及物品死亡时耐久度的损失程度。

以下是可用的配置选项：

**ItemsLossMode**  
此设置定义玩家死亡时物品丢失的整体行为。支持以下取值：
...
定义死亡时每个物品堆叠丢失的百分比。  
例如，值为 `50.0` 表示每个堆叠丢失一半。

**ItemsDurabilityLossPercentage**  
控制玩家死亡时物品耐久度的损失百分比。  
该值基于物品当前耐久度，允许死亡带来后果但不至于完全丢失物品。

这些设置结合起来，能让你精细调控死亡惩罚的宽容度或惩罚力度。  
合理调整能确保游戏体验平衡，符合服务器预期的难度和玩法风格。



## 死亡惩罚配置

死亡惩罚设置位于服务器主配置文件 `config.json` 中。这些值在服务器启动时读取，并对所有玩家全局生效。
...
这行定义了当前激活的游戏玩法配置文件。死亡惩罚配置块直接写在这行下面。找到后，添加死亡配置块来定义重生和物品丢失的处理方式：
...
此配置中，服务器使用 `Configured` 模式，物品丢失按百分比控制。  
...
配置好死亡惩罚后，你就能清楚掌握你的 Hytale 服务器上玩家死亡时的处理方式。  
如果有任何疑问或需要帮助，随时联系在线客服，我们每天都在这里为你服务！🙂
