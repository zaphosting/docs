...
title: "独立服务器：在 Linux 上安装 Joplin"
...
## 安装
满足所有要求并完成准备后，就可以安装 Joplin 了。执行以下命令：
...
安装完成后，建议配置基础设置以适配你的使用习惯。配置项可在 *设置* 菜单中找到。

**同步**
Joplin 支持多种同步目标，如 Nextcloud、Dropbox、OneDrive 或 WebDAV。在“同步”中选择你喜欢的服务并输入账号信息，实现多设备笔记实时同步。

**加密**
为保护同步笔记安全，可在“加密”设置中启用端到端加密。系统会生成密钥，需在所有访问笔记的设备上配置。

**编辑器与外观**
Joplin 使用 Markdown 作为笔记格式。在“编辑器”设置中可选择是否自动显示预览，并调整字体类型和大小。
...
通过这些核心设置，Joplin 可满足任务管理、项目文档或个人知识库等多种使用场景。
...
恭喜你！现在已经成功在独立服务器上安装并配置了 Joplin。我们还推荐你查看以下资源，能为你的服务器配置提供更多帮助和指导：
