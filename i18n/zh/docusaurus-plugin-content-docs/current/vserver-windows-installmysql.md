...
MySQL 服务器是许多应用的基础，比如游戏服务器或网站，里面可能存储着重要数据。接下来，我们会教你如何在你的 Windows VPS 上搭建属于你自己的 MySQL 数据库服务器。
<InlineVoucher />

## 安装步骤

首先，通过远程桌面连接登录你的服务器，下载最新版本的 MariaDB，它是我们用来搭建数据库服务器的：[MariaDB 下载](https://mariadb.org/download/?t=mariadb)。用 WinRAR 或 7Zip 解压下载好的压缩包，双击安装文件启动安装。

接着一路点击 **Next**，直到出现让你输入 root 密码的界面：
...
:::info
重要提示！如果你想通过 Navicat、HeidiSQL 或其他工具远程访问数据库，必须勾选 **Enable access from remote machins for root user**（允许 root 用户远程访问）选项！不过出于安全考虑，我们一般不推荐开启这个功能。
:::
...
现在你的数据库服务器已经上线，可以开始使用啦！

## 配置外部访问

如果你在安装时开启了“允许 root 用户远程访问”，还需要在 Windows 防火墙中开放 MySQL 默认端口 **3306**。打开防火墙，创建一条新规则即可。
