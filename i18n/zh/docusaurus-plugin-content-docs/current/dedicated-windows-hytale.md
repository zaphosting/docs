...
description: "了解如何在你的 Windows 独立服务器上搭建 Hytale 独立服务器，实现流畅的游戏管理 → 立即了解更多"
...
要运行 Hytale 服务器，你的系统必须满足一些基本要求。服务器基于 Java 25 运行，至少需要 4 GB 内存。支持 x64 和 arm64 架构。实际资源消耗取决于玩家数量、视距和世界活动情况，较大的服务器可能需要更多资源。
...
如果系统尚未安装 Java，请参考我们专门针对 Windows 服务器的 Java 安装指南。该指南详细说明了如何正确安装和配置 Java 环境。
...
CLI 下载器提供了结构化的方式来下载和更新 Hytale 服务器文件。下载后，将压缩包解压到临时目录。压缩包内含有 QUICKSTART.md 文件，介绍了工具的基本用法。

在命令行运行下载器，按照提示下载最新服务器版本。完成后，将下载的服务器文件和资源包复制到你的服务器目录。此时目录应包含服务器 JAR 文件和资源压缩包（如 Assets.zip）。
...
首次启动时，服务器必须完成认证，玩家才能连接。认证通过服务器控制台进行，使用设备登录流程。按照控制台提示完成认证。
...
默认情况下，服务器监听 UDP 端口 5520，绑定所有可用网卡。你可以根据需要更改地址和端口。服务器通过 UDP 使用 QUIC 协议通信。请确保防火墙允许所选端口的 UDP 入站流量。你可以使用 Iptables 或 UFW 配置防火墙。Windows PowerShell 下运行以下命令即可轻松添加规则：
...
视距是影响内存消耗的关键因素。视距越大，内存占用越高，因为需要同时保持更多世界数据处于活动状态。

大多数情况下，最大视距设置为 12 区块（384 方块）能在服务器性能和游戏体验间取得良好平衡。

对比来看，Minecraft 服务器默认视距为 10 区块（160 方块）。Hytale 默认的 384 方块约等于 24 个 Minecraft 区块，这也解释了更高的内存需求。请根据预期玩家数量和系统资源调整此数值。
