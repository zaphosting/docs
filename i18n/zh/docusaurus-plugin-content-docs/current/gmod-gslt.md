---
id: gmod-gslt
title: "Garry’s Mod：创建 GSLT（Steam 游戏服务器登录令牌）"
description: "了解如何使用 Steam 游戏服务器登录令牌认证你的游戏服务器，实现正确上架和 VAC 保护 → 立即了解"
...
没有有效的 GSLT，相关服务器可能无法出现在公共服务器列表中，或者无法启用 VAC 保护。
...
请注意，你对自己的令牌负全部责任。如果令牌被滥用或封禁，你可能会永久失去对应游戏的访问权限。切勿分享你的令牌，如果不小心泄露，务必立即删除。重置 Steam 密码时，所有令牌会自动重新生成。长时间未使用的令牌会过期，但你随时可以重新创建。
...
登录后，你可以通过页面上的表单创建新令牌。在 **App ID** 字段中输入 App ID `4000`，这是 **Garry’s Mod** 的专属 ID。务必输入正确的 ID，否则令牌无法生效。
...
:::info 多台 Garry’s Mod 服务器运营
如果你运营多台 **Garry’s Mod** 服务器，每台服务器都需要单独的 GSLT。<u>**不支持**</u>用一个 GSLT 运营多台游戏服务器。
:::
