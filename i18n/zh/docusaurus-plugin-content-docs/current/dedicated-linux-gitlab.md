...
title: "独立服务器：在 Linux 上安装 GitLab"
description: "了解如何在 Linux 上设置 GitLab，优化 DevOps 工作流程，提升团队协作 → 立即了解更多"
...
以下要求由官方 [GitLab 团队](https://docs.gitlab.com/ee/install/requirements.html) 推荐，强烈建议遵循这些前提条件，以避免后续出现不便和问题。
...
如果你想在使用 GitLab 时发送邮件通知，可以选择安装 Postfix（SMTP 服务器）。这是**可选**的。
...
安装 OpenSSH Server 及其所需的前置依赖。GitLab 的 Web 界面面板就是通过它来托管的。
...
如果你想使用其他邮件发送方案，请跳过此步骤，GitLab 安装完成后可参考官方指南 [配置外部 SMTP 服务器](https://docs.gitlab.com/omnibus/settings/smtp)。
:::
...
以下脚本会将 GitLab 的软件源添加到 Zypper 包管理器：
