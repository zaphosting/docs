...
title: "独立服务器：Windows 服务器上的用户管理"
description: "了解如何管理多个 Windows 服务器用户，实现安全的同时远程访问和个性化环境 → 立即了解更多"
...
Windows 操作系统自带用户管理功能，让你轻松管理额外用户。使用独立用户的好处包括通过各自的凭据实现同时（最多 2 个）远程桌面访问，拥有独立的桌面和文件访问权限，以及简单的权限系统。本指南将带你了解如何在 Windows 服务器上管理用户。

## 准备工作

首先通过 RDP 连接到你的 Windows 服务器。如果你需要帮助，请参考我们的[初始访问 (RDP)](dedicated-windows-userdp.md)指南。

:::important 管理员权限
确保你使用的是 **Administrator** 用户或任何具有管理员权限的用户登录 Windows 服务器，否则你将无法管理用户。
:::

你在 Windows 服务器上创建的每个用户，都可以使用自己的凭据通过远程桌面连接服务器。每个用户拥有独立的桌面、文件和程序，彼此互不干扰，只有具有管理员权限的用户才能查看所有内容。专门为所有用户安装的程序则是全局可用的。

使用普通的 Windows Server 许可证，**最多 2 个**账户可以同时登录并使用服务器。如果超过这个数量，后连接的用户会将先连接的用户踢出。除此之外，用户账户数量没有限制。
...
在概览页，选择 **管理其他账户**，进入主要用于管理用户的 **管理账户** 区域。
...
窗口会展开，方便你查找用户。点击 **立即查找**，返回用户列表，找到你刚创建的新用户，比如本例中的 `ZAP-Docs`。

![](https://screensaver01.zap-hosting.com/index.php/s/spQL9fTNd778bry/preview)

选中用户，点击 **确定**，关闭所有菜单并确认更改。

你已经成功创建了一个拥有远程桌面访问权限的新用户。建议用新用户的凭据通过 RDP 连接服务器，确保一切正常。
...
:::important 管理员权限
要管理用户，你必须使用主 **Administrator** 账户或具有管理员权限的账户，这样才能拥有所有必要权限。
:::

![](https://screensaver01.zap-hosting.com/index.php/s/yJPTWKieZNZXifH/preview)

在页面中，你可以使用多种功能管理该用户，包括修改用户名、密码、账户类型，或者删除用户。
