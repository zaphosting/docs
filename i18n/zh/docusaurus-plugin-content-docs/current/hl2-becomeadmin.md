...
要注册为管理员，必须安装 SourceMod 或 MetaMod。此外，你还需要你的 Steam 账号的 Steam ID。获取 Steam ID 有多种方式：你可以使用提供此服务的外部网站，或者直接通过游戏内控制台获取。
...
这会显示你的连接信息，包括你的 SteamID。接着，通过 FTP 连接服务器，打开位于：
...
**玩家名称** 不必与你的 Steam 账号名一致，仅供内部识别。`auth` 字段定义认证方式，保持为 `steam`。在 `identity` 中填写 SteamID32。`flags` 定义赋予管理员的权限：
...
多个标志可以组合使用（例如 `bc` 表示拥有踢人权限）。若要完全权限，只需使用 `z`。你可以通过在聊天中输入 `admin` 或在控制台输入 `sm_admin` 来打开管理员菜单。



## 管理员命令

下面是一些常用的管理员命令，你可以在服务器上使用它们。
