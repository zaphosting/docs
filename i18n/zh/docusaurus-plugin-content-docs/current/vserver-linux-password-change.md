...
title: "VPS：更改 Linux 服务器的 root 密码"
...
:::info
请注意，密码长度至少要有 8 个字符，并且必须包含大小写字母（a-z/A-Z）、数字（1-9）和特殊字符（!@=#$%&-?_;:.,）。如果不符合这些要求，密码将不会被应用到服务器上。
:::
...
登录控制台后，可以使用命令 `passwd` 来更改密码。之后需要再次输入密码以确认。如果两次输入的密码一致，控制台会确认密码更改成功。

如果两次密码不一致，控制台会终止操作，这时需要重新执行 `passwd` 命令。

:::info
重要提示：如果通过控制台更改密码，密码不会在我们的网页界面“访问与安全”中显示 3 小时。
:::
