...
| `/perm user add <uuid> <permission>`   | 直接为用户分配一个或多个权限。                              |
...
| `/perm group add <group> <permission>` | 为权限组添加权限。                                          |
...
Hytale 的命令系统为管理服务器、玩家和游戏行为提供了强大工具。掌握并使用这些命令，管理员能高效运营并定制自己的 Hytale 服务器环境。

如有更多问题或需要帮助，别犹豫，随时联系我们的支持团队，我们每天都在线为你服务！🙂
