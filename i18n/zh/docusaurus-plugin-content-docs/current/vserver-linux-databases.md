...
title: "VPS：数据库安装指南"
description: "了解如何在 Linux 上安装和配置各种数据库，提升性能与安全 → 立即了解"
sidebar_label: 安装数据库
services:
  - vserver
---
...
本指南提供了多种数据库的安装步骤。示例中使用的是 Ubuntu 20.04 作为操作系统，但我们网站上也提供了其他 Linux 发行版的等效命令。这些命令需通过 SSH 执行，如果你还不知道如何通过 SSH 连接服务器，可以查看这里：[初始访问（SSH）](vserver-linux-ssh.md)。
