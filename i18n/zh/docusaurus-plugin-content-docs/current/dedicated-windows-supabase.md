...
title: "独立服务器：在 Windows 上安装 Supabase"
...
确保所有依赖已安装且操作系统版本正确，避免安装 Supabase 时出现兼容性问题。
...
#### Git
Supabase 的数据会通过 GitHub 下载，因此需要先安装 Git。请在服务器上安装 [Git for Windows](https://git-scm.com/downloads/win)。
...
我们有一份完整的安装流程和 Docker 使用指南，详见我们的 [Docker 指南](dedicated-windows-docker.md)。
...
在暴露任何服务之前，先生成安全的 API 密钥。首先选择一个 40 字符的 JWT 密钥。你可以使用示例值，也可以自己生成。将此密钥安全地保存在本地，切勿泄露或提交到版本控制。使用该密钥生成 JWT，然后根据 Supabase 文档中的表单生成 anon 和 service API 密钥：https://supabase.com/docs/guides/self-hosting/docker#generate-api-keys
...
恭喜！你已经成功在独立服务器上安装并配置了 Supabase。我们还推荐你查看以下资源，能为你的服务器配置提供更多帮助和指导：
...
有具体问题没覆盖到？欢迎随时联系在线客服，我们每天都在线，随时帮你解决问题！🙂