...
title: "FiveM：txAdmin 界面"
description: "了解如何通过 txAdmin 的全功能网页面板高效管理和监控你的 FiveM 服务器 → 立即了解更多"
...
txAdmin 是一个完全免费的全功能网页面板，用于管理和监控你的 FiveM 服务器。它提供了丰富的功能，旨在让管理 FiveM 服务器变得轻松简单。在本指南中，我们将带你了解 txAdmin 界面，重点介绍其功能，并详细说明你可以用它做些什么。
...
导航菜单包含以下项目：**玩家**、**历史记录**、**白名单**、**管理员**、**设置** 和 **系统**。下面将对每个部分进行详细说明。

### 玩家

玩家部分提供玩家统计的总体概览，包括连接过的玩家总数、过去 24 小时内连接的玩家数，以及过去 24 小时和 7 天内的新玩家数。下方则列出了当前在线的玩家，显示他们的总游戏时长、首次连接时间和最后连接时间。
...
历史记录部分列出了所有对玩家发出的警告和封禁，包括玩家详细信息、原因、执行警告/封禁的管理员以及日期和时间。你还可以在这里撤销警告和封禁。

![](https://screensaver01.zap-hosting.com/index.php/s/qbLwEx39pmpY4sa/preview)

### 白名单

在白名单部分，你可以管理白名单功能。白名单允许你限制谁可以访问服务器。如果在 txAdmin 设置中启用了此选项，当新玩家首次连接时会自动创建白名单请求。你可以在此部分对这些请求进行管理。
...
在设置部分，你可以配置 txAdmin 和服务器本身的各种选项。由于设置项众多，分为以下几个分类：**全局**、**FXServer**、**封禁**、**白名单**、**Discord** 和 **游戏**。下面将详细介绍每个分类。

![img](https://screensaver01.zap-hosting.com/index.php/s/9r44PictxZLad6c/download)

#### 全局

在全局分类下，可以配置服务器名称、聊天/Discord 消息语言以及封禁模板。

![img](https://screensaver01.zap-hosting.com/index.php/s/7mr4D28GGqfPQQw/preview)

#### FXServer

FXServer 分类下可以设置服务器数据文件夹和 CFG 文件路径，以及额外参数，比如激活游戏模式/DLC、启用/禁用 OneSync、自动启动和静默模式。
...
封禁部分用于管理封禁功能。你可以定义是否主动检查账号封禁状态，以及当因封禁拒绝连接时显示的提示信息。

![img](https://screensaver01.zap-hosting.com/index.php/s/fTjM4EFbtB7cw4q/preview)

#### 白名单

白名单部分用于管理白名单功能。你可以启用或禁用白名单，并定义当因缺少白名单记录而拒绝连接时显示的提示信息。
...
在 Discord 分类下可以启用 Discord 集成，实现信息自动传输到关联的 Discord 服务器。启用前需配置 Discord 机器人的相关信息。

![img](https://screensaver01.zap-hosting.com/index.php/s/jSbXE9c23wjiKRf/preview)

#### 游戏

游戏分类下可以启用游戏内的 txAdmin 菜单，允许管理员通过 /tx 命令访问和管理菜单。还可以自定义布局、切换按钮配置以及通知/警告设置。

![img](https://screensaver01.zap-hosting.com/index.php/s/fLo976YMdbYkHts/preview)

### 系统

在设置部分，你还可以看到一些重要的通用选项和信息，分为以下几个分类：**主操作**、**诊断**、**控制台日志** 和 **操作日志**。下面将详细介绍每个分类。



#### 主操作

主操作中包含重置 FXServer、备份数据库、清理数据库以及定义玩家因长时间不活跃而从白名单移除的选项。
...
诊断部分显示有关环境、txAdmin 运行时、诊断报告、FXServer /info.json 以及进程的所有相关信息。
...
仪表盘显示服务器的所有重要通用信息。通过统计数据可以实时监控资源利用率和性能。侧边栏显示服务器状态（启动、在线或停止）。你可以从这里启动、停止或重启服务器，一键踢出所有玩家，发送公告，还能设置定时重启。侧边栏右侧显示当前在线玩家信息。
...
实时控制台提供服务器活动的实时信息。你可以查看活动详情、警告和错误信息，是调试问题时分析错误信息的重要工具。

![](https://screensaver01.zap-hosting.com/index.php/s/PDyPa7TfsHgTbAD/preview)

### 资源
资源部分展示所有资源的概览，无论是通过模板（配方）预装的，还是你后续添加的。你可以根据需要重启或停止它们。

![](https://screensaver01.zap-hosting.com/index.php/s/QJZnMTCqQpx92EL/preview)

### 服务器日志

服务器日志允许你监控服务器上的所有活动。例如，玩家连接或断开、聊天消息、游戏内死亡、菜单操作、执行的命令和系统事件等详细信息。
