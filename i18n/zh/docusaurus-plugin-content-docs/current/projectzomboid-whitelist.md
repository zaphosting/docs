...
通过 FTP 连接到你的服务器，打开服务器设置文件。该文件通常位于 `Zomboid/Server/` 目录下，文件名通常是你的服务器名，比如 `servertest.ini` 或 `<ServerName>.ini`。找到 `Open` 设置项，将其设置为 false。
...
如果要移除某个用户的访问权限，使用专用命令将其从白名单中删除。

```text
/removeuserfromwhitelist "username"
```

如果之前开启过开放注册，想把当前已连接的账号转换成白名单账号，可以使用将所有当前连接用户加入白名单的命令。
...
启用 `Open=false` 后，未通过 `/adduser` 添加的新用户应该无法加入。用一个已在白名单的账号测试确认是否能正常访问。

如果未列入白名单的用户仍能加入，请确认你编辑的是当前生效的 `<ServerName>.ini` 文件，并且已经重启了服务器。

## 总结

只要以上步骤都正确执行，你的白名单功能就已激活，可以精准控制谁能加入服务器。如果访问权限仍有问题，建议重启服务器一次，并再次检查文件或命令输出，确认更改已生效。
