---
id: tf2-fastdl
title: "Team Fortress 2: Ställ in FastDL"
description: "Upptäck hur du optimerar leverans av spelinnehåll med FastDL för snabbare nedladdningar och mindre serverbelastning → Läs mer nu"
...
FastDL (Fast Download) är en metod för att leverera custom content som kartor, ljud eller modeller via en extern webbserver istället för spelservern själv. Det ger snabbare nedladdningar för spelare och minskar belastningen på servern.
...
Du behöver en aktiv Team Fortress 2-server hos ZAP-Hosting och ett extra webspace-paket. FTP-åtkomst eller tillgång till Filhanteraren måste finnas både för spelservern och webspace.
...
I root-katalogen på ditt webspace skapar du en mapp som heter `fastdl`. Inuti den skapar du undermappar som `maps`, `sound` eller `models` beroende på vad du vill leverera. Mappstrukturen måste exakt spegla den som används på din Team Fortress 2-server.
...
Testa gärna genom att öppna länken i din webbläsare och kontrollera att filerna går att nå.

## Konfigurera spelservern

I ZAP-gränssnittet under spelserveradministration, öppna `server.cfg` via sidan **Configs** och lägg in följande rader:
...
FastDL går snabbt och säkert att sätta upp med ditt egna ZAP webspace. Det ger snabb och effektiv leverans av custom content och förbättrar upplevelsen för alla som spelar på din Team Fortress 2-server.
