...
title: "VPS: Installation av OpenVPN"
...
När detta är klart, kör ``crontab -e`` och välj **nano-editorn** [1]. Lägg till denna rad längst ner i filen:
``` @reboot /usr/sbin/tunscript.sh || exit 1 ```

Spara filen med `CTRL+O`, bekräfta med `Y` och avsluta med `Enter`. Tryck `CTRL+X` för att stänga. Nu ska skriptet köras automatiskt efter omstart.
...
En dialogruta dyker upp där du får välja mellan UDP eller TCP. Vi rekommenderar UDP, så skriv `1` och tryck `Enter`. Därefter blir du ombedd att ange en port för OpenVPN, vi rekommenderar att du använder standardporten.
...
Efter detta ska du ange ett klientnamn. Du kan välja vilket namn du vill, till exempel namnet på din enhet.

## Konfiguration

### Lägg till fler klienter

Om du vill skapa flera anslutningar är det smart att skapa flera konton. För att skapa ett nytt konto, kör samma kommando igen:
```
wget https://git.io/vpn -O openvpn-install.sh && bash openvpn-install.sh
```
...
Grattis, du har nu installerat och konfigurerat OpenVPN! Har du fler frågor eller problem är du alltid välkommen att kontakta vår support, som finns tillgänglig varje dag!
