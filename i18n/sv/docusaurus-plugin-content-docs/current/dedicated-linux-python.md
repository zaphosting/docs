...
Navigera först till mappen där du vill skapa din venv med `cd`. När du är där kör du `python3 -m venv .` för att installera nödvändiga filer på den platsen.

### Aktivera & avaktivera

För att köra kommandon som `pip install` inuti din venv måste du aktivera den med `source /bin/activate`. Nu körs allt i konsolen bara inom venv och skript får bara tillgång till lokalt installerade paket.
