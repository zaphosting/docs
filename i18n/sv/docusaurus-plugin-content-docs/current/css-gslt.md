---
id: css-gslt
title: "Counter-Strike: Source: Skapa en GSLT (Steam Game Server Login Token)"
description: "Förstå hur du autentiserar spelservrar med Steams GSLT för offentlig listning och VAC-skydd → Lär dig mer nu"
...
När du är inloggad kan du skapa en ny token via formuläret på sidan. I fältet **App ID** anger du App ID `220` som tillhör **Counter-Strike: Source**. Se till att ange rätt ID, eftersom token inte fungerar med fel värde.
...
:::info Driva flera Counter-Strike: Source-servrar
Om du driver flera **Counter-Strike: Source**-servrar behöver du en GSLT för varje server. Det är <u>**inte**</u> möjligt att driva flera servrar med en och samma GSLT.
:::
