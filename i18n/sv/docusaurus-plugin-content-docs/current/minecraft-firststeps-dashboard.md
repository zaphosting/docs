...
title: "Minecraft: Översikt Dashboard"
description: "Upptäck hur du effektivt hanterar din Minecraft-server med en tydlig översikt över funktioner och alternativ → Lär dig mer nu"
...
:::warning
Användning av DDoS Manager är endast möjlig om antingen en egen IP-adress eller optionen "DDoS Manager Overview" är bokad som tillägg. Om dessa komponenter saknas i ditt nuvarande paket kan du lägga till dem när som helst genom att uppgradera.
:::

### Tebex Shop

Med Tebex kan du göra din spelserver till en kassako genom att sälja virtuella varor och ranks för riktiga pengar. Det är ett kraftfullt och pålitligt verktyg speciellt designat för spelserveradministratörer som vill erbjuda en bättre spelupplevelse för sina spelare.
...
:::warning
Hittar du ingen hjälp i informationen eller förstår du den inte? Ingen fara! Vi finns dagligen i supporten. Skapa bara en **[Ticket](https://zap-hosting.com/en/customer/support/)** och förklara ditt problem för oss. Vi kollar på det så snabbt vi kan och hjälper dig att lösa det! :)
:::
...
**Live Console** ger dig en realtidsvy över alla aktuella händelser på din server. Här ser du information om spelarnas aktiviteter samt eventuella fel- och systemmeddelanden. Denna översikt ger dig en helhetsbild av vad som händer på din server och hjälper dig att upptäcka problem tidigt.

![](https://screensaver01.zap-hosting.com/index.php/s/dktDADkq3bzG3F6/preview)

Utöver att övervaka händelser kan du även skicka kommandon direkt till servern i **Live Console**. Det kan vara användbart för att justera inställningar, utföra åtgärder eller snabbt reagera på problem som uppstår.
...
Om vårt övervakningssystem upptäcker en generell driftstörning meddelas våra tekniker automatiskt och tar hand om problemet så snabbt som möjligt.

I detta avsnitt hittar du en översikt över driftstörningshistoriken för värdsystemet där din spelserver körs. Mer info och den senaste statusen hittar du på vår nya [statussida](https://status.zap-hosting.com/).

## Inställningar

I avsnittet **Inställningar** hittar du alla nödvändiga sidor för att konfigurera din server. De tillgängliga alternativen för serverkonfiguration listas i detalj nedan.

### Inställningar

Under inställningar hittar du en översikt över de vanligaste inställningsalternativen för din server. Dessa inkluderar även korta förklaringar. Använd gärna inställningssidan om du är ny inom serveradministration.
...
Config Editor låter dig redigera konfigurationsfilerna direkt på webbplatsen i klassisk vy, utan förenklad vy. Detta ger dig möjlighet att anpassa serverns inställningar efter dina personliga preferenser.
...
:::warning
Har du märkt att vissa ändringar delvis skrivs över? Var medveten om att vissa värden hämtas från inställningssidan. Vill du göra ändringarna direkt i config-filen måste du först avaktivera motsvarande alternativ på inställningssidan.
:::
...
Under **Versioner** kan du se och hantera den aktuella versionen av din spelserver. Här kan du uppdatera till senaste versionen när som helst eller aktivera automatiska uppdateringar för att alltid ha en uppdaterad server.
...
Vill du schemalägga serveromstarter vid specifika tider? Restart-planeraren låter dig ställa in automatiska omstarter av din spelserver vid valda tidpunkter eller med jämna mellanrum.

![](https://screensaver01.zap-hosting.com/index.php/s/Y6WciDS7YP98P4m/preview)

### DNS-inställningar

Med ZAP DNS-inställningar kan du skapa lättare adresser för din tjänst. Du kan använda en subdomän av någon av våra domäner eller skapa en DNS-post på dina egna domäner för en adress som är mer personlig och lättare att komma ihåg.

![](https://screensaver01.zap-hosting.com/index.php/s/tM5FGTHRSbZA5rN/preview)

### Spelarhantering

Spelarhanteringen låter dig snabbt och enkelt hantera dina spelare. Lägg till spelare som operatörer eller banna oönskade spelare med bara några klick.
...
Med **FTP Browser** får du all nödvändig info för att ansluta till FTP-accessen. FTP-accessen ger dig direkt tillgång till filerna på din spelserver. Vi rekommenderar att använda ett externt FTP-program som **FileZilla** eller **WinSCP** för bästa användarupplevelse. Om du är osäker på hur FTP fungerar kan guiden [Access via FTP](gameserver-ftpaccess.md) vara till hjälp.

![](https://screensaver01.zap-hosting.com/index.php/s/sX85i9HyzR9wmAM/preview)

**FileZilla:** FileZilla är troligen den enklaste och mest kända FTP-klienten. Installera, starta och skriv in dina inloggningsuppgifter i den övre raden som du hittar lite längre upp på denna sida. Klicka snabbt på "Anslut" så är du direkt inloggad på servern och kan hantera filerna.

**WinSCP:** WinSCP är egentligen för SCP och SFTP (de krypterade varianterna av FTP), men fungerar lika bra med vanlig FTP. Programmet är lite mer avancerat än FileZilla och passar bättre för vana användare.

### Databaser

Utöver våra spelserverprodukter erbjuder vi även 10 inkluderade databaser. Dessa behövs till exempel när data ska sparas i en databas. Under **Databaser** kan du skapa en databas och se inloggningsuppgifter. Du kan även aktivera backup-funktionen och importera befintliga databasbackuper.
...
Avsnittet **Vanliga frågor** ger en översikt över de vanligaste frågorna från våra kunder. Här hittar du hjälpsamma svar och detaljerad info om olika ämnen för att snabbt och effektivt hjälpa dig.
...
**Nyheter** ger dig en komplett översikt över alla meddelanden och nyheter relaterade till din tjänst. Här hittar du viktig info om olika saker som:
