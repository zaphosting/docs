...
Utöver att övervaka händelser kan du även skicka kommandon direkt till servern i **Live Console**. Det är användbart för att justera inställningar, utföra åtgärder eller snabbt reagera på problem.
...
Config Editor låter dig redigera konfigurationsfiler direkt på webbplatsen i klassisk vy, utan förenklad vy. Detta ger dig möjlighet att anpassa serverinställningarna efter dina personliga preferenser.
...
Under **Spel** kan du alltid byta spelvariant på ditt befintliga spel eller installera ett annat spel. Spel med samma slot-pris kan väljas direkt under **Tillgängliga Spel**. För andra spel krävs först en justering av slot-priset. Mer info finns i guiden [Spelbyte](gameserver-gameswitch.md).
...
**FileZilla:** FileZilla är troligen den enklaste och mest kända FTP-klienten. Installera, starta och fyll i dina inloggningsuppgifter i den övre raden som du hittar här på sidan. Klicka snabbt på "Anslut" så är du inne på servern och kan hantera filerna.

**WinSCP:** WinSCP är egentligen för SCP och SFTP (krypterade varianter av FTP) men fungerar lika bra med vanlig FTP. Programmet är lite mer avancerat än FileZilla och passar bättre för vana användare.
