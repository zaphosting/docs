---
id: tf2-gslt
title: "Team Fortress 2: Skapa en GSLT (Steam Game Server Login Token)"
description: "Lär dig hur du autentiserar din dedikerade spelserver med en giltig Steam Game Server Login Token för korrekt listning och VAC-skydd → Läs mer nu"
...
När du är inloggad kan du skapa en ny token via formuläret på sidan. I fältet **App ID** anger du App ID `440` som tillhör **Team Fortress 2**. Se till att ange rätt ID, eftersom token inte fungerar med fel värde.

I fältet **Memo** kan du lägga till en valfri beskrivning. Det hjälper dig att senare identifiera tokenens syfte, till exempel serverns namn eller roll. När du har fyllt i båda fälten klickar du på **Create** för att generera token. Token visas sedan och kan användas i din servers startkonfiguration.
...
:::info Flera Team Fortress 2-servrar
Om du kör flera **Team Fortress 2**-servrar behöver du en GSLT för varje server. Det är <u>**inte**</u> möjligt att köra flera servrar med en och samma GSLT.
:::
