...
title: "Starbound: Översikt över kontrollpanelen"
description: "Upptäck hur du effektivt hanterar och optimerar din Starbound-server med en tydlig översikt över alla viktiga funktioner och inställningar → Lär dig mer nu"
...
Med denna information får du en bra insikt i hur din server används. Dessutom är statistiken ofta hjälpsam för att identifiera eventuella prestandaproblem och begränsa deras orsaker. Du kan till exempel spåra när belastningen ökade och om det är kopplat till vissa händelser eller ändringar. Detta är särskilt viktigt vid plötsliga problem, eftersom det gör att du kan ta riktade åtgärder för felsökning och systematisk analys av möjliga orsaker.
...
:::warning
Användning av DDoS Manager är endast möjlig om antingen en egen IP-adress eller tillägget "DDoS Manager Översikt" har bokats som extra option. Om dessa komponenter saknas i ditt nuvarande paket kan du lägga till dem när som helst genom att uppgradera.
:::
...
:::warning
Hittar du ingen hjälp i informationen eller förstår du den inte? Ingen fara! Vi finns dagligen tillgängliga i supporten. Skapa bara ett **[supportärende](https://zap-hosting.com/en/customer/support/)** och förklara ditt problem för oss. Vi kollar på det så snabbt vi kan och hjälper dig att lösa problemet! :)
:::

### Händelselogg

**Händelseloggen** ger dig en komplett översikt över alla aktiviteter relaterade till serveradministrationen. Här kan du när som helst se när en server startade, stoppade eller kraschade, samt när uppdateringar, säkerhetskopior eller nya installationer genomfördes.
...
Om vårt övervakningssystem upptäcker en generell driftstörning meddelas våra tekniker automatiskt och tar hand om problemet så snart som möjligt.
...
Under inställningar hittar du en översikt över de vanligaste konfigurationsalternativen för din server. Dessa inkluderar även korta förklaringar. Användningen av inställningssidan rekommenderas särskilt för nybörjare inom serveradministration.
...
Config-filredigeraren låter dig redigera konfigurationsfilerna direkt på webbplatsen i klassisk vy, utan förenklad vy. Denna funktion gör att du kan anpassa serverns inställningar efter dina personliga preferenser.
...
Dessutom kan du aktivera e-postnotiser för automatiska uppdateringar så att du alltid hålls informerad och uppdaterad.
...
Med **FTP Browser** får du all nödvändig information för att upprätta en anslutning till FTP-accessen. FTP-accessen ger dig direkt tillgång till filerna på din spelserver. Vi rekommenderar att du använder ett externt FTP-program som **FileZilla** eller **WinSCP** för bästa användarupplevelse. Om du är osäker på hur FTP fungerar rekommenderar vi att du kollar in vår guide [FTP Access](gameserver-ftpaccess.md) som kan hjälpa dig.
...
Utöver våra spelserverprodukter erbjuder vi även 10 inkluderade databaser. Dessa databaser behövs till exempel när data ska sparas i en databas. Under **Databaser** kan du skapa en databas och se åtkomstuppgifterna. Du kan också aktivera backup-funktionen och importera befintliga databas-backuper.
...
I inställningarna kan du ange om säkerhetskopior ska skapas automatiskt och om detta ska ske dagligen eller veckovis. Dessutom kan du bestämma om de äldsta säkerhetskopiorna ska tas bort vid behov, till exempel när lagringsutrymmet börjar ta slut.

![img](https://screensaver01.zap-hosting.com/index.php/s/nm9za5LTMEcNsga/preview)

Vi erbjuder som standard 10 GB lagringsutrymme på lagringsservern för säkerhetskopior för våra spelserverprodukter. Om du behöver mer utrymme kan detta uppgraderas mot en avgift. På så sätt säkerställer du att du alltid har tillräckligt med plats för dina säkerhetskopior och kan komma åt äldre säkerhetskopior vid behov.
