---
id: gmod-gslt
title: "Garry’s Mod: Skapa en GSLT (Steam Game Server Login Token)"
description: "Lär dig hur du autentiserar din spelserver med en Steam Game Server Login Token för korrekt listning och VAC-skydd → Läs mer nu"
...
**GSLT (Game Server Login Token)** är en inloggningstoken som Steam/Valve kräver för att köra dedikerade servrar för vissa spel. Den är kopplad till ett Steam-konto och säkerställer att servern är korrekt autentiserad.
...
När du är inloggad kan du skapa en ny token via formuläret på sidan. I fältet **App ID** anger du App ID `4000` som tillhör **Garry’s Mod**. Se till att ange rätt ID, eftersom token inte fungerar med fel värde.
...
:::info Köra flera Garry’s Mod-servrar
Om du kör flera **Garry’s Mod**-servrar behöver du en GSLT för varje server. Det är <u>**inte**</u> möjligt att köra flera servrar med en och samma GSLT.
:::
...
Din GSLT är nu skapad och tillagd i serverkonfigurationen. Har du fler frågor eller behöver hjälp, tveka inte att kontakta vår support som finns tillgänglig varje dag för att hjälpa dig! 🙂
