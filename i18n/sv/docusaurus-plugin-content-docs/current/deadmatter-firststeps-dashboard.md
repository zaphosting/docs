...
title: "Dead Matter: Översikt Dashboard"
description: "Upptäck hur du effektivt hanterar din Dead Matter-server med en tydlig översikt över funktioner och alternativ anpassade efter dina behov → Lär dig mer nu"
...
I dashboarden kan du också starta, stoppa eller starta om din tjänst för att ha full kontroll över driften. Dessutom kan du öppna live-konsolen för att få realtidsinformation om serverns status och agera direkt vid behov.
...
Med denna information får du en bra insikt i hur din server används. Statistik är också ofta hjälpsam för att identifiera eventuella prestandaproblem och begränsa deras orsaker. Till exempel kan du spåra när belastningen ökade och om det är kopplat till vissa händelser eller ändringar. Detta är särskilt viktigt vid plötsliga problem, eftersom det gör att du kan ta riktade åtgärder för felsökning och systematisk analys av möjliga orsaker.

### DDoS Manager

Få en överblick över tidigare DDoS-attacker för att bättre bedöma potentiella hot och mönster. Aktivera notiser för pågående DDoS-attacker på din server för att snabbt kunna reagera vid incidenter.
...
:::warning
Hittar du ingen hjälp i informationen eller förstår du den inte? Ingen fara! Vi finns dagligen tillgängliga i supporten. Skapa bara en **[Ticket](https://zap-hosting.com/en/customer/support/)** och förklara ditt problem för oss. Vi kollar på det så snabbt som möjligt och hjälper dig att lösa det! :)
:::
...
Var din server tillfälligt offline och du kan inte förklara varför? En titt i händelseloggen och loggfilerna gav inga svar? Det kan vara så att problemet inte ligger hos din server utan hos värdsystemet där din server körs.
...
Under inställningar hittar du en översikt över de vanligaste inställningsalternativen för din server, inklusive korta förklaringar. Den här sidan rekommenderas särskilt för nybörjare inom serveradministration.
...
Config Editor låter dig redigera konfigurationsfilerna direkt på webbplatsen i klassisk vy, utan förenklad vy. Här kan du anpassa serverinställningarna efter dina personliga preferenser.
...
Under **Spel** kan du alltid byta spelvariant på ditt befintliga spel eller installera ett helt nytt spel. Spel med samma slot-pris kan väljas direkt under **Tillgängliga spel**. För andra spel krävs först en justering av slot-priset. Mer info finns i guiden [Spelbyte](gameserver-gameswitch.md).
...
**FileZilla:** FileZilla är troligen den enklaste och mest kända FTP-klienten. Installera, starta och fyll i dina inloggningsuppgifter i den övre raden som du hittar lite längre upp på sidan. Klicka på "Anslut" och du är direkt inloggad på servern och kan hantera filerna.

**WinSCP:** WinSCP är egentligen för SCP och SFTP (krypterade varianter av FTP) men fungerar lika bra med vanlig FTP. Programmet är lite mer avancerat än FileZilla och passar bättre för erfarna användare.
...
Under **Backuper** kan du skapa både manuella och automatiska backuper för din server och tillhörande databas. Det gör det enklare att återställa data vid problem eller dataförlust.
...
Vi erbjuder omfattande dokumentation för våra produkter som fungerar som en kunskapsbas för de vanligaste frågorna och funderingarna. Här finns guider och hjälp inom olika områden som hjälper dig att använda och hantera din produkt.
