...
title: "The Mean Greens: Översikt över kontrollpanelen"
description: "Upptäck hur du effektivt hanterar och optimerar din The Mean Greens-server för maximal prestanda och kontroll → Lär dig mer nu"
...
Du är intresserad av en **The Mean Greens-server** eller äger redan en, men saknar fortfarande en komplett översikt över administrationen och hur du hittar alla nödvändiga inställningar? Speciellt i början kan det kännas lite klurigt. Men oroa dig inte, vi ger dig en detaljerad introduktion till administrationen av din tjänst nedan, så att du får bästa möjliga överblick.

Efter att ha gått igenom den här guiden kommer du enkelt kunna hantera din server effektivt och snabbt hitta alla nödvändiga inställningar och funktioner. Här får du en strukturerad översikt som hjälper dig steg för steg att bekanta dig med serveradministrationen och utöka din kunskap på ett målmedvetet sätt. På så sätt kan du säkerställa att din **The Mean Greens-server** är optimalt anpassad efter dina behov och att du får ut det mesta av de tillgängliga möjligheterna.
...
:::warning
DDoS Manager kan endast användas om antingen en egen IP-adress eller tillägget "DDoS Manager Översikt" är bokat som tilläggsoption. Saknas dessa i ditt nuvarande paket kan du lägga till dem när som helst genom att uppgradera.
:::
...
Var din server offline en stund och du vet inte varför? En titt i händelseloggen och loggfilerna gav inga svar? Det kan vara så att problemet inte ligger direkt hos din server, utan hos värdsystemet där din server körs.

Om vårt övervakningssystem upptäcker en generell driftstörning får våra tekniker automatiskt en notis och tar hand om problemet så snabbt som möjligt.

Här hittar du en översikt över driftstörningar på värdsystemet där din spelserver körs. Mer info och den senaste statusen hittar du på vår nya [statussida](https://status.zap-hosting.com/).
...
Config-filredigeraren låter dig redigera konfigurationsfilerna direkt på webbplatsen i klassisk vy, utan förenklad vy. Detta ger dig möjlighet att anpassa serverinställningarna efter dina personliga preferenser.

Med editorn får du direkt tillgång till alla tillgängliga alternativ och kan finjustera dem exakt. Perfekt för avancerade användare som vill ha full kontroll och vet exakt vilka ändringar de vill göra.
...
Under **Spel** kan du alltid byta spelvariant på ditt befintliga spel eller installera ett helt annat spel. Spel med samma slot-pris kan väljas direkt under **Tillgängliga spel**. För andra spel krävs först en justering av slot-priset. Mer info om detta finns i guiden [Spelbyte](gameserver-gameswitch.md).
...
Med **FTP Browser** får du all nödvändig info för att koppla upp dig mot FTP-accessen. FTP-accessen ger dig direkt åtkomst till filerna på din spelserver. Vi rekommenderar att använda ett externt FTP-program som **FileZilla** eller **WinSCP** för bästa användarupplevelse. Om du är ny på FTP kan du kika på vår guide [FTP Access](gameserver-ftpaccess.md) som kan hjälpa dig.
...
**WinSCP:** WinSCP är egentligen gjort för SCP och SFTP (de krypterade varianterna av FTP), men funkar lika bra med vanlig FTP. Programmet är lite mer avancerat än FileZilla och passar bättre för vana användare.
...
I inställningarna kan du välja om backuper ska skapas automatiskt och om det ska ske dagligen eller veckovis. Du kan också bestämma om de äldsta backuperna ska tas bort vid behov, till exempel när lagringsutrymmet börjar ta slut.

![img](https://screensaver01.zap-hosting.com/index.php/s/nm9za5LTMEcNsga/preview)

Vi erbjuder som standard 10 GB lagringsutrymme på backup-servern för våra spelserverprodukter. Behöver du mer utrymme kan det uppgraderas mot en avgift. På så sätt säkerställer du att du alltid har plats för dina backuper och kan komma åt äldre backuper vid behov.
...
**Nyheter** ger dig en komplett översikt över alla meddelanden och nyheter kopplade till din tjänst. Här hittar du viktig info om olika saker som:
