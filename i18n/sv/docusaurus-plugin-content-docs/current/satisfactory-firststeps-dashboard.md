...
title: "Satisfactory: Översikt av Kontrollpanelen"
description: "Upptäck hur du effektivt hanterar din Satisfactory-server och optimerar dess funktioner efter dina behov → Lär dig mer nu"
...
**Kontrollpanelen** för din server är hjärtat i administrationen. Här presenteras all grundläggande och viktig information om din tjänst på ett tydligt sätt. Det inkluderar till exempel information om plats, status, IP: Port, använd minne, pågående spel, servernamn, karta och antal tillgängliga platser. Dessutom visas även användningen av CPU, minne och RAM.
...
- Grundinställningar (Aktivt sparspel (utan filändelsen .sav), Servernamn, Autosparintervall)
...
Config-filredigeraren låter dig redigera konfigurationsfilerna direkt från webbplatsen på det klassiska sättet, utan förenklad vy. Detta alternativ gör att du kan anpassa serverns inställningar och konfigurera dem efter dina personliga preferenser.
...
Med save game-managern kan du enkelt spara, återställa och importera spelprogression. Detta förenklar hanteringen av dina sparfiler och hjälper dig att hålla koll på din utveckling utan krångel.

![](https://screensaver01.zap-hosting.com/index.php/s/BADmpNYcn7Hkktt/preview)

Har du redan en befintlig sparfil som du vill integrera i save game-managern? Inga problem! Använd bara save game-uppladdaren för att ladda upp din fil. Observera att filstorleken inte får överstiga maxgränsen på 100 MB. Annars måste sparfilen laddas upp manuellt via FTP.
...
**WinSCP:** WinSCP är egentligen avsett för SCP och SFTP (de krypterade varianterna av filöverföringsprotokollet FTP) men kan hantera FTP på samma sätt. Programmet är mer omfattande än FileZilla och passar avancerade användare.



### Databaser

Utöver våra spelserverprodukter erbjuder vi även 10 inkluderade databaser. Dessa databaser behövs till exempel när data ska sparas i en databas. Under **Databaser** kan du skapa en databas och se åtkomstuppgifterna. Du har också möjlighet att aktivera backup-alternativet och importera befintliga databasbackuper.
