...
title: "Satisfactory: Översikt Dashboard"
description: "Upptäck hur du effektivt hanterar din XY-server och optimerar dess funktioner efter dina behov → Lär dig mer nu"
...
Med Tebex har du möjligheten att göra din spelserver till cash genom att sälja virtuella varor och ranker för riktiga pengar. Det är ett kraftfullt och pålitligt verktyg designat speciellt för spelserveradministratörer som vill erbjuda en bättre spelupplevelse för sina spelare.
...
:::info
Hjälper inte informationen där eller förstår du den inte? Ingen fara! I så fall finns vi tillgängliga i supporten varje dag. Skapa bara en **[Ticket](https://zap-hosting.com/en/customer/support/)** och förklara ditt problem för oss. Vi kollar på det så snabbt som möjligt och hjälper dig att lösa problemet! :)
:::
...
Var din server tillfälligt offline och du kan inte förklara varför? En titt i händelseloggen och loggfilerna gav inte heller någon information? Det kan vara så att problemet inte ligger direkt hos din server, utan hos värdsystemet där din server körs.
...
I detta avsnitt hittar du en översikt över driftstörningshistoriken för värdsystemet där din spelservertjänst körs. Du hittar mer information och den senaste versionen av vår servicestatus på vår nya [statussida](https://status.zap-hosting.com/).
...
- Grundläggande inställningar (Aktivt sparspel (utan filändelsen .sav), Servernamn, Autosparintervall)
...
Med save game-managern kan du enkelt spara, återställa och importera spelprogression. Detta förenklar hanteringen av dina sparfiler och hjälper dig att hålla koll på dina framsteg utan krångel.

![](https://screensaver01.zap-hosting.com/index.php/s/BADmpNYcn7Hkktt/preview)

Har du redan en befintlig sparfil som du vill integrera i save game-managern? Inga problem! Använd bara save game-uplodern för att ladda upp din fil. Observera att filstorleken inte får överstiga maxgränsen på 100 MB. Annars måste sparfilen laddas upp manuellt via FTP.
...
Vill du schemalägga serveromstarter vid specifika tider? Restart Planner låter dig ställa in schemalagda automatiska omstarter av din spelserver. Med denna funktion kan du ställa in omstarter vid specifika tider eller med jämna mellanrum.
...
I avsnittet **Verktyg** i din navigationspanel hittar du en rad underobjekt som hjälper dig att hantera din server. Dessa inkluderar **FTP Browser**, **Databaser** och **Backuper**. Nedan får du en detaljerad introduktion till dessa olika områden.



### FTP-Browser

Med **FTP Browser** får du all nödvändig information för att upprätta en anslutning till FTP-accessen. FTP-accessen ger dig direkt tillgång till filerna på din spelserver. Vi rekommenderar att använda ett externt FTP-program som **Filezilla** eller **WinSCP** för optimal användning. Om du inte är bekant med hur man använder FTP kan det hjälpa att kika på guiden [Access via FTP](gameserver-ftpaccess.md).
...
Utöver våra spelserverprodukter erbjuder vi även 10 inkluderade databaser. Dessa databaser behövs till exempel när data ska lagras i en databas. Under **Databaser** kan du skapa en databas och se åtkomstuppgifterna. Du har också möjlighet att aktivera backup-alternativet och importera befintliga databasbackuper.
