...
title: "Dedikerad Server: Linux Säkerhetstips"
...
Den här guiden innehåller tips och råd om hur du gör din Linux-server säkrare. Speciellt eftersom servrar måste nås externt, är grundläggande skydd mot oönskad åtkomst verkligen rekommenderat och bör inte förbises.
...
Som standard används lösenordsbaserad inloggning för SSH. Detta har dock den stora nackdelen att autentisering relativt enkelt kan kringgås med en brute force-attack, särskilt om du använder ett för enkelt lösenord för din SSH-inloggning. Så om du väljer att använda lösenordslösningen, använd ett **säkert** lösenord.

För att skydda din server ännu bättre mot oönskad SSH-åtkomst bör du aktivera autentisering enbart via SSH-nycklar och inaktivera lösenordsinloggning. Kolla in [SSH Key](vserver-linux-sshkey.md)-guiden som förklarar hur du genererar och lägger till SSH-nycklar.
...
Tjänster som SSH eller FTP använder alltid samma portar som standard (några av dem finns i tabellen ovan). Om en extern illasinnad aktör vill brute force:a din servers SSH-tjänst måste de först veta vilken port som används för SSH. Om du inte ändrar dessa portar, är port 22 och 21 vanligtvis målet för att köra kommandon direkt på servern eller komma åt filer via FTP.

För att förhindra detta rekommenderar vi att du sätter portarna för standardtjänsterna som användardefinierade. I nästa del av guiden kan du läsa hur:
...
Öppna sedan filen med nano (som root eller med *sudo*).
```
...
Öppna filen med nano och ta bort "#" framför `port` och skriv in din önskade port efter. Observera informationen ovan så att du inte anger en ogiltig port.
...
Vilken metod du väljer är upp till dig. Beroende på användning kan du behöva IPTables flexibilitet, men ibland räcker UFW gott och väl. (t.ex. om du bara vill öppna/stänga portar, vilket ändå är bättre än inget!)
...
Observera att regeln bara aktiveras för **port 22** (värdet efter `--dport`) och kommandon för andra tjänster måste anpassas.

:::note
Följande kommandon fungerar kanske inte på alla Linux-distributioner, men de fungerar på de allra flesta populära distros.
:::

Logga först in på din Linux-server. Behöver du hjälp med detta, följ vår guide [Initial access (SSH)](vserver-linux-ssh.md). Kör sedan följande kommandon i ordning.
...
1.  Den första regeln släpper paket om det finns fler än 2 anslutningar i `TIME_WAIT`-status till port 22 från en enda IP.
2.  Den andra regeln lägger till IP-adressen för en ny anslutning i `recent`-listan.
3.  Den tredje regeln släpper nya anslutningar från IP-adresser som försökt ansluta mer än 2 gånger under den senaste sekunden.
...
Logga in på din Linux-server. Behöver du hjälp, följ vår guide [Initial access (SSH)](vserver-linux-ssh.md). Instruktionerna är testade på Debian och Ubuntu men bör fungera på andra distros också.
...
:::tip
Brandväggen (oavsett IPTables eller UFW) kan bara "tanklöst" räkna anslutningsförsök och blockera därefter. Med Fail2Ban kan du kolla loggfiler efter avvikelser. Nästa avsnitt visar hur du installerar och aktiverar Fail2Ban.
:::

## Extra skydd med Fail2Ban

Fail2Ban är en tjänst som blockerar IP-adresser som försöker ansluta till servern med troligen illasinnade avsikter. Fail2Ban övervakar loggfiler efter avvikelser och skyddar ditt system effektivt på ett relativt enkelt sätt.

Efter installation kommer Fail2Ban med förkonfigurerade inställningar för vanliga tjänster som:
- apache
- lighttpd
- sshd
- qmail
...
```
Fail2Ban använder denna loggfil och övervakar misslyckade autentiseringar. Eftersom loggfilen innehåller angriparens IP-adress kan Fail2Ban blockera den efter några misslyckade försök.

### Installera Fail2Ban

Logga in på din Linux-server. Behöver du hjälp, följ vår guide [Initial access (SSH)](vserver-linux-ssh.md). Kör kommandon som root eller med *sudo*.
...
Fail2Ban är nu installerat men inte aktivt eller konfigurerat. Titta i `/etc/fail2ban` där följande filer bör finnas:
```
...
Scrolla ner lite tills du hittar detta och justera så här som exempel:
...
Nu är standardinställningarna klara. För att övervaka SSH-tjänsten, scrolla ner till `[sshd]`-taggen. Om du ändrat port ska du ange den under `port`.
...
```
:::tip
Som du ser går det att göra individuella inställningar per tjänst (här t.ex. `maxretry` som är lägre än standard). Även om du gjort generella inställningar kan du konfigurera de flesta inställningar per tjänst. Om du inte gör det används standardinställningen.
:::
...
Om du har tillgång till en VPN eller en annan server kan du testa att bli blockerad av Fail2Ban för att se om det funkar. Med VPN eller hotspot via mobilen får du en annan IP som du kan testa med.

:::danger
Testa inte detta på ditt vanliga nätverk, då kan din egen IP blockeras och du **blir utelåst**.
:::
...
Som du ser nekas nu anslutningen (`Connection refused` istället för `Permission denied`).

Visa status för Fail2Ban. Här ser du att en IP är blockerad.
```
...
Vill du låsa upp en IP igen kan du göra det med: `fail2ban-client set sshd unbanip {din IP}`.

:::info
Om du har ovanligt många IP-blockeringar är det smart att öka bantiden för varje misslyckat försök för att minska antalet inloggningsförsök.
:::
...
bantime = 1h
# Bantiden ökar för varje blockering av denna IP
...
## Skydda webbservrar med Cloudflare

Många använder Cloudflare som DNS-hanterare för sin domän. Cloudflare har inte bara ett av världens största nätverk som ger snabbare laddning, lägre latens och bättre upplevelse, utan skyddar också dina webbplatser mot DoS/DDoS-attacker, inklusive flooding och nya typer av attacker som dyker upp varje dag.
...
Din server är dock fortfarande åtkomlig utanför Cloudflare, så du måste begränsa åtkomst till port 80 och 443 (TCP) på din Linux-server och bara tillåta trafik från legitima Cloudflare-adresser.
...
Den här guiden har visat dig grundläggande och avancerade funktioner för att säkra din Linux-server. Om du följt alla rekommendationer som gäller för ditt system är din server redan mycket säkrare än tidigare – grattis!
