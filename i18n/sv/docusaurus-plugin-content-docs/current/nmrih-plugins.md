---
id: nmrih-plugins
title: "No More Room In Hell: Installera plugins på din egen server"
...
Lär dig hur du installerar Sourcemod och Metamod och hur du använder plugins på din server. Båda tilläggen låter dig anpassa spel som CS:S, CS:GO, DoD:S eller TF2 och utökar kraftigt de tillgängliga alternativen för serverhantering.
...
När ovanstående steg är klara kan du fortsätta med installationen. Ladda upp filerna till din server via FTP med en FTP-klient. Om du är ovan vid FTP och hur det fungerar, kolla in guiden: [Åtkomst via FTP](gameserver-ftpaccess.md)
...
Installationen av **Sourcemod** och **Metamod** är nu klar. Du kan ansluta till din server i spelet och använda kommandot ``sm version`` i konsolen för att kontrollera att Sourcemod och Metamod installerades korrekt. Utdatan ska se ut så här:
...
När Sourcemod och Metamod är installerade och fungerar som de ska kan du börja lägga till plugins. Ladda ner de plugins du vill installera på din server från [Sourcemod](https://sourcemod.net/), där du hittar en stor och växande samling som publicerats genom åren. För varje plugin, se till att du får motsvarande .smx-fil.
