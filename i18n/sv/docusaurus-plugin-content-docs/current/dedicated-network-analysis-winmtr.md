...
title: "Dedikerad Server: Identifiera nätverksproblem med WinMTR/MTR"
description: "Upptäck hur du diagnostiserar nätverksproblem genom att analysera utgående och inkommande rutter för att effektivt identifiera latens och paketförlust → Lär dig mer nu"
...
När du använder en dator med Mac OS operativsystem görs nätverksanalysen via MTR-applikationen. MTR är dock inte förinstallerat på Mac OS och finns inte heller i Apple Store. För att installera MTR behöver du först installera Homebrew som pakethanterare på din dator.
...
Anslut till din server via **[SSH](vserver-linux-ssh.md)**. För att installera MTR på servern, kör rätt installationskommando för serverns operativsystem:
...
Anslut till din server via **[Remote Desktop Connection](vserver-windows-userdp.md)**. För att installera WinMTR på servern, ladda först ner applikationen från ZAP-Hostings webbplats. Efter nedladdning, packa upp filen. Då får du den körbara filen `WinMTR.exe`.
...
MTR används genom att öppna terminalen och köra följande kommando:
...
I fältet `Host` skriver du in IP-adressen för din anslutning. Starta sedan rapporten. Låt applikationen köra i minst **en minut** för att samla tillräckligt med data. När du har tillräckligt med data, stoppa analysen och spara resultatet genom att klicka på `Export TEXT`.
...
I detta exempel sker paketförlust på serversidan, synligt på både utgående och inkommande rutter.
...
Utvärderingen visar att paketförlust sker på serversidan. Båda rutterna visar förluster vid de sista hoppen (Hop 7 och 8), vilket pekar på problem hos servern.
...
I detta exempel beskrivs höga latenstider på klientsidan, synligt på både utgående och inkommande rutter.
...
Utvärderingen visar att det finns hög latens på klientsidan. Båda rutterna visar ökad latens vid de första hoppen (Hop 1 och 2), vilket pekar på problem hos klienten.
...
I detta exempel beskrivs höga latenstider på serversidan, synligt på både utgående och inkommande rutter.
...
Utvärderingen visar att det finns hög latens på serversidan. Båda rutterna visar ökad latens vid de sista hoppen (Hop 7 och 8), vilket pekar på problem hos servern.
...
I detta exempel fungerar rutten från Klient X till servern utan problem, medan det finns routingproblem från Klient Y till servern som leder till paketförlust. Detta tyder på problem med routing hos internetleverantören (ISP).
...
Utvärderingen visar att det finns ett routingproblem hos ISP. Medan rutten från Klient X till servern är stabil, visar rutten från Klient Y betydande problem med paketförlust och hög latens.
...
I detta exempel sker paketförlust endast på den utgående rutten (från klient till server), medan den inkommande rutten (från server till klient) inte visar någon förlust. Detta kan tyda på problem i ett nätverkssegment mot servern, kanske orsakad av felaktiga routrar, belastning eller dålig anslutning längs vägen.
...
Utvärderingen visar att paketförlust endast sker på den utgående rutten. Förlusterna syns vid hoppen 4, 5 och 6, vilket pekar på problem längs vägen från klient till server, som belastning eller felaktiga nätverksenheter i ett segment av rutten. På den inkommande rutten finns däremot inga förluster, vilket betyder att anslutningen i motsatt riktning är stabil.
...
I detta exempel sker paketförlust endast på den inkommande rutten (från server till klient), medan den utgående rutten (från klient till server) inte visar någon förlust. Detta kan tyda på problem i ett nätverkssegment på den inkommande vägen.
...
Utvärderingen visar att paketförlust endast sker på den inkommande rutten. Förlusterna syns vid hoppen 4, 5 och 6 på den inkommande rutten, vilket pekar på problem längs vägen från server till klient.
