...
title: "Dedikerad Server: Installation av Mastodon"
description: "Upptäck hur du sätter upp ett säkert, självhostat Mastodon-socialt nätverk på Debian-baserade servrar för oberoende mikrobloggning → Lär dig mer nu"
...
När vi installerat beroenden för Mastodon kan vi installera nödvändiga systempaket. Det gör vi enkelt med följande två kommandon:  
```bash
apt update  
apt install -y \  
...
```
Låt paketen installeras, det kan ta en stund. Därefter måste vi konfigurera yarn, som är en paketmanager för den tidigare installerade Node.JS:  
```bash
corepack enable  
...
Nu installerar vi Ruby Manager `rbenv`, som gör hantering av Ruby-versioner mycket enklare. Installera den med dessa kommandon:  
```bash
...
```
Det kan ta en stund, så ha tålamod och fixa en kopp te eller kaffe under tiden. När det är klart kan du byta tillbaka till root-användaren med kommandot `exit`.

## Konfigurera PostgreSQL  
Mastodon använder PostgreSQL som databassystem. Konfiguration görs enkelt så här:  
```bash
sudo -u postgres psql  
```

Inne i PostgreSQL-prompten, skriv följande:  
```sql
...
```
Nästa steg är att installera några fler och sista beroenden för Ruby och Javascript. Gör så här:  
```bash
...
Nu kommer det lite kluriga. Du måste konfigurera din webbserver. Först skapar du en A-Record och/eller AAAA-Record i din DNS som pekar direkt till din server. Alternativt kan du peka root-record till din server.

Använd sedan följande två kommandon för att kopiera och aktivera Mastodons webbserverkonfiguration:  
```bash
...
```
Skriv in din domän. Du kommer bli tillfrågad om några uppgifter. I slutet frågar den om du vill omdirigera http till https automatiskt – vi rekommenderar att du aktiverar det.

## Skapa en Mastodon-tjänst  
Till sist skapar vi en Mastodon systemtjänst. Det är ganska rakt på sak.  
