...
title: "VPS: Sätt upp din dedikerade spelserver som en Linux-tjänst"
description: "Upptäck hur du skapar och hanterar Linux-tjänster för dedikerade spelservrar för automatisk start och enkel underhållning → Lär dig mer nu"
...
När nano-editorn är öppen, kopiera följande grundläggande innehåll som är en mall för tjänstfilen som du kan återanvända. Vi har två versioner, en för guider som använder SteamCMD och en annan för spelguider utan SteamCMD.
...
Låt oss bryta ner innehållet i filen för att förstå allt.
...
- `ExecStartPre` (endast SteamCMD): Här sätter vi upp samma SteamCMD-installationskommando som tidigare, som körs varje gång servern startas om för att säkerställa att den är uppdaterad. Du bör kopiera detta från respektive dedikerad spelserverguide, eller ersätta värdena manuellt med spelets värden.
- `ExecStart`: Den fördefinierade uppgiften som ska köras med tjänsten. Återigen, kopiera sökvägen från respektive dedikerad spelserverguide, eller ersätt värdena manuellt för att navigera till startfilen.

:::important Wine-kompatibilitetslager
För spel som kräver **Wine**-kompatibilitetslagret för att kunna köras, bör du inkludera kommandona **xvfb-run** och **wine** i `ExecStart`-parametern. Exempel för V-Rising:
```
/usr/bin/xvfb-run wine /home/steam/V-Rising-Server/start_server.bat
```

Se även till att ditt `ExecStartPre` SteamCMD-installationskommando inkluderar `+@sSteamCmdForcePlatformType`-parametern om det behövs för att tvinga fram en plattformsvariant.
:::

När du har anpassat alla värden för din dedikerade spelserver kan du spara filen och avsluta nano med `CTRL + X`, följt av `Y` för att bekräfta och sedan `ENTER`.
...
Slutligen, om du någonsin vill stoppa tjänsten från att starta automatiskt vid boot, kan du enkelt inaktivera den.
```
...
Du har nu framgångsrikt satt upp en tjänst för din dedikerade spelserver. Servern startar nu automatiskt vid serverboot.
