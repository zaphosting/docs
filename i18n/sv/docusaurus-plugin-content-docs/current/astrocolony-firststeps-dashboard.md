...
:::warning
Användning av DDoS Manager är endast möjlig om antingen en egen IP-adress eller alternativet "DDoS Manager Översikt" har bokats som tillägg. Om dessa komponenter saknas i ditt nuvarande paket kan du när som helst lägga till dem genom att uppgradera.
:::
...
Med **FTP Browser** får du all nödvändig information för att upprätta en anslutning till FTP-accessen. FTP-accessen ger dig direkt tillgång till filerna på din spelserver. Vi rekommenderar att använda ett externt FTP-program som **Filezilla** eller **WinSCP** för optimal användning. Om du inte är bekant med hur man använder FTP kan du ta en titt på guiden [Access via FTP](gameserver-ftpaccess.md).
