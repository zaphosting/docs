...
id: csgo-cstv
title: "CS:GO: CSTV "
description: "Upptäck hur du kan streama spel live med fördröjning och automatisk inspelning för åskådare och återupplev matcher när som helst → Lär dig mer nu"
sidebar_label: CSTV
services:
  - gameserver-csgo
---
...
CSTV ger dig möjligheten att streama spel live med en liten fördröjning. Det betyder att folk som inte är med i spelet också kan följa vad som händer på servern. Den här tekniken används även i de välkända Counter-Strike-turneringarna så att åskådarna kan se spektaklet. Dessutom kan sändningarna sparas automatiskt så att du kan kolla på dem igen senare.

<InlineVoucher />

## Aktivering

Aktiveringen av CSTV-tjänsten görs via konfigurationsfilen **server.cfg**. De nödvändiga kommandona måste läggas till där:
...
Kommandona ovan är obligatoriska för att CSTV master-servern ska aktiveras och vara tillgänglig. Men här är fler kommandon för avancerad konfiguration:
...
| tv_chattimelimit "sekunder"  | Begränsar tittare att skriva bara var x:e sekund |
|       tv_debug "0\|1"        |             Visar CSTV debug-info             |
|   tv_delaymapchange "0\|1"   | Fördröjer kartbyte tills sändningen är klar |
|      tv_maxclients "n"       |          Max antal klienter på CSTV-servern          |
|        tv_maxrate "n"        | Max tillåten bandbredd för CSTV-tittare, 0 == obegränsat |
...
|        tv_stoprecord         |            Stoppar inspelning av CSTV-demo             |
|           tv_title           |           Definierar namnet på CSTV-sändningen           |

## Anslutning

Om funktionen är aktiverad på servern kan du ansluta via IP-adressen och den CSTV-port som är tilldelad servern. För att ansluta behöver du konsolen i spelet. Där kan du ansluta med följande kommando:
