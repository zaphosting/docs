...
title: "VPS: Installera Plex på Linux"
description: "Upptäck hur du enkelt hanterar och streamar ditt personliga mediebibliotek med Plex för smidig åtkomst på alla enheter → Läs mer nu"
...
Efter nedladdning kan installationsprogrammet köras för att sätta upp Plex Media Server. När det är installerat hanteras konfigurationen via webbgränssnittet i din webbläsare, där du kan skapa bibliotek och hantera din mediesamling. Kör följande kommando för att starta installationen:

```
sudo dpkg -i plexmediaserver_1.42.1.10060-4e8b05daf_amd64.deb
```

För att konfigurera Plex Media Server, på samma maskin där du installerade servern, öppna ett webbläsarfönster och gå till `http://127.0.0.1:32400/web`. Här kommer du bli ombedd att logga in med ett befintligt Plex-konto eller skapa ett nytt om du inte redan har ett.
...
När du når adressen kommer du omdirigeras till Plex inloggningssida där du kan hantera dina bibliotek och inställningar. För säker extern åtkomst rekommenderas att aktivera Remote Access i Plex-inställningarna, vilket garanterar en krypterad anslutning och dirigerar trafiken via Plex-tjänsten.
...
Grattis! Du har nu framgångsrikt installerat och konfigurerat Plex på din VPS. Vi rekommenderar även att kolla in följande resurser som kan ge dig extra hjälp och vägledning under din serverkonfiguration:
