---
id: hl2-becomeadmin
title: "Half-Life 2: Bli admin"
description: "Upptäck hur du ger administratörsrättigheter för full kontroll över din server och förbättrad in-game hantering → Lär dig mer nu"
...
För att registrera dig som admin krävs installation av SourceMod eller MetaMod. Dessutom behöver du Steam-ID:t för ditt Steam-konto. Det finns flera sätt att få tag på det: du kan antingen använda en extern webbplats som tillhandahåller denna tjänst eller hämta det direkt via spelets konsol.
...
Alternativt kan du hämta ditt SteamID in-game via konsolen. Starta spelet, anslut till din server, öppna konsolen och skriv kommandot:
...
**Spelarnamnet** behöver inte matcha ditt Steam-kontonamn; det är bara för intern referens. Fältet `auth` definierar autentiseringsmetoden, som ska vara `steam`. I `identity` anger du SteamID32. `flags` definierar vilka rättigheter adminen får:
...
Flera flaggor kan kombineras (t.ex. `bc` för kick-rättigheter). För full åtkomst använder du bara `z`. Du kan öppna adminmenyn genom att skriva `admin` i chatten eller `sm_admin` i konsolen. Med kommandot `admin` i chatten eller `sm_admin` i konsolen kan menyn öppnas.
...
| `sm_restartgame <seconds>`        | Startar om spelet efter en nedräkning          |
...
Grattis! Om du följt allt rätt bör du nu ha konfigurerat adminrättigheterna för din server. Har du fler frågor eller behöver hjälp, tveka inte att kontakta vår support som finns tillgänglig varje dag för att hjälpa dig! 🙂
