...
title: "Portal Knights: Vanliga problem"
description: "Upptäck hur du felsöker och löser vanliga Portal Knights-serverproblem för en smidig spelupplevelse → Läs mer nu"
...
Att servern inte syns kan hända om initieringen inte slutfördes korrekt. Det kan till exempel bero på en felaktig konfiguration eller korrupta filer. Mer info hittar du oftast i serverkonsolen eller loggfilerna. Utöver detta bör du se till att inga felaktiga filterinställningar används i serverlistan som gör att servern inte visas.
...
Du kan ändra din konfiguration via inställningarna i **Inställningar** eller direkt i konfigurationsfilerna under **Konfig** i ditt webbgränssnitt.
...
Om du upplever lagg eller högt ping på din server finns några enkla steg för att förbättra prestandan. Se först till att din server uppfyller eller överträffar de rekommenderade specifikationerna för ditt spel och projekt. Att välja en serverplats nära din spelbas kan också minska latensen.
...
Regelbundna backuper sparar dig mycket stress och oro. Skapa regelbundna backuper så att du vid problem alltid har en äldre version sparad när allt fungerade. Vi erbjuder en [backup-lösning](gameserver-backups.md) för våra Portal Knights-spelservrar, där du kan skapa manuella eller schemalagda automatiska backuper.
...
Dessa steg borde hjälpa dig lösa problemet. Om du hittar ditt problem här, pekar den matchande lösningen redan i rätt riktning och hjälper dig få igång allt igen.
