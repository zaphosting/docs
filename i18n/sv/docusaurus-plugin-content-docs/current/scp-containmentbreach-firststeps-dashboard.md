...
title: "SCP: Containment Breach: Översikt Dashboard"
description: "Upptäck hur du effektivt hanterar din SCP: Containment Breach-server och optimerar dess prestanda med en tydlig, strukturerad översikt → Lär dig mer nu"
...
Felmeddelanden i synnerhet kan hjälpa till att identifiera och bättre förstå orsaken till ett problem. Ofta kan svårigheter upptäckas och lösas genom att ta en riktad titt i loggfilerna.
...
**Händelseloggen** ger dig en komplett översikt över alla aktiviteter relaterade till serveradministrationen. Här kan du när som helst följa när en server startades, stoppades eller kraschade, samt när uppdateringar, backuper eller nya installationer utfördes.
...
Din server var tillfälligt offline och du kan inte förklara varför? En titt i händelseloggen och loggfilerna gav inte heller någon information? Det kan vara så att orsaken till problemet inte ligger direkt hos din server, utan hos värdsystemet där din server körs.
...
I det här avsnittet hittar du en översikt över driftstörningshistoriken för värdsystemet där din spelserver-tjänst körs. Du hittar mer information och den senaste versionen av vår servicestatus på vår nya [statussida](https://status.zap-hosting.com/).
...
Config-filredigeraren låter dig redigera konfigurationsfilerna direkt från webbplatsen på det klassiska sättet, utan den förenklade vyn. Detta alternativ gör att du kan anpassa inställningarna för din server och konfigurera dem efter dina personliga preferenser.
...
Vill du schemalägga serveromstarter vid specifika tider? Restartplaneraren låter dig ställa in schemalagda automatiska omstarter av din spelserver. Med denna funktion kan du ställa in omstarter vid specifika tider eller med jämna mellanrum.
...
I avsnittet **Verktyg** i din navigationspanel hittar du en rad underkategorier som hjälper dig att hantera din server. Dessa inkluderar **FTP Browser**, **Databaser** och **Backuper**. Nedan får du en detaljerad introduktion till dessa olika områden.



### FTP-Browser

Med **FTP Browser** får du all nödvändig information för att upprätta en anslutning till FTP-accessen. FTP-accessen ger dig direkt tillgång till filerna på din spelserver. Vi rekommenderar att använda ett externt FTP-program som **Filezilla** eller **WinSCP** för optimal användning. Om du inte är bekant med hur man använder FTP kan det vara hjälpsamt att kika på guiden [Access via FTP](gameserver-ftpaccess.md).
...
**WinSCP:** WinSCP är egentligen tänkt för SCP och SFTP (de krypterade varianterna av FTP), men kan lika gärna hantera vanlig FTP. Programmet är lite mer avancerat än FileZilla och passar bättre för mer erfarna användare.



### Databaser

Utöver våra spelserverprodukter erbjuder vi även 10 inkluderade databaser. Dessa databaser behövs till exempel när data ska sparas i en databas. Under **Databaser** kan du skapa en databas och se åtkomstuppgifterna. Du har även möjlighet att aktivera backup-alternativet och importera befintliga databassäkerhetskopior.
...
Under **Backuper** har du möjlighet att skapa både manuella och automatiska backuper för din server och dess tillhörande databas. Detta gör det enklare att återställa dina data vid eventuella problem eller dataförlust.
