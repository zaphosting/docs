...
title: "Dedikerad Server: Farming Simulator 2022 Dedikerad Server Windows Setup"
description: "Upptäck hur du sätter upp Farming Simulator Dedicated Server 2022 på din Windows VPS eller dedikerade server → Lär dig mer nu"
...
Har du en Windows VPS eller dedikerad server och vill installera Farming Simulator Dedicated Server 2022 på den? Då har du kommit rätt. I den här guiden går vi igenom steg för steg hur du installerar tjänsten på din server.
...
Dessutom krävs en egen GameLicense (inte Steam-version), samt en publik IPv4-adress. GPU eller ljudkort behövs inte för Dedicated Server.  
Beroende på hur mycket dedikerade servern moddas, utökas med mods eller DLCs kan fler resurser behövas. 


## Steg 1 Köp digital licens

En digital version av Farming Simulator 2022 krävs, det kan inte vara Steam-versionen.  
...
Anslut via [Initial Access (RDP)](vserver-windows-userdp.md) till din Windows-server. När du är inloggad, öppna webbläsaren och gå till [länken](https://eshop.giants-software.com/downloads.php) från mailet igen. Ange din GameKey och välj nedladdning för Windows 8/10. 
...
Läs igenom TOS och godkänn. Installationen kan göras som Standard eller Custom, oftast räcker Standard.  
Klicka på installera och Farming Simulator 2022 installeras nu, det kan ta några minuter. 
...
Felmeddelanden om saknad GPU eller liknande kan ignoreras eller stängas med Nej/Avbryt. Farming Simulator behöver bara startas för att kunna ange nyckeln. 
...
Nu måste ett inloggningsnamn och lösenord sättas eller ändras/läsas ut i Config för Dedicated Server, så att inloggning till webgränssnittet för Dedicated Server är möjlig.  
...
För att starta den dedikerade servern måste `"dedicatedServer.exe"` köras i samma mapp där `"dedicatedServer.xml"` ligger.
...
Logga in med de inloggningsuppgifter som sattes/lästes från `"dedicatedServer.xml"`.  
Resten av konfigurationen är självinstruerande i webgränssnittet och kan göras med några klick, efter dina personliga önskemål. 

## Steg 5 Öppna portar i brandväggen

Servern ska såklart vara publikt tillgänglig för att spela med kompisar. Därför måste serverns portar öppnas i Windows-brandväggen. Portarna 10823 och 8080 måste tillåtas för TCP-protokollet. 10823 är spelserverporten och 8080 är webbporten för webgränssnittet, webbporten behöver bara öppnas om den ska användas, administration kan fortfarande göras lokalt på Windows-servern via RDP.  
Så här öppnar du portar i Windows: [Port Forwarding (Firewall)](vserver-windows-port.md). 

När portarna är öppnade är servern publikt tillgänglig, om den är startad. 
