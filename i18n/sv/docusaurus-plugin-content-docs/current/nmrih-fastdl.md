---
id: nmrih-fastdl
title: "No More Room In Hell: Ställ in FastDL"
description: "Upptäck hur du optimerar leveransen av spelinnehåll med FastDL via ZAP-Hostings webspace för snabbare nedladdningar och mindre serverbelastning → Lär dig mer nu"
...
Du behöver en aktiv No More Room In Hell-server hos ZAP-Hosting och ett extra webspace-paket. FTP-åtkomst eller tillgång till Filhanteraren måste finnas både för spelservern och webspace.

På serversidan måste filerna du vill distribuera (t.ex. `.bsp`-kartor, ljudfiler eller modeller) redan vara korrekt organiserade. Samma mappstruktur måste sedan finnas på webspace för att FastDL ska fungera.
...
I root-mappen på ditt webspace skapar du en mapp som heter `fastdl`. Inuti den skapar du undermappar som `maps`, `sound` eller `models` beroende på vilket innehåll du ska leverera. Mappstrukturen måste exakt spegla den som används på din No More Room In Hell-server.
...
FastDL går snabbt och smidigt att sätta upp med ditt egna ZAP webspace. Det ger snabb och effektiv leverans av custom content och förbättrar upplevelsen för alla som spelar på din No More Room In Hell-server.
