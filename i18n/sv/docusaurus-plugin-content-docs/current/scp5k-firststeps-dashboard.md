...
Efter att ha gått igenom den här guiden kommer du enkelt kunna hantera din server effektivt och snabbt hitta alla nödvändiga alternativ och funktioner. Här får du en strukturerad översikt som hjälper dig steg för steg att bekanta dig med serveradministrationen och utöka din kunskap på ett målmedvetet sätt. På så vis kan du säkerställa att din **SCP: 5K-server** är optimalt anpassad efter dina behov och att du får ut maximalt av de tillgängliga möjligheterna.

<YouTube videoId="xUxIWKvKbG8" imageSrc="https://screensaver01.zap-hosting.com/index.php/s/ZXRwR3iocrwjPoA/preview" title="Sätt upp SCP: 5K-server på bara EN MINUT!" description="Känns det lättare att förstå när du ser saker i praktiken? Vi fixar det! Dyka ner i vår video som förklarar allt. Oavsett om du har bråttom eller bara föredrar att ta in info på det mest engagerande sättet!"/>
...
:::warning
Hittar du ingen hjälp i informationen eller förstår du den inte? Ingen fara! Vi finns dagligen tillgängliga i supporten. Skapa bara ett **[Ticket](https://zap-hosting.com/en/customer/support/)** och förklara ditt problem. Vi kollar på det så snabbt vi kan och hjälper dig att lösa det! :)
:::
...
Var din server tillfälligt offline utan förklaring? Kollade du i händelseloggen och loggfilerna utan resultat? Orsaken kan ligga utanför din server, hos värdsystemet där din server körs.

Om vårt övervakningssystem upptäcker en generell störning, meddelas våra tekniker automatiskt som snabbt tar tag i problemet.
...
Under inställningar finns en översikt över de vanligaste konfigurationsalternativen för din server, med korta förklaringar. Den här sidan rekommenderas särskilt för nybörjare inom serveradministration.
...
Config Editor låter dig redigera konfigurationsfiler direkt på webbplatsen i klassiskt läge, utan förenklad vy. Här kan du anpassa serverinställningarna efter dina personliga preferenser.
...
Under **Spel** kan du alltid byta spelvariant på din befintliga server eller installera ett helt annat spel. Spel med samma slotpris kan väljas direkt under **Tillgängliga Spel**. För andra spel krävs först en justering av slotpriset. Mer info finns i guiden [Spelbyte](gameserver-gameswitch.md).
...
**FileZilla:** FileZilla är troligen den enklaste och mest kända FTP-klienten. Installera, starta och skriv in dina inloggningsuppgifter i den övre raden som du hittar här på sidan. Klicka snabbt på "Anslut" så är du direkt inloggad på servern och kan hantera filerna.
...
I inställningarna kan du välja om backuper ska skapas automatiskt och om det ska ske dagligen eller veckovis. Du kan även bestämma om de äldsta backuperna ska tas bort vid behov, till exempel när lagringsutrymmet börjar ta slut.
...
- Annan relevant info som kan påverka din användning av tjänsten
