...
title: "Dedikerad Server: Installera Standard Notes på Linux"
...
Se till att alla beroenden är installerade och att rätt OS-version används för att undvika kompatibilitetsproblem vid installation av Standard Notes.



## Förberedelser

Innan du sätter upp **Standard Notes** behöver du förbereda systemet. Det innebär att uppdatera operativsystemet till senaste versionen och installera alla nödvändiga beroenden. Dessa förberedelser säkerställer en stabil miljö och hjälper till att undvika problem under eller efter installationen.
...
När uppdateringen är klar kan du installera beroenden. Standard Notes körs i en uppsättning Docker-containrar, så Docker måste installeras först. Kör följande kommando:
...
## Installation
Nu när alla krav är uppfyllda och förberedelserna klara kan du fortsätta med installationen av Standard Notes.

Börja med att skapa en mapp som ska innehålla alla konfigurations- och installationsfiler. Denna mapp blir din arbetskatalog för installationen.
...
Denna fil innehåller bara de minimala variabler som krävs för en fungerande setup. Den fullständiga listan över variabler som används finns här: [docker-entrypoint.sh](https://github.com/standardnotes/server/blob/main/docker/docker-entrypoint.sh)
...
Standardinställningarna i den medföljande `docker-compose.yml`-filen kan anpassas efter dina behov. En viktig del är `server`-tjänsten där du kan justera portmappningarna i `ports`-egenskapen. Det låter dig välja vilka portar på hosten appen ska vara tillgänglig på, vilket hjälper dig undvika konflikter med andra tjänster eller anpassa till ditt nätverk.

Att säkra databasen är också viktigt. Standardlösenord bör bytas ut mot starka, slumpmässigt genererade strängar. Skapa ett säkert lösenord med:

```
openssl rand -hex 12
```

Lägg till det genererade värdet i `.env` som `DB_PASSWORD`. Samma värde måste även sättas för `MYSQL_ROOT_PASSWORD` och `MYSQL_PASSWORD` i `docker-compose.yml` för att hålla containrarna synkade.
...
För att skapa ditt konto i Standard Notes, klicka på avatar-ikonen nere till höger i appen. I menyn som dyker upp väljer du “Create Account” för att börja skapa en ny användarprofil. Ange en giltig e-postadress och ett säkert lösenord.
...
Grattis! Du har nu framgångsrikt installerat och konfigurerat Standard Notes på din dedikerade server. Vi rekommenderar också att du kollar in följande resurser som kan ge dig extra hjälp och vägledning under din serverkonfiguration:
