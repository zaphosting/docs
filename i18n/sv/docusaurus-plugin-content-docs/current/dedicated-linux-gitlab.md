...
title: "Dedikerad Server: Installera GitLab på Linux"
description: "Upptäck hur du sätter upp GitLab på Linux för att effektivisera DevOps-flöden och förbättra teamets samarbete → Läs mer nu"
...
Du behöver ansluta via en SSH-klient för att installera GitLab på din Linux-server. Kolla in vår guide för [Initial åtkomst (SSH)](vserver-linux-ssh.md) för mer info.
...
När processen är klar ska GitLab vara redo på din Linux-server. Gå vidare till nästa steg där du konfigurerar viktiga inställningar för att få servern att fungera.
...
##!
##! Obs: Under installation/uppgraderingar används värdet från miljövariabeln
##! EXTERNAL_URL för att fylla i/ersätta detta värde.
##! På AWS EC2-instans försöker vi även hämta publik hostname/IP
##! från AWS. Mer info:
...
# letsencrypt['contact_emails'] = [`Din e-post här`] # Detta ska vara en array med e-postadresser som kontakt
...
Kör sedan följande kommando för att konfigurera om GitLab med de nya inställningarna.
...
# 1. Om det angavs manuellt (antingen via `GITLAB_ROOT_PASSWORD` miljövariabel eller via `gitlab_rails['initial_root_password']` i `gitlab.rb`) innan databasen skapades första gången.

# 2. Lösenordet har inte ändrats manuellt, varken via UI eller kommandorad.

#

# Om lösenordet här inte fungerar måste du återställa admin-lösenordet enligt https://docs.gitlab.com/ee/security/reset_user_password.html#reset-your-root-password.

Password: `[DITT_LÖSENORD_HÄR]`

# OBS: Denna fil tas automatiskt bort vid första reconfigure-körningen efter 24 timmar.
```
...
Hoppa över detta om du inte vill konfigurera brandvägg eller redan använder t.ex. `firewalld` på OpenSUSE.
Se till att tillåta port 80/443 och port 22.
...
:::warning
Detta kommer som standard blockera all trafik förutom de tillåtna portarna. Se till att vitlistan är korrekt innan du kör detta kommando.
:::
