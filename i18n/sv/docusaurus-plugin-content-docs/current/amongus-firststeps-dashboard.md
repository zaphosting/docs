...
title: "Impostor Among Us: Översikt Dashboard"
description: "Upptäck hur du effektivt hanterar och optimerar din Impostor - Among Us Server för bästa spelupplevelse → Lär dig mer nu"
...
Har du ett tekniskt problem och är osäker på orsaken? En första möjlig lösning är att kolla **Loggfilerna**. Dessa filer innehåller mycket info om servern, som aktuella händelser, ledtrådar eller felmeddelanden.
...
**Live Console** ger dig en realtidsvy över alla aktuella händelser på din server. Här ser du info om spelarnas aktiviteter samt eventuella fel- och systemmeddelanden. Denna översikt låter dig få en helhetsbild av vad som händer på servern och upptäcka problem tidigt.

![](https://screensaver01.zap-hosting.com/index.php/s/J9SsmSrPXnG7P9w/preview)

Utöver att övervaka händelser kan du även skicka kommandon direkt till servern i **Live Console**. Det är användbart för att justera inställningar, utföra åtgärder eller snabbt reagera på problem som uppstår.
...
Här hittar du en översikt över driftstörningar på värdsystemet där din spelserver körs. Mer info och senaste statusen hittar du på vår nya [statussida](https://status.zap-hosting.com/).

## Inställningar

I avsnittet **Inställningar** hittar du alla nödvändiga sidor för att konfigurera din server. De tillgängliga alternativen för serverkonfiguration listas detaljerat nedan.
...
Config-filredigeraren låter dig redigera konfigurationsfiler direkt på webbplatsen i klassisk vy, utan förenklad vy. Detta ger dig möjlighet att anpassa serverns inställningar efter dina personliga preferenser.
...
Med ZAP DNS-inställningar kan du skapa lättare adresser för din tjänst. Du kan använda en subdomän av våra domäner eller skapa en DNS-post på dina egna domäner för en adress som bättre kopplas till dig personligen.
...
**FileZilla:** FileZilla är troligen den enklaste och mest kända FTP-klienten. Installera, starta och skriv in dina inloggningsuppgifter i den övre raden som du hittar lite högre upp på sidan. Med ett snabbt klick på "Anslut" är du direkt inloggad på servern och kan hantera filerna.

**WinSCP:** WinSCP är egentligen för SCP och SFTP (de krypterade varianterna av FTP) men fungerar lika bra med vanlig FTP. Programmet är lite mer avancerat än FileZilla och passar bättre för erfarna användare.

### Databaser

Utöver våra spelserverprodukter erbjuder vi även 10 inkluderade databaser. Dessa behövs till exempel när data ska sparas i en databas. Under **Databaser** kan du skapa en databas och se inloggningsuppgifter. Du kan också aktivera backupfunktionen och importera befintliga databasbackuper.
...
I inställningarna kan du ange om backuper ska skapas automatiskt och om det ska ske dagligen eller veckovis. Du kan även välja om de äldsta backuperna ska raderas vid behov, till exempel när lagringsutrymmet börjar ta slut.
