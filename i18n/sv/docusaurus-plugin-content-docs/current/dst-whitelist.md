...
För att göra servern effektivt whitelistad, sätt `whitelist_slots` lika med värdet på `max_players`.
...
## Verifiera Whitelist-funktionalitet

Efter omstart, försök gå med med ett konto som inte finns i `whitelist.txt`. Om `whitelist_slots` är lika med `max_players` ska anslutningen misslyckas. Gå sedan med med ett listat KU_-ID för att bekräfta åtkomst.

Om servern fortfarande tillåter olistade spelare, kontrollera att du redigerade den aktiva cluster-mappen och att `[NETWORK]`-sektionen innehåller raden `whitelist_slots`.
