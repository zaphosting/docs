...
لتحقيق أفضل أداء واستقرار ووظائف، يُنصح بالإعداد التالي لاستضافة سيرفر Nextcloud. سيرفر Nextcloud غير متوافق مع Windows وMacOS، على الأقل ليس بدون استخدام تقنيات افتراضية إضافية أو حلول مشابهة.<br/>
...
تظهر إعدادات سكربت التثبيت، حيث يتم إنشاء مستخدم root وتعريف معلومات قاعدة البيانات:
...
ننصح بشدة بوضع مجلد البيانات خارج مجلد الجذر الخاص بالويب (أي خارج /var/www). أسهل طريقة للقيام بذلك هي أثناء التثبيت الجديد. يمكن تحديد المجلد أثناء الإعداد. لكن يجب إنشاء المجلد أولًا وضبط الصلاحيات المناسبة. يمكن تخزين البيانات مثلاً في مجلد باسم Cloud داخل مجلد المنزل.
...
يجب أن يكون الوصول إلى السحابة عبر اتصال SSL فقط. بدون تشفير SSL، تنتقل البيانات والمعلومات كنص عادي. يمكن اعتراض هذه المعلومات وقراءتها بسهولة وسرعة بدون تشفير.
...
علاوة على ذلك، يجب إعادة توجيه كل حركة HTTP إلى HTTPS باستخدام إعادة توجيه دائمة مع رمز الحالة 301. يمكن تحقيق ذلك باستخدام Apache مع تكوين مثل التالي:
...
يمكن الوصول إلى Nextcloud عبر المتصفح، وكذلك عبر الهاتف الذكي والكمبيوتر باستخدام التطبيق. يمكن العثور على مصادر التحميل هنا: https://nextcloud.com/install/#install-clients
