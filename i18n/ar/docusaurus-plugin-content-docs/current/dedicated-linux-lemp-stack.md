...
سنستخدم أيضًا نطاق اختبار `zapdocs.example.com` طوال المثال، لأنه في سيناريو حقيقي ستستخدم نطاقًا. يجب عليك **إعداد سجل DNS من نوع A** للنطاق يشير إلى عنوان IP الخاص بسيرفرك. إذا كنت تحتاج مساعدة، يرجى تصفح دليلنا [سجلات النطاق](domain-records.md#a-records).
...
مبروك، لقد قمت بتثبيت وإعداد حزمة LEMP بنجاح! كخطوة تالية، نوصي **بشدة** بإعداد نطاق وشهادة SSL لضمان نقل البيانات بأمان إلى مواقعك. يرجى الاطلاع على دليلنا [Certbot](vserver-linux-certbot.md#webroot-plugin) مع التركيز على **إضافة Nginx** واتباع الإعداد التفاعلي لتثبيت شهادة بسرعة وسهولة لنطاقك المختار.
