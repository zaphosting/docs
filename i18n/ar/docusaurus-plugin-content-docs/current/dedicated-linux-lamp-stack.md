...
سنستخدم أيضًا نطاق اختبار `zapdocs.example.com` طوال الدليل، لأنه في سيناريو حقيقي من المحتمل أن تستخدم نطاقًا. **يجب** أن تقوم بإعداد سجل DNS من نوع `A` للنطاق يشير إلى عنوان IP الخاص بسيرفرك. إذا كنت تحتاج مساعدة في ذلك، يرجى تصفح دليلنا [سجلات النطاق](domain-records.md#a-records).
