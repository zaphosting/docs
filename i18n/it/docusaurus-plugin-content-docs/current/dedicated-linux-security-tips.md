...
title: "Server Dedicato: Consigli per la Sicurezza Linux"
...
Questa guida contiene alcuni consigli e suggerimenti su come rendere il tuo server Linux più sicuro. Soprattutto perché i server devono essere accessibili dall’esterno, una protezione base contro accessi indesiderati è assolutamente consigliata e non va trascurata.
...
Di default, per SSH si usa il login tramite password. Tuttavia, questo ha lo svantaggio che l’autenticazione può essere aggirata abbastanza facilmente con un attacco brute force, soprattutto se usi una password troppo semplice per l’accesso SSH. Quindi, se decidi di usare la soluzione password, usa una password **sicura**.
...
Servizi come SSH o FTP usano sempre le stesse porte di default (alcune sono elencate nella tabella sopra). Se un attaccante esterno vuole fare brute force sul servizio SSH del tuo server, prima deve sapere quale porta usa SSH. Se non cambi queste porte, di solito le porte 22 e 21 sono il bersaglio per eseguire comandi direttamente sul server o accedere ai file via FTP.
...
Aggiungi la tua porta desiderata dopo `Port`. Se `Port` è commentato (es. `#Port 22`), togli il "#" e sostituisci il 22 con la porta che vuoi. Ora salva il file (in Nano con Ctrl + o) e chiudilo (Nano: Ctrl + x).

![Port sshd](https://screensaver01.zap-hosting.com/index.php/s/F4Z4SDjLW4y2iDp/preview)

Il servizio SSH deve essere riavviato per applicare le modifiche.
```
...
Apri il file con nano, togli il "#" prima di `port` e inserisci la porta che vuoi. Ricorda le indicazioni sopra per non inserire una porta non valida.
...
Il principio dell’accessibilità esterna di un server è sempre lo stesso: una porta deve essere aperta per permettere l’accesso dall’esterno. Nel caso di SSH, questa è **di default** la porta 22/TCP. (vedi sopra come cambiare la porta di default)
...
- **IPTables**: il firewall Linux originale. Offre tante opzioni ma è un po’ più complicato da usare.
- **UFW**: un’interfaccia più semplice per IPTables, senza dover usare comandi complessi, ma un po’ più limitata.

Alla fine scegli tu quale usare. A seconda dell’uso potresti aver bisogno della versatilità di IPTables, ma a volte UFW basta. (es. se vuoi solo aprire/chiudere porte, è comunque meglio di niente!)
...
Nota che questa regola si attiva solo per **porta 22** (il valore dopo `--dport`), per altri servizi i comandi vanno adattati.
...
2. La seconda aggiunge l’IP di una nuova connessione alla lista `recent`.
3. La terza scarta nuove connessioni da IP che hanno tentato più di 2 connessioni nell’ultimo secondo.
...
Accedi al server Linux. Se ti serve aiuto, segui la guida [Accesso iniziale (SSH)](vserver-linux-ssh.md). Le istruzioni sono testate su Debian e Ubuntu, ma dovrebbero funzionare anche su altre distro.
...
Ora esegui questo comando per limitare le connessioni a 6 al minuto.
```
ufw limit 22/tcp
```
...
:::tip
Il firewall (sia IPTables che UFW) può solo contare “meccanicamente” i tentativi di connessione e bloccarli. Con Fail2Ban puoi invece controllare i log per anomalie. La prossima sezione spiega come installare e attivare Fail2Ban.
:::

## Misure aggiuntive con Fail2Ban

Fail2Ban è un servizio che blocca gli IP che tentano di connettersi al server con intenzioni probabilmente malevole. Fail2Ban monitora alcuni file di log per anomalie e protegge il sistema in modo efficace e semplice.
...
- postfix
- Courier Mail Server
Altri servizi si possono aggiungere usando espressioni regolari (RegEx) e specificando il file di log.
...
```
Fail2Ban usa questo file di log e monitora i tentativi falliti. Poiché il log contiene l’IP dell’attaccante, Fail2Ban può bloccare quell’IP dopo pochi tentativi falliti.

### Installazione di Fail2Ban

Accedi al server Linux. Se ti serve aiuto, segui la guida [Accesso iniziale (SSH)](vserver-linux-ssh.md). Esegui i comandi come root o con *sudo*.
...
Dopo l’installazione, controlla lo stato con `systemctl`: (esci da systemctl con Ctrl+C)
```
...
Scorri un po’ fino a questa parte e modificala così, per esempio:

| Attributo    | Spiegazione                                         | Valore      |
|--------------|----------------------------------------------------|-------------|
| ignoreip     | IP che non devono essere bloccati                   | 127.0.0.1/8 |
| ignorecommand| Non rilevante                                       |             |
| bantime      | Quanto dura il blocco di un IP                      | 1h          |
| findtime     | Intervallo di tempo per considerare i tentativi    | 10m         |
...
# Un host è bannato se ha generato "maxretry" durante gli ultimi "findtime"
# secondi.
findtime  = 10m
...
Hai finito di configurare le impostazioni di base. Per monitorare il servizio SSH, scorri un po’ più giù fino al tag `[sshd]`. Se hai cambiato porta, inseriscila sotto `port`.
...
```
:::tip
Come vedi, puoi fare impostazioni individuali per ogni servizio (qui `maxretry` è più basso del valore di default). Anche se hai fatto impostazioni generali, puoi configurare quasi tutte le opzioni per ogni servizio. Se non lo fai, si usa la configurazione generale.
:::

Ora devi solo riavviare Fail2Ban per iniziare il monitoraggio.
```
...
:::danger
Non fare questo sulla tua rete normale, perché potresti bloccare il tuo IP e **verresti bloccato fuori**.
:::

Prova a connetterti via SSH al server (con IP diverso!) e inserisci la password sbagliata ogni volta. Il risultato dovrebbe essere simile a questo:
```
...
:::info
Se hai un numero insolitamente alto di ban IP, conviene aumentare il tempo di ban ad ogni tentativo fallito per ridurre i tentativi possibili.
:::
...
## Proteggere webserver con Cloudflare

Molti usano Cloudflare come DNS manager del dominio. Cloudflare non solo ha una delle reti più grandi al mondo, che offre tempi di caricamento più bassi, latenza ridotta e migliore esperienza, ma protegge anche i siti da attacchi DoS/DDoS, flooding e nuovi tipi di attacchi che emergono ogni giorno.
In questa guida vedrai come proteggere il tuo webserver dagli attacchi.

Partiamo dal presupposto che il tuo dominio sia già gestito da Cloudflare, altrimenti puoi seguire [la loro guida](https://developers.cloudflare.com/fundamentals/get-started/setup/add-site/) per trasferirlo. Vai alla scheda DNS Records nel pannello Cloudflare e assicurati che il record verso il tuo webserver abbia la nuvola arancione e dica "Proxied".

![](https://screensaver01.zap-hosting.com/index.php/s/wptKPBTLwmKxx8F/preview)

Ora tutto il traffico verso il tuo dominio passerà da Cloudflare e poi al server, quindi sarà traffico legittimo.
...
## Conclusione - il tuo server è ora molto più sicuro!

Questa guida ti ha mostrato funzioni base e avanzate per mettere in sicurezza il tuo server Linux. Se hai applicato tutti i consigli che valgono per il tuo sistema, il tuo server è già molto più sicuro di prima - complimenti!
