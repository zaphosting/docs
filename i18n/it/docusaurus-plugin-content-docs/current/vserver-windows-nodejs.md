...
title: "VPS: Configura Node.js su Windows"
...
Per usare Node.js in un ambiente isolato, devi prima scaricare l’immagine ufficiale basata su Alpine. Esegui questo comando per prendere l’ultima immagine Node.js 22, pronta per il deployment in container:

```
docker pull node:22-alpine
```

Poi puoi avviare un container basato su questa immagine e aprire una shell interattiva dentro di esso. Usa questo comando per partire con una shell interattiva. L’opzione `--rm` fa sì che il container venga rimosso automaticamente quando esci, così il tuo host resta pulito.  
...
Questo attiva la modalità produzione, con ottimizzazioni di performance e logging adeguato. Per lo sviluppo, puoi invece impostare NODE_ENV=development per avere messaggi di errore dettagliati e funzionalità di debug.  
