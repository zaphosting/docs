...
title: "VPS: Installazione di Docker"
description: "Scopri come installare Docker sul tuo server Linux per eseguire applicazioni isolate in modo efficiente e risparmiare risorse di sistema → Scopri di più ora"
...
Per iniziare, devi collegarti al tuo server Linux tramite SSH. Dai un’occhiata alla nostra guida [Accesso iniziale (SSH)](vserver-linux-ssh.md) se ti serve una mano con questo passaggio. In questa guida useremo Ubuntu come distribuzione Linux.

## Installazione

Ora che sei connesso al tuo server Linux, puoi procedere con i metodi di installazione. Scegli una delle repository Linux qui sotto per vedere i passaggi specifici.
...
Ora che hai aggiunto il repository Docker alle sorgenti, esegui l’aggiornamento con `apt-get update`.

```
sudo apt-get update
```

A questo punto hai configurato correttamente il repository Docker per `apt`. Come ultimo passo, installa i pacchetti Docker. Puoi installare l’ultima versione con questo comando.
...
Ora che Docker è installato, puoi fare qualche setup extra per evitare di dover usare `sudo` ogni volta che esegui comandi Docker e per far partire Docker automaticamente all’avvio del server.

### Gestire Docker senza Sudo

Puoi evitare di dover anteporre `sudo` a tutti i comandi Docker creando un gruppo Docker e aggiungendo i tuoi utenti a questo gruppo. È più comodo, ma attenzione: questo dà indirettamente privilegi di root all’utente.
...
Ora verifica che puoi eseguire comandi Docker senza `sudo` provando di nuovo con `docker run hello-world`.

:::tip
Se prima hai eseguito il comando con `sudo`, potresti ricevere un errore riguardo un file di configurazione. Per risolvere, elimina la cartella Docker con `rmdir ~/.docker/`, che verrà ricreata automaticamente al prossimo comando.
:::

Se il comando funziona senza problemi, hai configurato Docker per funzionare senza `sudo`.

### Avviare Docker all’avvio del server
...
:::tip
Su Ubuntu e Debian Docker è già configurato per partire automaticamente all’avvio, quindi non devi fare nulla.
:::

Per abilitare Docker all’avvio, esegui questi comandi.
