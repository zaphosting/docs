...
title: "Server Dedicato: Configura Plausible su OS"
description: "Scopri come configurare una piattaforma di web analytics incentrata sulla privacy che offre insight chiave con facilità e velocità → Scopri di più ora"
...
Una guida completa all’installazione e all’uso di Docker è disponibile nella nostra guida [Docker](vserver-linux-docker.md).
...
Poi crea un file ambiente (.env) e definisci le variabili essenziali. Il BASE_URL deve puntare al dominio reale che ha già un record DNS che risolve verso il tuo server. Devi anche generare una SECRET_KEY_BASE casuale di almeno 64 byte:
```
touch .env
...
Queste porte di default permettono a Let’s Encrypt di emettere automaticamente i certificati TLS. Se vuoi eseguire Plausible dietro un reverse proxy, puoi modificare questi valori. Inoltre, crea un file `compose.override.yml` per esporre le porte corrette per il container Plausible:
...
Infine, avvia i servizi richiesti con Docker Compose:
...
Plausible può essere integrato con Google Search Console per arricchire le tue analytics con dati sulle query di ricerca. Così puoi vedere quali keyword portano traffico al tuo sito, ottenendo insight più profondi sul comportamento dei visitatori.
...
Se non hai ancora un client OAuth o non conosci la procedura, la Wiki ufficiale di Plausible offre una guida dettagliata passo passo. Spiega come creare un progetto su Google Cloud, abilitare le API necessarie, configurare un client OAuth e aggiungere le credenziali generate a Plausible: [Plausible Wiki: Google Integration](https://github.com/Plausible/community-edition/wiki/google-integration).
...
Complimenti! Hai installato e configurato Plausible con successo sul tuo Server Dedicato. Ti consigliamo anche di dare un’occhiata a queste risorse, che possono darti supporto e approfondimenti durante la configurazione del server:
