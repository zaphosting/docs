...
title: "DayZ: Panoramica del Dashboard"
description: "Scopri come gestire e ottimizzare al meglio il tuo server DayZ con una guida chiara e passo dopo passo → Scopri di più ora"
...
Grazie a queste informazioni, hai un’ottima visione sull’utilizzo del server. Inoltre, queste statistiche sono spesso utili per individuare possibili problemi di performance e restringere le cause. Per esempio, puoi risalire a quando l’utilizzo è aumentato e se è legato a eventi o cambiamenti specifici. Questo è particolarmente importante quando sorgono problemi improvvisi, perché ti permette di agire in modo mirato per risolverli e analizzare sistematicamente le cause.

### DDoS Manager

Visualizza gli attacchi DDoS passati per valutare meglio minacce e schemi ricorrenti. Attiva le notifiche per attacchi DDoS in corso sul tuo server, così puoi reagire velocemente in caso di incidente.
...
Hai un problema tecnico e non sai da dove venga? Una prima strategia è controllare i **File di Log**. Questi file contengono tante informazioni sul server, come eventi attuali, suggerimenti o messaggi di errore.

I messaggi di errore in particolare aiutano a identificare e capire meglio la causa del problema. Spesso, dando un’occhiata mirata ai file di log, si riescono a scovare e risolvere le difficoltà.
...
Il tuo server è stato offline temporaneamente e non sai perché? Né il registro eventi né i file di log ti hanno dato indizi? Potrebbe essere che il problema non sia direttamente sul tuo server, ma sul sistema host che lo ospita.
...
- Impostazioni base: **Nome server**, password del game server, mod, mod server, password admin, missione, password RCON, attivare/disattivare RCON
...
Crea attività pianificate che si eseguono automaticamente a orari definiti per automatizzare vari processi sul server. Puoi impostare se un’attività deve partire una volta o ripetersi. Tipi di attività supportate: avviare, fermare o riavviare servizi, riavviare il server se online, creare backup o eseguire comandi personalizzati.
...
Oltre ai nostri game server, offriamo anche 10 database inclusi. Questi database servono per esempio quando devi salvare dati in un database. In **Database** puoi crearne uno e vedere le credenziali di accesso. Hai anche l’opzione di attivare i backup e importare backup di database esistenti.
...
Per i nostri game server offriamo di default 10 GB di spazio per i backup sul server di storage. Se ti serve più spazio, puoi fare un upgrade a pagamento. Così hai sempre spazio a sufficienza per i backup e puoi accedere anche a quelli più vecchi se serve.
...
Offriamo una documentazione completa per i nostri prodotti, che funge da base di conoscenza per le domande e dubbi più frequenti. Troverai guide e aiuti su vari temi per supportarti nell’uso e gestione del prodotto.
