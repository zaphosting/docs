---
id: insurgency-gslt
title: "Insurgency: Crea un GSLT (Steam Game Server Login Token)"
description: "Scopri come proteggere e autenticare il tuo server di gioco dedicato con un Steam Game Server Login Token per una migliore visibilità e sicurezza → Scopri di più ora"
...
Una volta loggato, puoi creare un nuovo token usando il modulo sulla pagina. Nel campo **App ID**, inserisci l’App ID `17700` che corrisponde a **Insurgency**. Assicurati di inserire l’ID corretto, altrimenti il token non funzionerà.
...
:::info Gestire più server Insurgency
Se gestisci più server **Insurgency**, ti serve un GSLT per ciascun server. <u>**Non**</u> è possibile far girare più server con un solo GSLT.
:::
...
Il GSLT è stato creato e applicato alla configurazione del tuo server. Per altre domande o supporto, non esitare a contattare il nostro team di assistenza, disponibile ogni giorno per aiutarti! 🙂
