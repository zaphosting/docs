...
## Introduzione
Hai un VPS Windows e vuoi ospitare il tuo server Hytale? In questa guida ti spieghiamo passo passo come installare e far girare il server dedicato Hytale su un sistema Windows.
...
Di default, il server ascolta sulla porta UDP 5520 e si lega a tutte le interfacce disponibili. Puoi cambiare indirizzo e porta se vuoi. Il server comunica via UDP usando QUIC. Assicurati che il firewall permetta traffico UDP in ingresso sulla porta scelta, usando Iptables o UFW. Per applicare facilmente questa regola firewall su Powershell, esegui:
