...
title: "Contagion: Problemi comuni"
description: "Scopri come risolvere i problemi comuni del server Contagion per mantenere il gioco fluido e senza interruzioni → Scopri di più ora"
...
Le cause dei problemi del server possono variare e spesso non sono subito evidenti. Impara a fare troubleshooting passo dopo passo per i problemi tipici e mantieni tutto fluido e stabile.
...
Il server potrebbe non essere visibile se l’inizializzazione non è stata completata con successo. Questo può succedere per una configurazione errata o file corrotti. Ulteriori dettagli si trovano solitamente nella console del server o nei log. Inoltre, assicurati che non ci siano filtri sbagliati nella lista server che impediscono la visualizzazione.
...
I crash spesso sono causati da estensioni difettose o non aggiornate. Che sia un framework, plugin, mod o risorsa, possono sorgere problemi se non sono compatibili con l’ultima versione del gioco o contengono bug.

Questo può portare a crash, freeze o errori, specialmente se più estensioni problematiche interagiscono. Se sospetti un’estensione, prova a disabilitarla temporaneamente e verifica se il server resta stabile. È un modo semplice per capire quale crea problemi.
...
Impostazioni errate o mal regolate possono far consumare troppe risorse e causare lag o stuttering. Assicurati che i valori di configurazione corrispondano alle impostazioni consigliate per il tuo gioco e la dimensione del server. Rivedili e aggiustali per mantenere il server il più efficiente possibile.
...
A seconda della portata del progetto e del contenuto aggiuntivo, le risorse richieste potrebbero essere già più alte o aumentare col tempo. In questi casi, fare un upgrade del pacchetto server è la soluzione più semplice per mantenere performance e stabilità.
...
Questo può portare a CPU alta, memory leak, lag o crash, specialmente se più componenti non ottimizzati interagiscono. Assicurati sempre che le estensioni siano mantenute attivamente, ben documentate e testate per le performance. In caso di dubbi, consulta feedback della community o monitora le prestazioni del server per individuare elementi problematici.

Per isolare la causa dei problemi di performance, è utile disabilitare temporaneamente contenuti aggiuntivi. Parti da una configurazione minima e verifica se il problema persiste. Se sparisce, reintroduci estensioni, mod o risorse una alla volta testando dopo ogni passo. Questo metodo aiuta a identificare l’elemento specifico che crea problemi, sia esso un conflitto, memory leak o uso eccessivo di risorse.
...
Se sospetti problemi di routing o rete esterna, contatta il supporto. Ti aiuteranno ad analizzare e trovare la soluzione migliore.
...
Backup regolari evitano un sacco di stress e preoccupazioni. Crea backup frequenti per avere sempre una versione precedente funzionante in caso di problemi. Offriamo una [soluzione di backup](  - gameserver-corekeeper-backups.md) per i server Contagion, che puoi usare per backup manuali o automatici programmati.
