...
title: "ECO Global Survival: Panoramica del Dashboard"
description: "Scopri come gestire al meglio il tuo server Eco - Global Survival e ottimizzare le impostazioni per la miglior esperienza di gioco → Scopri di più ora"
...
La **Console Live** ti mostra in tempo reale tutti gli eventi attuali sul tuo server. Qui puoi vedere info sulle attività dei giocatori, messaggi di errore o di sistema. Questa panoramica ti permette di capire cosa succede sul server e di individuare problemi in anticipo.
...
Il tuo server è stato offline per un po’ e non sai perché? Il registro eventi e i file di log non hanno dato info? Potrebbe essere che il problema non sia direttamente sul tuo server, ma sul sistema host che lo ospita.
...
Qui trovi una panoramica della storia delle interruzioni del sistema host su cui gira il tuo server. Per info aggiornate e lo stato attuale, visita la nostra nuova [pagina di status](https://status.zap-hosting.com/).
...
Oltre ai nostri server di gioco, offriamo anche 10 database inclusi. Questi sono necessari, per esempio, quando devi salvare dati in un database. In **Database** puoi crearne uno e vedere le credenziali di accesso. Puoi anche attivare i backup e importare backup esistenti.
