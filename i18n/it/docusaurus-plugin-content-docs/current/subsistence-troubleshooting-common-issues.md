...
title: "Subsistence: Problemi comuni"
description: "Scopri come risolvere i problemi comuni del server Subsistence per un’esperienza di gioco fluida → Scopri di più ora"
...
Una buona performance del server è essenziale per un’esperienza di gioco top. Problemi come lag, ritardi o crash improvvisi succedono spesso quando la configurazione non è ottimale, l’hardware non regge il gioco o le estensioni sovraccaricano il sistema.
...
A seconda della portata del progetto e dei contenuti extra, le risorse richieste potrebbero essere già più alte dall’inizio o aumentare col tempo. In questi casi, fare un upgrade del pacchetto server è il modo più semplice per mantenere performance e stabilità.
...
Questo può portare a CPU alta, memory leak, lag o crash, soprattutto se più componenti non ottimizzati interagiscono. Assicurati sempre che le estensioni siano mantenute attivamente, ben documentate e testate per le performance. In caso di dubbi, consulta feedback della community o monitora le performance per individuare elementi problematici.

Per isolare la causa dei problemi di performance, disabilita temporaneamente contenuti extra. Parti da una configurazione minima e verifica se il problema persiste. Se sparisce, reintroduci gradualmente estensioni, mod o risorse una alla volta testando dopo ogni passo. Questo metodo aiuta a identificare l’elemento specifico che crea problemi, che sia un conflitto, memory leak o uso eccessivo di risorse.
...
Se pensi che il problema sia routing o rete esterna, contatta il nostro supporto. Ti aiuteranno a capire la situazione e trovare la soluzione migliore.
...
Anche se la maggior parte di queste minacce è fuori dal controllo dell’utente medio, ZAP-Hosting offre protezioni integrate per difendere il tuo server da attacchi comuni e avanzati. Se sospetti che il tuo server sia sotto attacco e questo crea problemi, contatta il supporto per assistenza e consigli.
...
Backup regolari evitano un sacco di stress e preoccupazioni. Crea backup regolari per essere sicuro che in caso di problemi hai sempre una versione precedente funzionante. Offriamo una [soluzione di backup](gameserver-backups.md) per i server Subsistence, che puoi usare per fare backup manuali o automatici programmati.
...
Questi passaggi dovrebbero aiutarti a risolvere il problema. Se trovi il tuo problema qui, la soluzione indicata ti darà già una direzione chiara per far ripartire tutto.

Se invece serve una mano in più, non esitare a contattare il nostro supporto, disponibile ogni giorno per aiutarti! 🙂
