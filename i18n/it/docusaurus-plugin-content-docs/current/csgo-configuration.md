...
title: "CS:GO: Configurazione server"
...
Nelle impostazioni puoi scegliere la gamemode che vuoi usare. I comandi di questa config verranno caricati di conseguenza. Questi possono essere gestiti sia dall’amministrazione del server di gioco sia manualmente via FTP sotto **gXXXXXX/CS:GO/game/csgo/cfg/**.
...
I Gameserver Login Tokens (GSLT) sono una procedura implementata da Steam come misura di protezione. Per far girare server di alcuni giochi Steam (principalmente giochi Source) è necessario generare un token. Maggiori informazioni qui:
...
La startmap serve a definire quale mappa deve essere caricata all’avvio del server. È importante che il nome della mappa sia scritto esattamente e correttamente. L’estensione del file non è necessaria.
...
L’hostname definisce il nome del server. Questo nome verrà mostrato nella lista server e nello scoreboard.
...
Come detto all’inizio, puoi definire la gamemode/tipo di gioco che vuoi usare nell’interfaccia alla pagina delle impostazioni. Dopo devi regolare le tue impostazioni e modificare il file di configurazione corrispondente. Nei file di configurazione delle gamemode puoi fare modifiche dettagliate al gameplay, al comportamento dei bot ecc.
...
Questo comando definisce quanti soldi riceve un giocatore piantando la bomba.
...
Questo comando definisce quanti soldi si guadagnano o perdono salvando un ostaggio in modalità Hostage.
...
Questo comando definisce quanti soldi guadagna ogni terrorista vincendo un round su mappa ostaggio eliminando tutti gli avversari.



**cash_team_elimination_hostage_map_ct**
Questo comando definisce quanti soldi guadagna ogni membro del team CT eliminando tutti e 5 i terroristi su mappa ostaggio.
...
Questo comando definisce di quanto devono aumentare i soldi per una serie di sconfitte consecutive. Se questo valore è $500 (impostazione di default), significa che ogni round perso consecutivamente darà al team perdente $500 in più rispetto al precedente. Questo vale fino a 5 round persi consecutivi.
...
Questo comando definisce quanti soldi guadagna il team che protegge gli ostaggi vincendo il round impedendo il salvataggio entro il tempo limite.



**cash_team_win_by_time_running_out_bomb**
Questo comando definisce quanti soldi guadagnano i giocatori CT vincendo un round perché il tempo è scaduto. Significa che i terroristi non hanno piazzato la bomba o eliminato tutti i CT entro il tempo.
...
**mp_buy_anywhere**
Questo comando definisce se i giocatori possono accedere al menu di acquisto fuori dalla zona di acquisto. Ricorda che il tempo per comprare resta valido.
...
Questo comando definisce quale granata (se presente) viene lasciata cadere alla morte di un giocatore:
...
**mp_death_drop_gun**
Questo comando definisce quale arma viene lasciata cadere alla morte di un giocatore. Il valore di default è 1 (la migliore). Scegli tra:

- Valore: 0 - Non lasciare armi alla morte
- Valore: 1 - Lascia la migliore arma alla morte
- Valore: 2 - Se il giocatore ha un’arma in mano alla morte, lascia quella, altrimenti lascia la più preziosa



**mp_defuser_allocation**
Questo comando definisce se i giocatori ricevono un kit di disinnesco gratis all’inizio del round e chi lo riceve:
...
Questo comando definisce quanti secondi un giocatore ha per scegliere squadra. Dopo questo tempo la squadra viene scelta automaticamente. Il valore di default è 15 secondi.



**mp_forcecamera**

Questo comando definisce cosa possono vedere gli spettatori (chi possono guardare quando muoiono). Ecco le opzioni:

- Valore: 0 - I morti possono guardare tutti i giocatori
- Valore: 1 - I morti possono guardare solo i compagni di squadra
...
Questo comando definisce per quanto tempo i giocatori restano congelati all’inizio di ogni round. Questo tempo permette di comprare equipaggiamento e discutere tattiche prima che inizi il round. Default è 15 secondi.



**mp_friendlyfire**

Questo comando definisce se il fuoco amico è abilitato o disabilitato. Se attivato, i giocatori possono danneggiare i compagni di squadra. Il valore di default dipende dalla modalità di gioco: nelle competitive è attivo, nelle casual no.



**mp_win_panel_display_time**

Questo comando definisce per quanti secondi viene mostrato lo scoreboard tra i round. Il valore di default è 3 secondi.
...
Questo comando definisce il numero massimo di minuti che dura un round prima che venga vinto o perso. Se il round finisce dopo questo tempo, la vittoria va ai counter-terroristi. I secondi sono dati in decimali, 1.92 = 1m55s.
...
- Valore: 1 - Munizioni infinite senza ricarica
- Valore: 2 - Numero infinito di caricatori, ma bisogna ricaricare quando vuoti
