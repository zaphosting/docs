...
title: "Killing Floor 2: Panoramica del Dashboard"
description: "Scopri come gestire e ottimizzare al meglio il tuo server Killing Floor 2 con una panoramica chiara delle funzionalità e opzioni chiave → Scopri di più ora"
...
I messaggi di errore in particolare possono aiutarti a identificare e capire meglio la causa del problema. Spesso, dando un’occhiata mirata ai file di log, si possono scoprire e risolvere le difficoltà.
...
In questa sezione trovi una panoramica della cronologia delle interruzioni del sistema host su cui è attivo il tuo server di gioco. Maggiori info e lo stato aggiornato del servizio li trovi sulla nostra nuova [pagina di status](https://status.zap-hosting.com/).
...
Vuoi programmare riavvii del server a orari specifici? Il Pianificatore Riavvii ti permette di impostare riavvii automatici schedulati del tuo server di gioco. Con questa funzione puoi programmare riavvii a orari precisi o a intervalli regolari.
...
Con l’**FTP Browser** ottieni tutte le info necessarie per connetterti via FTP. L’accesso FTP ti dà accesso diretto ai file del tuo server di gioco. Per un uso ottimale consigliamo un client FTP esterno come **Filezilla** o **WinSCP**. Se non sai come usare FTP, dai un’occhiata alla nostra guida [Accesso FTP](gameserver-ftpaccess.md) che ti può aiutare.
...
Oltre ai nostri server di gioco, offriamo anche 10 database inclusi. Questi database servono, ad esempio, quando i dati devono essere salvati in un database. Sotto **Database** puoi creare un database e vedere le credenziali di accesso. Hai anche l’opzione di attivare i backup e importare backup di database esistenti.
...
Per i nostri server di gioco forniamo di default 10 GB di spazio per backup sul server di storage. Se ti serve più spazio, puoi fare un upgrade a pagamento. Così ti assicuri sempre spazio sufficiente per i backup e puoi accedere anche ai backup più vecchi se serve.
