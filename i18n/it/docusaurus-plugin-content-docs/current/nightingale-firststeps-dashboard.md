...
description: "Scopri come gestire e ottimizzare al meglio il tuo server Nightingale per massimizzare prestazioni e controllo → Scopri di più ora"
...
Nella sezione **Amministrazione Prodotto** trovi tutte le pagine necessarie per gestire il tuo server. Qui sotto elenchiamo nel dettaglio tutte le opzioni per la gestione del server.



### Dashboard

Il **Dashboard** del tuo server è il cuore dell’amministrazione. Qui trovi tutte le informazioni base ed essenziali sul tuo servizio in modo chiaro e immediato. Ad esempio, informazioni sulla location, stato, IP:Porta, memoria usata, gioco in corso, nome del server, mappa e numero di slot disponibili. Inoltre, viene mostrato anche l’utilizzo di CPU, memoria e RAM.

![img](https://screensaver01.zap-hosting.com/index.php/s/mrmTDbDfnqg95Qn/preview)

Dal dashboard puoi anche avviare, fermare o riavviare il tuo servizio, così hai il pieno controllo sul suo funzionamento. Inoltre, puoi accedere alla console live per avere informazioni in tempo reale sullo stato del server e intervenire direttamente se serve.
...
Grazie a queste informazioni, hai un’ottima visione sull’utilizzo del server. Inoltre, queste statistiche sono spesso utili per individuare possibili problemi di performance e restringere le cause. Ad esempio, puoi risalire a quando l’utilizzo è aumentato e se è collegato a eventi o modifiche specifiche. Questo è particolarmente importante in caso di problemi improvvisi, perché ti permette di agire in modo mirato per risolverli e analizzare sistematicamente le cause.
...
Con l’**FTP Browser** trovi tutte le info necessarie per connetterti via FTP. L’accesso FTP ti dà accesso diretto ai file del tuo server di gioco. Per un uso ottimale consigliamo un programma FTP esterno come **Filezilla** o **WinSCP**. Se non sai come usare FTP, dai un’occhiata alla guida [Accesso via FTP](gameserver-ftpaccess.md).

![](https://screensaver01.zap-hosting.com/index.php/s/sX85i9HyzR9wmAM/preview)

**FileZilla:** FileZilla è probabilmente il client FTP più semplice e conosciuto. Basta installarlo, avviarlo e inserire in alto i dati di accesso che trovi qui sopra. Con un click su "Connetti" sei subito collegato al server e puoi gestire i file.
...
Oltre ai server di gioco, offriamo anche 10 database inclusi. Questi database servono ad esempio quando devi salvare dati in un database. Sotto **Database** puoi crearne uno e vedere le credenziali di accesso. Hai anche l’opzione di attivare i backup e importare backup esistenti.
...
Nelle impostazioni puoi scegliere se creare backup automatici e se farli giornalmente o settimanalmente. Inoltre puoi decidere se eliminare i backup più vecchi quando lo spazio di archiviazione si riduce.

![](https://screensaver01.zap-hosting.com/index.php/s/nm9za5LTMEcNsga/preview)

Per i nostri server di gioco forniamo di default 10 GB di spazio per backup sul server di archiviazione. Se ti serve più spazio, puoi fare un upgrade a pagamento. Così hai sempre spazio a sufficienza per i backup e puoi accedere anche a quelli più vecchi se serve.
...
Offriamo una documentazione estesa per i nostri prodotti, che funge da knowledge base per le domande e dubbi più frequenti. Troverai guide e aiuti su vari temi per supportarti nell’uso e gestione del prodotto.
