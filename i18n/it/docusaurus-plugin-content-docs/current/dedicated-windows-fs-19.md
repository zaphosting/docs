...
Du hast einen Windows VPS oder Dedicated Server und willst darauf den Farming Simulator Dedicated Server 2019 installieren? Dann bist du hier genau richtig. In dieser Anleitung zeigen wir dir Schritt für Schritt, wie du den Service auf deinem Server installierst.



## Vorbereitung

Benötigt wird ein Server mit Windows Server 2016/2019, mindestens 4x 2,4 GHz CPU (AMD/Intel), mindestens 2GB RAM (DDR3/4) und 6GB freiem Speicher (SSD oder besser, empfohlen). Der Server muss auf 64-Bit Architektur laufen.  
Außerdem brauchst du eine eigene GameLicense (keine Steam-Version) sowie eine öffentliche IPv4-Adresse. Eine GPU oder Soundkarte sind für den Dedicated Server nicht nötig.  
Je nachdem, wie stark der Dedicated Server mit Mods oder DLCs erweitert wird, können mehr Ressourcen erforderlich sein. 
...
Die Datei wird heruntergeladen und sollte im Download-Ordner liegen (der Pfad kann variieren).

![](https://screensaver01.zap-hosting.com/index.php/s/2nPDeWB97FXoFer/preview)

Per Doppelklick öffnest du die Datei und kannst die Setup.exe starten.  
Danach liest du die AGB und bestätigst sie. Die Installation kannst du als Standard oder Custom durchführen, Standard reicht normalerweise aus.  
Klicke auf Installieren und Farming Simulator 2019 wird jetzt installiert – das kann ein paar Minuten dauern. 

![](https://screensaver01.zap-hosting.com/index.php/s/agaffABodEkxrse/preview)

Wenn die Installation fertig ist, erscheint die Meldung, die du mit „Fertigstellen“ bestätigst. 
...
Fehlermeldungen wegen fehlender GPU oder ähnlichem kannst du ignorieren oder mit Nein/Abbrechen schließen. Farming Simulator muss nur gestartet werden, um den Key einzugeben. 
...
Jetzt musst du einen Login-Namen und ein Passwort festlegen oder in der Config des Dedicated Servers auslesen/ändern, damit du dich später im Webinterface anmelden kannst.  
...
Um den Dedicated Server zu starten, musst du `"dedicatedServer.exe"` im gleichen Verzeichnis wie `"dedicatedServer.xml"` ausführen.
...
Der Login erfolgt mit den zuvor gesetzten/ausgelesenen Zugangsdaten aus der `"dedicatedServer.xml"`.  
...
Damit dein Server öffentlich erreichbar ist und du mit Freunden spielen kannst, müssen die Ports in der Windows-Firewall freigegeben werden.  
Die Ports 10823 und 8080 müssen für das TCP-Protokoll freigegeben werden. 10823 ist der Game-Server-Port, 8080 der Webport fürs Webinterface. Der Webport muss nur freigegeben werden, wenn du das Webinterface von außen nutzen willst – die Verwaltung ist auch lokal via RDP möglich.  
Wie du Ports unter Windows freigibst: [Portfreigabe (Firewall)](vserver-windows-port.md).  

Nach der Portfreigabe ist dein Server öffentlich erreichbar, sofern er gestartet wurde. 
