...
title: "Risk of Rain 2: Panoramica del Dashboard"
description: "Scopri come gestire e ottimizzare al meglio il tuo server Risk of Rain 2 con una panoramica chiara delle funzionalità e dei controlli principali → Scopri di più ora"
...
Grazie a queste info, hai una buona visione sull’utilizzo del server. Queste statistiche sono utili anche per identificare eventuali problemi di performance e restringere le cause. Ad esempio, puoi capire quando l’utilizzo è aumentato e se è legato a eventi o cambiamenti specifici. Questo è particolarmente importante se sorgono problemi improvvisi, così puoi intervenire in modo mirato per risolverli e analizzare sistematicamente le cause.
...
Se ti senti più sicuro e vuoi fare modifiche più precise, ti consigliamo di usare il Config Editor. Maggiori info le trovi nella categoria **Configs**.
...
Sotto **Giochi** puoi sempre cambiare la variante di gioco del tuo server esistente o installare un altro gioco in generale. I giochi con lo stesso prezzo per slot possono essere selezionati direttamente sotto **Giochi Disponibili**. Per tutti gli altri giochi serve prima un adeguamento del prezzo per slot. Maggiori info le trovi nella guida [Cambio Gioco](gameserver-gameswitch.md).
...
Oltre ai nostri server di gioco, offriamo anche 10 database inclusi. Questi database servono, ad esempio, quando i dati devono essere salvati in un database. Sotto **Database** puoi crearne uno e vedere le credenziali di accesso. Hai anche l’opzione di attivare i backup e importare backup di database esistenti.
...
Nelle impostazioni puoi decidere se i backup devono essere creati automaticamente e se farlo giornalmente o settimanalmente. Puoi anche scegliere se eliminare i backup più vecchi quando lo spazio disponibile si riduce.
