...
title: "Mount & Blade: Panoramica del Dashboard"
description: "Scopri come gestire al meglio il tuo server Mount & Blade e ottimizzarne le funzionalità per le migliori prestazioni → Scopri di più ora"
...
Il **Dashboard** del tuo server è il cuore dell’amministrazione. Qui trovi tutte le informazioni base ed essenziali sul tuo servizio in modo chiaro e ordinato. Per esempio, informazioni sulla location, stato, IP: Porta, memoria usata, gioco in corso, nome del server, mappa e numero di slot disponibili. Inoltre, viene mostrato anche l’utilizzo di CPU, memoria e RAM.

![img](https://screensaver01.zap-hosting.com/index.php/s/4AHGadrCTPaeTCF/preview)

Dal dashboard puoi anche avviare, fermare o riavviare il tuo servizio, così hai il pieno controllo sul suo funzionamento. Inoltre, puoi accedere alla console live per avere info in tempo reale sullo stato del server e intervenire direttamente se serve.
...
A seconda della location del server e della protezione DDoS disponibile, puoi anche monitorare gli attacchi attivi in tempo reale. Questo include dettagli sul traffico, mostrando sia il traffico legittimo che quello bloccato dai sistemi di mitigazione. Questo livello di trasparenza ti aiuta a capire come funziona la protezione e a identificare eventuali rischi in anticipo.
...
Il tuo server è stato offline temporaneamente e non sai perché? Controllare event log e log files non ha dato info? Potrebbe essere che il problema non sia direttamente sul tuo server, ma sul sistema host che lo ospita.
...
Qui trovi una panoramica della cronologia delle interruzioni del sistema host su cui gira il tuo server di gioco. Per info aggiornate e lo stato attuale dei servizi, visita la nostra nuova [pagina di status](https://status.zap-hosting.com/).
...
Nella sezione **Strumenti** del pannello di navigazione trovi varie sottosezioni che ti aiutano a gestire il server. Ci sono **FTP Browser**, **Database** e **Backup**. Qui sotto ti spieghiamo tutto nel dettaglio.
...
Oltre ai nostri server di gioco, offriamo anche 10 database inclusi. Questi sono necessari, per esempio, quando devi salvare dati in un database. Sotto **Database** puoi crearne uno e vedere le credenziali di accesso. Puoi anche attivare i backup e importare backup di database esistenti.
