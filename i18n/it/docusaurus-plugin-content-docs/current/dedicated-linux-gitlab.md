...
title: "Dedicated Server: Installare GitLab su Linux"
description: "Scopri come configurare GitLab su Linux per ottimizzare i flussi DevOps e migliorare la collaborazione del team → Scopri di più ora"
...
Per installare GitLab sul tuo server Linux devi prima connetterti tramite un client SSH. Dai un’occhiata alla nostra guida [Accesso iniziale (SSH)](vserver-linux-ssh.md) per saperne di più.
...
Se vuoi usare un’altra soluzione per inviare email, salta questo passo e [configura un server SMTP esterno](https://docs.gitlab.com/omnibus/settings/smtp) dopo aver installato GitLab seguendo la guida ufficiale di GitLab.
:::
