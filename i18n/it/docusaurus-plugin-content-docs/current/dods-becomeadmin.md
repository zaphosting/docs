---
id: dods-becomeadmin
title: "Day of Defeat: Source: Diventa admin"
description: "Scopri come assegnare i permessi admin per il controllo totale del server e gestire al meglio le funzioni di gioco → Scopri di più ora"
...
Assegnare i permessi di amministratore ti permette una gestione semplice e completa con il controllo totale del tuo server. Come admin, puoi usare tutte le opzioni e funzioni disponibili direttamente in-game. Qui sotto trovi tutti i passaggi per assegnare i permessi admin al tuo server.
...
Per registrarti come admin, è obbligatorio installare SourceMod o MetaMod. Inoltre, ti servirà lo Steam ID del tuo account Steam. Ci sono diversi modi per ottenerlo: puoi usare un sito esterno che fornisce questo servizio oppure recuperarlo direttamente dalla console in-game.
...
La pagina ti mostrerà il tuo SteamID insieme ad altre info dell’account. Per la registrazione admin ti serve il **SteamID32**:

```
steamID32: STEAM_0:1:XXXXXX
```

In alternativa, puoi recuperare il tuo SteamID direttamente in-game tramite la console. Avvia il gioco, connettiti al server, apri la console e digita:
...
Qui sotto trovi una lista dei comandi admin più comuni da usare sul tuo server.
...
Congratulazioni, se hai seguito tutto correttamente, hai configurato con successo i permessi admin per il tuo server. Per qualsiasi domanda o supporto, non esitare a contattare il nostro team di assistenza, disponibile ogni giorno per aiutarti! 🙂
