...
title: "Wurm Unlimited: Problemi comuni"
description: "Scopri come risolvere i problemi più comuni del server Wurm Unlimited per un’esperienza di gioco senza intoppi → Scopri di più ora"
...
A seconda della portata del progetto e del contenuto aggiuntivo, le risorse richieste potrebbero già essere più alte dall’inizio o aumentare col tempo. In questi casi, fare un upgrade del pacchetto server è la soluzione più semplice per mantenere performance e stabilità.
...
Non tutte le estensioni sono sviluppate con la performance in mente. Che sia un framework, plugin, mod o risorsa, un’implementazione scarsa può causare grossi problemi di performance sul server. Spesso la funzionalità prevista funziona, ma il modo in cui è realizzata è inefficiente, troppo complesso o carica inutilmente le risorse del server.

Questo può causare alto uso di CPU, memory leak, lag o crash, specialmente se più componenti non ottimizzati interagiscono. Assicurati sempre che le estensioni siano mantenute attivamente, ben documentate e testate per la performance. In caso di dubbi, consulta feedback della community o monitora le prestazioni del server per individuare gli elementi problematici.
...
Backup regolari evitano un sacco di stress e preoccupazioni. Fai backup frequenti per essere sicuro che in caso di problemi hai sempre una versione precedente funzionante. Offriamo una [soluzione di backup](gameserver-backups.md) per i server Wurm Unlimited, che puoi usare per creare backup manuali o automatici programmati.
