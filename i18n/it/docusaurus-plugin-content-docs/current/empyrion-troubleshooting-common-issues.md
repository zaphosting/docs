...
title: "Empyrion: Problemi comuni"
description: "Scopri come risolvere i problemi comuni del server Empyrion per un'esperienza di gioco fluida → Scopri di più ora"
...
Questo può causare alto uso di CPU, memory leak, lag o crash, specialmente quando più componenti non ottimizzati interagiscono. Assicurati sempre che le estensioni siano mantenute attivamente, ben documentate e testate per le performance. In caso di dubbi, consulta il feedback della community o monitora le performance del server per individuare elementi problematici.
...
Se hai lag o ping alto, ci sono alcuni passi semplici per migliorare la situazione. Prima di tutto, assicurati che il server rispetti o superi le specifiche consigliate per il tuo gioco e progetto. Scegliere una location server vicina ai giocatori aiuta a ridurre la latenza.
...
Backup regolari evitano un sacco di stress e preoccupazioni. Crea backup frequenti per essere sicuro che in caso di problemi tu abbia sempre una versione precedente funzionante. Offriamo una [soluzione di backup](gameserver-backups.md) per i nostri server Empyrion, che puoi usare per creare backup manuali o automatici programmati.
