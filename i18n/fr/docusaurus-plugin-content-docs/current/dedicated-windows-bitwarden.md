...
Vous pensez à héberger ce service vous-même ? On vous guide pas à pas pour l’installer et le configurer, avec tout ce qu’il faut savoir pour bien démarrer.
...
Pour être sûr que votre système tourne avec les dernières améliorations logicielles et de sécurité, commencez toujours par faire les mises à jour système. Cela garantit que votre serveur dispose des derniers patchs de sécurité et des versions logicielles à jour avant de continuer.
...
Dans Docker Desktop, allez dans **Paramètres → Ressources → Partage de fichiers**. Ajoutez le dossier `C:\Bitwarden` aux chemins partagés. Cliquez sur Appliquer & Redémarrer pour valider les changements.
...
## Installation
Maintenant que tous les prérequis sont remplis et que les préparatifs sont faits, vous pouvez lancer l’installation de l’application Bitwarden.
...
Une configuration correcte affiche un message de succès ; sinon vous verrez des messages d’erreur sur OpenSSL manquant ou des valeurs incorrectes. Appliquez les changements avec `.\bitwarden.ps1 -restart`.

Ensuite, vérifiez les paramètres d’installation dans `.\bwdata\config.yml`. Ce fichier contrôle les ressources générées et doit être ajusté pour des environnements spécifiques, par exemple si vous êtes derrière un proxy ou utilisez des ports alternatifs. Appliquez ces modifications avec `.\bitwarden.ps1 -rebuild`.
...
Félicitations ! Vous avez maintenant installé et configuré avec succès Bitwarden sur votre serveur dédié. On vous recommande aussi de jeter un œil aux ressources suivantes, qui peuvent vous apporter un coup de main et des conseils supplémentaires pendant la configuration de votre serveur :
