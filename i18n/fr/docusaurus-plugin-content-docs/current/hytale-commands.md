...
description: "Aperçu des commandes console et en jeu disponibles pour les serveurs Hytale → En savoir plus maintenant"
...
Les serveurs Hytale offrent une large gamme de commandes permettant aux admins de gérer le gameplay, les joueurs, les mondes et la configuration du serveur. Ces commandes peuvent être exécutées soit via la console live du serveur, soit directement en jeu, à condition d’avoir les permissions nécessaires.
...
| --------------------- | ------------------------------------------------------------ |
| `/auth login device`  | Lance le processus d’authentification par appareil. Le serveur affiche une URL et un code appareil à valider avec un compte Hytale. |
...
Ces commandes gèrent l’accès admin et l’attribution fine des permissions pour les utilisateurs et groupes.

| Commande                                   | Description                                             |
| ----- | - |
| `/op add <PlayerName>`                    | Donne les privilèges d’opérateur au joueur spécifié.     |
| `/op remove <PlayerName>`                 | Retire les privilèges d’opérateur au joueur spécifié.    |
...
Les commandes de monde contrôlent la création, le chargement, le déchargement et la suppression des mondes.
...
Commandes utilisées pour gérer les positions des joueurs et leur visibilité.
...
| `/status`         | Montre le statut actuel du serveur et les infos d’exécution. |



## Conclusion

Le système de commandes Hytale offre des outils puissants pour gérer les serveurs, les joueurs et le comportement du gameplay. En comprenant et en utilisant ces commandes, les admins peuvent exploiter et personnaliser efficacement leur environnement serveur Hytale.
