...
description: "Découvrez comment récupérer l’accès à votre serveur dédié Windows après un verrouillage par mot de passe et minimisez les temps d’arrêt → En savoir plus maintenant"
...
Il est maintenant temps de réinitialiser l’accès du compte administrateur. Pour cela, suivez à nouveau les premières étapes de l’installation. Cependant, vous n’avez besoin d’aller que jusqu’au début de l’installation, puis cliquez sur **Réparer votre ordinateur**.

![](https://screensaver01.zap-hosting.com/index.php/s/qwPgHyqNaQdsqzm/preview)



Cela ouvre les options avancées. Cliquez sur l’option **Dépannage** puis sur **Invite de commandes**. Cela ouvrira l’invite de commandes (cmd.exe).
...
Après le redémarrage du serveur, sur l’écran de connexion, appuyez sur la combinaison de touches **Win+U**. Dans la fenêtre de l’invite de commandes, tapez la commande suivante :
...
Votre mot de passe a maintenant été changé. Cependant, vous devez annuler vos modifications sur les fichiers **Utilman**. Pour cela, redémarrez simplement votre serveur et refaites ces étapes. Dans l’invite de commandes, exécutez à nouveau les commandes suivantes :
...
Une fois le processus terminé, vous devriez avoir défini avec succès un nouveau mot de passe pour l’utilisateur administrateur. Vous pouvez maintenant vous reconnecter via la connexion bureau à distance. Pour toute question ou assistance, n’hésitez pas à contacter notre équipe support, disponible tous les jours pour vous aider ! 🙂