...
title: "Iron Armada : Connexion au serveur"
description: "Découvrez comment vous connecter facilement à votre serveur Iron Armada et résoudre les problèmes courants pour une expérience de jeu fluide → En savoir plus maintenant"
...
## Introduction
Vous ne savez pas comment vous connecter à votre **serveur Iron Armada** ou ce dont vous avez besoin pour commencer ? Pas de stress, on s’occupe de tout ! On va vous guider pas à pas – des outils et infos nécessaires jusqu’au processus de connexion, en passant par les points clés pour garantir une connexion fluide et sans galère. Suivez notre guide et vous serez connecté en un rien de temps !
...
Pour vous connecter à votre serveur via le navigateur de serveurs en jeu, ouvrez-le en cliquant sur **Multijoueur** dans le menu principal. Vous pouvez vous connecter dans la liste des serveurs soit via le nom du serveur, soit via la **connexion directe** en utilisant l’adresse IP et le port de votre serveur.
