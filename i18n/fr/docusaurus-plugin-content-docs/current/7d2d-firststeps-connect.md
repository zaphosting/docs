...
title: "7 Days To Die : Connexion au serveur"
description: "Découvrez comment vous connecter facilement à votre serveur 7 Days To Die et résoudre les problèmes courants pour un gameplay sans interruption → En savoir plus maintenant"
...
## Introduction
Vous ne savez pas comment vous connecter à votre **serveur de jeux 7 Days To Die** ou ce dont vous avez besoin pour commencer ? Pas de souci, on s’occupe de tout ! On va vous guider à travers tout ce qu’il faut – des outils et infos nécessaires jusqu’au processus de connexion, avec les points clés pour garantir une connexion fluide et sans accrocs. Suivez notre guide et vous serez connecté en un rien de temps !
...
<Tabs>
    <TabItem value="connect_solution_server_direct" label="Connexion directe (In game)" default>

Pour vous connecter à votre serveur, ouvrez le jeu. Une fois dans le jeu, cliquez sur **Rejoindre une partie**. Cliquez sur **Connexion via IP** en bas et entrez l’adresse IP et le port du serveur dans la fenêtre.
...
Félicitations, si vous avez bien suivi tout ça, vous devriez être connecté avec succès au serveur. Pour toute autre question ou aide, n’hésitez pas à contacter notre support, dispo tous les jours pour vous filer un coup de main ! 🙂
