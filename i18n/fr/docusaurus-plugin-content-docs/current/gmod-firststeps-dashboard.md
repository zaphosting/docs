...
description: "Découvrez comment gérer et optimiser efficacement votre serveur de jeux Garry's Mod avec un aperçu clair et structuré → En savoir plus maintenant"
...
Le **Tableau de Bord** de votre serveur est le cœur de l’administration. Toutes les informations de base et essentielles concernant votre service y sont présentées clairement. Cela inclut, par exemple, des infos sur l’emplacement du serveur, son statut, l’IP : Port, la mémoire utilisée, le jeu en cours, le nom du serveur, la map et le nombre de slots disponibles. En plus, l’utilisation du CPU, de la mémoire et de la RAM est aussi affichée.
...
L’utilisation du Gestionnaire DDoS nécessite soit une adresse IP dédiée, soit l’option supplémentaire "Aperçu Gestionnaire DDoS". Si votre forfait actuel ne comprend pas ces composants, vous pouvez les ajouter à tout moment en améliorant votre service.
:::

![](https://screensaver01.zap-hosting.com/index.php/s/ScCCCY52CMLgfyE/preview)

:::warning
L’utilisation du Gestionnaire DDoS n’est possible que si une adresse IP propre ou l’option "Aperçu Gestionnaire DDoS" a été réservée en option supplémentaire. Si ces composants manquent dans votre forfait actuel, vous pouvez les ajouter à tout moment en effectuant une montée en gamme.
:::
...
Les messages d’erreur en particulier peuvent aider à identifier et mieux comprendre la cause d’un problème. Souvent, les difficultés peuvent être découvertes et résolues en regardant attentivement les fichiers journaux.

> ⚠️ Les infos trouvées ne vous aident pas ou vous ne les comprenez pas ? Pas de souci ! Dans ce cas, notre support est disponible tous les jours. Il vous suffit de créer un **[Ticket](https://zap-hosting.com/en/customer/support/)** et de nous expliquer votre problème. Nous le traiterons au plus vite et vous aiderons à résoudre le souci ! :)
...
En plus de surveiller les événements, vous pouvez aussi envoyer des commandes directement au serveur via la **Console en direct**. Cela peut être utile pour ajuster des paramètres, effectuer certaines actions ou réagir rapidement à des problèmes.
...
L’éditeur de fichiers de config vous permet de modifier directement les fichiers de configuration depuis le site web, à l’ancienne, sans la vue simplifiée. Cette option vous permet de personnaliser les réglages de votre serveur selon vos préférences.
...
Si vous cherchez du nouveau contenu pour enrichir votre expérience de jeu, le Steam Workshop propose une large sélection de mods et maps pour plusieurs jeux. Pour utiliser une collection de mods ou maps dans votre jeu, il vous suffit d’entrer l’ID de la collection Steam Workshop. Ensuite, vous devrez redémarrer votre serveur pour que le nouveau contenu soit chargé. Une fois fait, vous pourrez utiliser ce contenu sur votre serveur et ajouter plus de variété et de fun à votre jeu.
