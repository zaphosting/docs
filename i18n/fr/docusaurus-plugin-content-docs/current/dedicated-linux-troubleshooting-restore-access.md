...
description: "Découvrez comment retrouver l’accès à votre serveur dédié Linux après un blocage de mot de passe et minimiser les temps d’arrêt → En savoir plus maintenant"
...
En mode récupération, le système de fichiers root est monté en lecture seule par défaut. Pour pouvoir changer le mot de passe, il faut le remonter en mode lecture-écriture. Exécutez la commande suivante :

```bash
mount -o remount,rw /
```

Si vous ne connaissez pas le nom d’utilisateur, vous pouvez lister tous les comptes disponibles en vérifiant les dossiers home :
...
Remplacez `username` par le nom du compte réel. Entrez le nouveau mot de passe et confirmez-le quand demandé. La saisie du mot de passe ne s’affichera pas à l’écran. Une fois le mot de passe mis à jour avec succès, redémarrez le système avec la commande `reboot`.
