...
title: "Vintage Story : Problèmes courants"
description: "Découvrez comment résoudre les problèmes courants des serveurs Vintage Story et maintenir un gameplay fluide pour votre communauté → En savoir plus maintenant"
...
Un serveur qui n’apparaît pas peut être dû à une initialisation incomplète. Ça peut venir d’une mauvaise config ou de fichiers corrompus. Tu peux souvent trouver plus d’infos dans la console du serveur ou les fichiers logs. Aussi, vérifie que tu n’as pas de filtres incorrects dans la liste des serveurs qui empêcheraient l’affichage.
...
Ça peut provoquer des crashs, freezes ou erreurs, surtout si plusieurs extensions problématiques interagissent. Si tu penses qu’une extension est en cause, essaie de la désactiver temporairement et vois si ton serveur reste stable sans. C’est une méthode simple pour identifier l’extension qui pose problème.

Assure-toi que toutes tes extensions sont à jour, bien maintenues et testées pour la compatibilité avec la version actuelle du jeu pour éviter crashs et interruptions.
...
Une bonne perf serveur est essentielle pour une expérience de jeu au top. Les soucis comme le lag, les retards ou les crashs soudains arrivent souvent quand la config serveur n’est pas optimale, que le matos ne suit pas ou que les extensions surchargent le système.
...
ZAP-Hosting propose une config minimale recommandée lors de la commande. Ces suggestions sont basées sur des cas d’usage typiques et t’aident à éviter les soucis de perf comme le lag, les crashs ou les temps de chargement longs.
...
Selon l’ampleur de ton projet et la quantité de contenu additionnel, les ressources nécessaires peuvent être plus élevées dès le départ ou augmenter avec le temps. Dans ce cas, upgrader ton package serveur est la solution simple pour assurer perf et stabilité sur la durée.
...
### Réseau
Les problèmes réseau peuvent causer du lag, des retards ou des déconnexions. Ces soucis ont différentes causes mais se règlent souvent avec les bons réglages et mesures de sécurité.
...
Si tu penses que des problèmes de routage ou réseau externe causent des délais, n’hésite pas à contacter notre support. Ils t’aideront à analyser et trouver la meilleure solution.
...
Faire des sauvegardes régulières évite beaucoup de stress et de galères. Crée des sauvegardes régulières pour être sûr qu’en cas de pépin, tu as toujours une version plus ancienne qui fonctionnait. On propose une [solution de sauvegarde](gameserver-backups.md) pour nos serveurs de jeux Vintage Story, que tu peux utiliser pour faire des backups manuels ou automatiques programmés.
...
Sinon, n’hésite surtout pas à contacter notre support pour des questions ou de l’aide, ils sont dispo tous les jours pour te filer un coup de main ! 🙂
