...
title: "Terraria : Aperçu du Tableau de Bord"
description: "Découvrez comment gérer et optimiser efficacement votre serveur de jeux Terraria avec un guide d’administration clair et étape par étape → En savoir plus maintenant"
...
Le **Tableau de Bord** de votre serveur est le cœur de l’administration. Toutes les informations de base et essentielles sur votre service y sont présentées clairement. Cela inclut, par exemple, des infos sur l’emplacement du serveur, son statut, l’adresse IP : port, la mémoire utilisée, le jeu en cours, le nom du serveur, la carte et le nombre de slots disponibles. En plus, l’utilisation du CPU, de la mémoire et de la RAM est aussi affichée.
...
En plus de surveiller les événements, vous pouvez aussi envoyer des commandes directement au serveur via la **Console Live**. C’est super utile pour ajuster des réglages, effectuer certaines actions ou réagir vite à un problème.

### Interruptions de service

Votre serveur a été temporairement hors ligne et vous ne comprenez pas pourquoi ? Un coup d’œil dans le journal des événements et les fichiers journaux n’a rien donné non plus ? Il se peut que la cause ne vienne pas directement de votre serveur, mais du système hôte sur lequel il tourne.
...
- Paramètres de base : Protection anti-triche supplémentaire, Titre du Monde, Taille du nouveau monde, Créer un nouveau monde ?, Mot de passe du serveur
...
Dans la section **Outils** de votre panneau de navigation, vous trouverez plusieurs sous-menus qui vous aideront à gérer votre serveur. Cela inclut **Navigateur FTP**, **Bases de données** et **Sauvegardes**. Vous aurez ci-dessous une introduction détaillée à ces différentes zones.
