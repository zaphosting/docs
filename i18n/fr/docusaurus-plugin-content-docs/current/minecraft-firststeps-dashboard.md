...
title: "Minecraft : Aperçu du Tableau de Bord"
description: "Découvrez comment gérer efficacement votre serveur de jeux Minecraft avec un aperçu clair des fonctionnalités et options → En savoir plus maintenant"
...
Le **Tableau de Bord** de votre serveur est le cœur de l’administration. Toutes les informations de base et essentielles sur votre service y sont présentées clairement. Cela inclut, par exemple, des infos sur l’emplacement du serveur, son statut, l’adresse IP : port, la mémoire utilisée, le jeu en cours, le nom du serveur, la map et le nombre de slots disponibles. En plus, l’utilisation du CPU, de la mémoire et de la RAM est aussi affichée.
...
L’utilisation du Gestionnaire DDoS nécessite soit une adresse IP dédiée, soit l’option supplémentaire "Vue d’ensemble du Gestionnaire DDoS". Si votre offre actuelle ne comprend pas ces composants, vous pouvez les ajouter à tout moment en améliorant votre service.
:::

![](https://screensaver01.zap-hosting.com/index.php/s/ScCCCY52CMLgfyE/preview)

:::warning
L’utilisation du Gestionnaire DDoS n’est possible que si une adresse IP propre ou l’option "Vue d’ensemble du Gestionnaire DDoS" a été réservée en option supplémentaire. Si ces composants manquent dans votre offre actuelle, vous pouvez les ajouter à tout moment en effectuant une montée en gamme.
:::
...
Votre serveur a été temporairement hors ligne et vous ne comprenez pas pourquoi ? Un coup d’œil dans le journal des événements et les fichiers journaux n’a rien donné non plus ? Il se peut que la cause ne vienne pas directement de votre serveur, mais du système hôte sur lequel il est exploité.
...
Dans cette section, vous trouverez un historique des interruptions du système hôte sur lequel votre service de serveur de jeux est hébergé. Plus d’infos et la dernière version de notre statut de service sont disponibles sur notre nouvelle [page de statut](https://status.zap-hosting.com/).
...
Dans les paramètres, vous trouverez un aperçu des options de réglage les plus courantes pour votre serveur. Elles sont accompagnées d’une courte explication. L’utilisation de cette page est particulièrement recommandée aux débutants en administration de serveur.

Si vous êtes plus à l’aise et souhaitez faire des modifications plus précises, nous vous conseillons d’utiliser plutôt l’Éditeur de Configs. Vous trouverez plus d’infos à ce sujet dans la catégorie **Configs**.

### Configs

L’éditeur de fichiers de configuration vous permet de modifier directement les fichiers de config depuis le site, à l’ancienne, sans la vue simplifiée. Cette option vous permet de personnaliser les réglages de votre serveur selon vos préférences.
...
De plus, vous pouvez activer la notification par email pour les mises à jour automatiques afin d’être informé des mises à jour et rester toujours à la page.
...
La gestion des joueurs vous permet de gérer vos joueurs rapidement et facilement. Ajoutez des joueurs en tant qu’opérateurs ou bannissez ceux qui ne sont pas les bienvenus en quelques clics.
