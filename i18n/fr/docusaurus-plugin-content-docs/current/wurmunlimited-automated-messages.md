...
title: "Wurm Unlimited : Messages Automatisés du Serveur - Tiens tes joueurs au courant des news, liens sociaux et plus encore !"
description: "Configure des messages automatisés via des tâches planifiées pour afficher des annonces récurrentes"
...
Avec les **Tâches Planifiées**, les messages du serveur peuvent être envoyés automatiquement dans le chat en jeu de **Wurm Unlimited** en exécutant des commandes prédéfinies selon un planning récurrent.
...
Les tâches planifiées offrent un contrôle total sur le moment et la fréquence d’apparition des messages. Voici quelques cas d’usage courants :
...
L’intervalle peut être modifié à tout moment sans redémarrer le serveur, ce qui permet d’ajuster facilement selon l’évolution de ton serveur.
...
Si le jeu supporte des plugins, mods ou extensions offrant plus de flexibilité et d’options de personnalisation, il peut être intéressant d’utiliser ces solutions. Dans ce cas, des systèmes de messagerie spécialisés peuvent proposer des fonctionnalités plus riches que les commandes natives du jeu et être mieux adaptés à des serveurs complexes ou très personnalisés.
