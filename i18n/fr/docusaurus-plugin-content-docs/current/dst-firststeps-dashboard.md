...
description: "Découvrez comment gérer efficacement votre serveur de jeux Don't Starve Together et optimiser ses réglages pour une expérience de jeu au top → En savoir plus maintenant"
...
Le **Tableau de Bord** de ton serveur est le cœur de l’administration. Toutes les infos de base et essentielles sur ton service y sont clairement présentées. Ça inclut, par exemple, des infos sur l’emplacement du serveur, son statut, l’IP : Port, la mémoire utilisée, le jeu en cours, le nom du serveur, la carte et le nombre de slots disponibles. En plus, l’utilisation du CPU, de la mémoire et de la RAM est aussi affichée.
...
L’utilisation du Gestionnaire DDoS nécessite soit une adresse IP dédiée, soit l’option supplémentaire "Vue d’ensemble du Gestionnaire DDoS". Si ton pack actuel ne comprend pas ces composants, tu peux les ajouter à tout moment en upgrade ton service.
:::
...
En plus de surveiller les événements, tu peux aussi envoyer des commandes directement au serveur via la **Console Live**. Pratique pour ajuster les réglages, effectuer certaines actions ou réagir vite aux problèmes.

### Interruptions de Service

Ton serveur a été hors ligne temporairement et tu ne comprends pas pourquoi ? Le journal des événements et les fichiers de log n’ont rien donné non plus ? Il se peut que la cause ne vienne pas directement de ton serveur, mais du système hôte sur lequel il tourne.
...
Crée des tâches planifiées qui s’exécutent automatiquement à des moments définis pour automatiser différents processus sur ton serveur. Tu peux choisir si une tâche doit s’exécuter une seule fois ou se répéter. Les types de tâches supportées incluent démarrer, arrêter ou redémarrer des services, redémarrer le serveur s’il est en ligne, créer des sauvegardes ou exécuter des commandes personnalisées.
...
**FileZilla :** FileZilla est probablement le client FTP le plus simple et connu. Installe-le, lance-le et entre tes identifiants en haut, que tu trouveras un peu plus haut sur cette page. Un clic rapide sur "Connexion" et tu es direct connecté au serveur pour gérer les fichiers.
...
En plus de nos produits de serveurs de jeux, on propose aussi 10 bases de données incluses. Ces bases sont nécessaires, par exemple, quand des données doivent être stockées dans une base. Sous **Bases de Données**, tu peux créer une base et voir les identifiants d’accès. Tu peux aussi activer l’option de sauvegarde et importer des sauvegardes de bases existantes.
