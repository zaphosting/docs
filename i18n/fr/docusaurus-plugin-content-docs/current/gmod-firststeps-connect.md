...
title: "Garry's Mod : Se connecter au serveur"
description: "Découvrez comment vous connecter facilement à votre serveur Garry's Mod et profiter d'une expérience de jeu sans interruption → En savoir plus maintenant"
...
## Introduction
Vous ne savez pas comment vous connecter à votre **serveur de jeux Garry's Mod** ou ce dont vous avez besoin pour commencer ? Pas de stress, on s’occupe de tout ! On va vous guider à travers tout ce qu’il faut – des outils et infos nécessaires jusqu’au processus de connexion en lui-même, avec les points clés pour assurer une connexion fluide et sans galère. Suivez notre guide et vous serez connecté en un rien de temps !
...
Vous aurez besoin de toutes les infos nécessaires pour vous connecter à votre serveur. Ces infos se trouvent sur le tableau de bord de votre panneau web du serveur de jeux. Les infos importantes sont les suivantes :
...
Pour vous connecter à votre serveur via le navigateur de serveurs en jeu, ouvrez-le depuis le menu principal. Pour cela, cliquez sur **Trouver une partie multijoueur** puis ouvrez le **Navigateur Legacy**. Une fois là, cherchez votre serveur avec le **champ de recherche** et le nom de votre serveur, puis cliquez sur **Se connecter**.
...
Pour vous connecter à votre serveur via la console, ouvrez la console avec la touche `~`. Une fois la console ouverte, tapez la commande suivante : 
