...
title: "BeamMP : Aperçu du Tableau de Bord"
description: "Découvrez comment gérer et optimiser efficacement votre serveur BeamMP avec un guide clair et étape par étape adapté à vos besoins → En savoir plus maintenant"
...
Tebex est facile à utiliser et propose plein de fonctionnalités pour personnaliser et optimiser votre boutique. Vous pouvez modifier la mise en page, ajouter des options de paiement, et même lancer des campagnes marketing pour toucher plus de joueurs.

En cliquant sur l’image, vous pouvez créer votre propre boutique pour serveur de jeux avec Tebex. La mise en place est rapide et simple, vous serez prêt à vendre vos biens virtuels en un rien de temps.



### Fichiers journaux

Un problème technique survient et vous ne savez pas d’où ça vient ? Une première piste est de vérifier les **fichiers journaux**. Ces fichiers contiennent plein d’infos sur le serveur, comme les événements actuels, des indices ou des messages d’erreur.
...
:::info
Les infos trouvées ne vous aident pas ou vous ne les comprenez pas ? Pas de stress ! Dans ce cas, notre support est dispo tous les jours. Créez simplement un **[ticket](https://zap-hosting.com/en/customer/support/)** et expliquez-nous votre problème. On le regarde au plus vite et on vous aide à le résoudre ! :)
:::
...
Votre serveur a été temporairement hors ligne et vous ne comprenez pas pourquoi ? Un coup d’œil dans le journal des événements et les fichiers journaux n’a rien donné non plus ? Il se peut que la cause du problème ne vienne pas directement de votre serveur, mais du système hôte sur lequel il est hébergé.
...
Ici, vous pouvez par exemple faire les réglages suivants :

- Paramètres de base : Nom du serveur, Description du serveur, Map, Clé d’authentification, Serveur privé
...
De plus, vous pouvez aussi activer la notification par email pour les mises à jour automatiques afin de rester informé et toujours à la page.
...
Dans les paramètres, vous pouvez définir si les sauvegardes doivent être créées automatiquement et si cela doit être fait quotidiennement ou hebdomadairement. En plus, vous pouvez décider si les sauvegardes les plus anciennes doivent être supprimées si nécessaire, par exemple quand l’espace de stockage disponible devient limité.

![](https://screensaver01.zap-hosting.com/index.php/s/nm9za5LTMEcNsga/preview)

Nous fournissons par défaut 10 Go d’espace de stockage sur le serveur de stockage pour les sauvegardes de nos produits de serveur de jeux. Si vous avez besoin de plus d’espace, vous pouvez l’upgrader moyennant un coût. Ainsi, vous vous assurez d’avoir toujours assez de place pour vos sauvegardes et pouvoir accéder aux plus anciennes si besoin.
...
Nous proposons une documentation complète pour nos produits, qui sert de base de connaissances pour les questions et préoccupations les plus courantes. Vous y trouverez différents guides et aides sur plusieurs sujets qui vous assisteront dans l’utilisation et la gestion de votre produit.
