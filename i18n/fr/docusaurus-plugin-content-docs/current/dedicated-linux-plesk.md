...
Pour que Plesk s’installe entièrement en automatique avec les composants standards utilisés par la majorité des utilisateurs Plesk, une seule commande suffit :
...
Une fois ces trois commandes lancées, la licence doit être approuvée avec "F" :
...
Ensuite, le système demande quel type de la version choisie vous souhaitez installer. En général, le type « Recommandé » déjà sélectionné suffit. Des modules non installés à ce stade mais nécessaires peuvent être ajoutés plus tard dans le panneau Plesk.
...
L’installation est terminée dès que ce message apparaît :
...
:::info
Au premier accès au panneau Web, le message « Ce n’est pas une connexion sécurisée » s’affiche. Il faut le confirmer une première fois pour ouvrir la page.
:::
...
Une fois l’inscription réussie, le compte Admin doit être configuré. Il faut renseigner un nom de contact, un email et un mot de passe. Si vous avez déjà une licence Plesk, vous pouvez la saisir directement. Sinon, une licence d’essai de 15 jours peut être demandée à Plesk. Enfin, le contrat utilisateur doit être validé.
Plesk est maintenant prêt à être utilisé.

### Choisir la langue

Après l’installation, le panneau Plesk est en anglais. Vous pouvez passer en allemand sous Outils & Paramètres ➡️ Apparence Plesk ➡️ Langues. Sélectionnez « de-DE ». Ensuite, cliquez sur « Définir par défaut » pour appliquer la langue à tous.
...
Saisissez votre domaine. Vous devez aussi définir une IP ainsi qu’un nom d’utilisateur et un mot de passe pour l’hébergement web du domaine. Si le sous-domaine « www » est déjà configuré dans le DNS du domaine, un certificat SSL Let's Encrypt peut être généré directement. Cette option peut être sélectionnée après avoir saisi une adresse mail, puis validée avec le bouton bleu « OK ».
