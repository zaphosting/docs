...
title: "Enshrouded : Aperçu du Tableau de Bord"
description: "Découvrez comment gérer efficacement votre serveur Enshrouded avec un aperçu clair des fonctionnalités et options → En savoir plus maintenant"
...
Vous êtes intéressé par un **serveur Enshrouded** ou vous en possédez déjà un, mais il vous manque encore une vue d’ensemble complète de l’administration et de la localisation de toutes les options nécessaires ? Surtout au début, cela peut être un vrai défi. Mais pas de panique, nous vous proposons ci-dessous une introduction détaillée à l’administration de votre service, pour que vous ayez la meilleure vue d’ensemble possible. Après avoir parcouru ce guide, vous pourrez gérer votre serveur facilement et rapidement, en trouvant toutes les options et fonctionnalités nécessaires sans prise de tête. Ici, vous aurez un aperçu structuré qui vous aidera pas à pas à vous familiariser avec l’administration du serveur et à approfondir vos connaissances de manière ciblée. Ainsi, vous pourrez vous assurer que votre **serveur Enshrouded** est parfaitement adapté à vos besoins et que vous tirez le meilleur parti des options disponibles.

<YouTube videoId="wgvfvQdtwN4" imageSrc="https://screensaver01.zap-hosting.com/index.php/s/tHLf63nL2FcGTFm/preview" title="Configurer un serveur Enshrouded en seulement UNE MINUTE !" description="Vous comprenez mieux en voyant les choses en action ? On a ce qu’il vous faut ! Plongez dans notre vidéo qui vous explique tout. Que vous soyez pressé ou que vous préfériez apprendre de manière super engageante !"/>
...
:::info
Les infos trouvées ne vous aident pas ou vous ne les comprenez pas ? Pas de souci ! Dans ce cas, notre support est dispo tous les jours. Il suffit de créer un **[Ticket](https://zap-hosting.com/en/customer/support/)** et de nous expliquer votre problème. On le regarde au plus vite et on vous aide à résoudre ça ! :)
:::
...
En plus de surveiller les événements, vous pouvez aussi envoyer des commandes directement au serveur via la **Console Live**. C’est pratique pour ajuster des réglages, effectuer certaines actions ou réagir vite aux problèmes qui surviennent.



### Interruptions de service

Votre serveur a été temporairement hors ligne et vous ne comprenez pas pourquoi ? Un coup d’œil dans le journal des événements et les fichiers journaux n’a rien donné non plus ? Il se peut que la cause ne vienne pas directement de votre serveur, mais du système hôte sur lequel il est hébergé.
...
Dans cette section, vous trouverez un historique des interruptions du système hôte sur lequel tourne votre service de serveur de jeux. Pour plus d’infos et la dernière version de notre statut de service, rendez-vous sur notre nouvelle [page de statut](https://status.zap-hosting.com/).
...
Dans les paramètres, vous avez un aperçu des options de réglages les plus courantes pour votre serveur. Elles sont accompagnées d’une courte explication. La page des paramètres est particulièrement recommandée aux débutants en administration de serveur.
...
L’éditeur de fichiers de config vous permet de modifier directement les fichiers de configuration depuis le site, à l’ancienne, sans passer par la vue simplifiée. Cette option vous permet de personnaliser les réglages de votre serveur selon vos préférences.
...
Dans la section **Outils** de votre panneau de navigation, vous trouverez plusieurs sous-éléments qui vous aideront à gérer votre serveur. Cela inclut **Navigateur FTP**, **Bases de données** et **Sauvegardes**. Vous aurez ci-dessous une introduction détaillée à ces différents domaines.



### Navigateur FTP

Avec le **Navigateur FTP**, vous obtenez toutes les infos nécessaires pour établir une connexion à l’accès FTP. L’accès FTP vous permet un accès direct aux fichiers de votre serveur de jeux. On recommande d’utiliser un programme FTP externe comme **Filezilla** ou **WinSCP** pour une utilisation optimale. Si vous ne connaissez pas bien FTP, jeter un œil au guide [Accès via FTP](gameserver-ftpaccess.md) peut vous aider.
...
**WinSCP :** WinSCP est en fait prévu pour SCP et SFTP (les descendants chiffrés du protocole FTP), mais il gère aussi le FTP. Le programme est plus complet que FileZilla et convient aux utilisateurs avancés.
