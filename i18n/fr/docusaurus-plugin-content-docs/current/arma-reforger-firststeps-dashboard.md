...
description: "Découvrez comment gérer et optimiser efficacement votre serveur Arma Reforger avec un aperçu clair des fonctionnalités clés et des options → En savoir plus maintenant"
...
Le **Tableau de Bord** de votre serveur est le cœur de l’administration. Toutes les informations de base et essentielles concernant votre service y sont présentées de manière claire. Cela inclut, par exemple, des infos sur l’emplacement du serveur, son statut, l’IP : Port, la mémoire utilisée, le jeu en cours, le nom du serveur, la carte et le nombre de slots disponibles. De plus, l’utilisation du CPU, de la mémoire et de la RAM est également affichée.
...
L’éditeur de fichiers de config vous permet de modifier les fichiers de configuration directement depuis le site, à l’ancienne, sans la vue simplifiée. Cette option vous permet de personnaliser les réglages de votre serveur et de les configurer selon vos préférences personnelles.
...
La section **Questions fréquentes** offre un aperçu des questions les plus posées par nos clients. Vous y trouverez des réponses utiles et des informations détaillées sur divers sujets pour vous aider rapidement et efficacement.
