...
title: "Satisfactory : Aperçu du Tableau de Bord"
description: "Découvrez comment gérer efficacement votre serveur Satisfactory et optimiser ses fonctionnalités selon vos besoins → En savoir plus maintenant"
...
Le **Tableau de Bord** de votre serveur est le cœur de l’administration. Toutes les informations de base et essentielles sur votre service y sont présentées clairement. Cela inclut, par exemple, des infos sur l’emplacement, le statut, l’IP : Port, la mémoire utilisée, le jeu en cours, le nom du serveur, la carte et le nombre de slots disponibles. En plus, l’utilisation du CPU, de la mémoire et de la RAM est aussi affichée.
...
Consultez les attaques DDoS passées pour mieux évaluer les menaces potentielles et les tendances. Activez les notifications pour les attaques DDoS en cours sur votre serveur afin de réagir rapidement en cas d’incident.
...
Un problème technique survient et vous ne savez pas d’où ça vient ? Une première stratégie possible est de vérifier les **fichiers logs**. Ces fichiers contiennent plein d’infos sur le serveur, comme les événements en cours, des indices ou des messages d’erreur.
...
:::info
Les infos trouvées ne vous aident pas ou vous ne les comprenez pas ? Pas de souci ! Dans ce cas, notre support est dispo tous les jours. Créez simplement un **[Ticket](https://zap-hosting.com/en/customer/support/)** et expliquez-nous votre problème. On le regardera au plus vite et vous aidera à résoudre ça ! :)
:::
...
Le **Journal des Événements** vous offre une vue complète de toutes les activités liées à l’administration du serveur. Vous pouvez suivre à tout moment quand un serveur a démarré, arrêté ou planté, ainsi que quand des mises à jour, sauvegardes ou nouvelles installations ont été effectuées.
...
En plus de surveiller les événements, vous pouvez aussi envoyer des commandes directement au serveur via la **Console Live**. C’est pratique pour ajuster des réglages, effectuer certaines actions, ou réagir rapidement aux problèmes qui surviennent.



### Interruptions de Service

Votre serveur a été temporairement hors ligne et vous ne comprenez pas pourquoi ? Un coup d’œil dans le journal des événements et les fichiers logs n’a rien donné non plus ? Il se peut que la cause ne vienne pas directement de votre serveur, mais du système hôte sur lequel il tourne.
...
Dans les paramètres, vous avez un aperçu des options de réglages les plus courantes pour votre serveur. Elles sont accompagnées d’une courte explication. L’utilisation de la page des paramètres est particulièrement recommandée aux débutants en administration de serveur.

![](https://screensaver01.zap-hosting.com/index.php/s/MJ9asmEY8W5LBKS/preview)

Ici, vous pouvez par exemple faire les ajustements suivants : 

- Paramètres de base (Sauvegarde active (sans extension de fichier .sav), Nom du serveur, Intervalle d’autosave)
...
Dans **Versions**, vous pouvez voir et gérer la version actuelle de votre serveur de jeux. Vous pouvez mettre à jour vers la dernière version à tout moment ou activer les mises à jour automatiques pour que votre serveur soit toujours à jour.

![](https://screensaver01.zap-hosting.com/index.php/s/Xqa6EQDFsjtxMEQ/preview)

En plus, vous pouvez activer la notification par email pour les mises à jour automatiques afin de rester informé et toujours à jour.



### Gestionnaire de Sauvegardes de Partie

Avec le gestionnaire de sauvegardes de partie, vous pouvez facilement sauvegarder, restaurer et importer vos progrès de jeu. Cette option simplifie la gestion de vos sauvegardes et vous aide à suivre votre progression sans effort.

![](https://screensaver01.zap-hosting.com/index.php/s/BADmpNYcn7Hkktt/preview)

Vous avez déjà une sauvegarde existante que vous voulez intégrer au gestionnaire ? Pas de souci ! Utilisez simplement l’uploader de sauvegarde pour envoyer votre fichier. Notez que la taille du fichier ne doit pas dépasser la limite maximale de 100 Mo. Sinon, la sauvegarde devra être uploadée manuellement via FTP.



### Tâches Planifiées

Créez des tâches planifiées qui s’exécutent automatiquement à des horaires définis pour automatiser divers processus sur votre serveur. Vous pouvez choisir si une tâche doit s’exécuter une seule fois ou se répéter selon vos besoins. Les types de tâches supportés incluent démarrer, arrêter ou redémarrer des services, redémarrer le serveur s’il est en ligne, créer des sauvegardes ou exécuter des commandes personnalisées.
...
Nous fournissons par défaut 10 Go d’espace de stockage sur le serveur de stockage pour les sauvegardes de nos produits de serveurs de jeux. Si vous avez besoin de plus d’espace, il est possible de l’upgrader moyennant un coût. Ainsi, vous vous assurez d’avoir toujours assez de place pour vos sauvegardes et d’accéder aux plus anciennes si besoin.
...
Nous proposons une documentation complète pour nos produits, qui sert de base de connaissances pour les questions et soucis les plus fréquents. Vous y trouverez divers guides et aides sur différents sujets pour vous accompagner dans l’utilisation et la gestion de votre produit.

![](https://screensaver01.zap-hosting.com/index.php/s/n48ct6aZBrNq7eT/preview)

### Actualités
La section **Actualités** vous donne un aperçu complet de toutes les annonces et nouveautés liées à votre service. Vous y trouverez des infos importantes sur divers aspects comme :
