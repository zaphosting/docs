...
title: "Serveur dédié : Changer le port RDP Windows"
...
Le protocole Remote Desktop utilise par défaut le port 3389. Ce port est largement connu et souvent ciblé par des scans automatisés. Changer le port RDP peut réduire les tentatives de connexion non désirées et améliorer la sécurité de base.

<InlineVoucher />



## Modifier le port RDP dans le registre
...
Modifiez la règle existante pour Remote Desktop ou créez une nouvelle règle entrante qui autorise le trafic TCP sur le port nouvellement configuré. Assurez-vous que la règle permet la connexion et s’applique aux profils réseau nécessaires.
