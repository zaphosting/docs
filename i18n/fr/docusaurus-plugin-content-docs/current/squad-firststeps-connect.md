...
title: "Squad : Se connecter au serveur"
description: "Découvrez comment vous connecter facilement à votre serveur Squad et résoudre les problèmes courants pour une expérience de jeu fluide → En savoir plus maintenant"
...
## Introduction
Vous ne savez pas comment vous connecter à votre **serveur de jeux Squad** ou ce dont vous avez besoin pour commencer ? Pas de stress, on s’occupe de tout ! On vous guide à travers tout ce qu’il faut – des outils et infos nécessaires jusqu’au processus de connexion, avec les points clés pour garantir une connexion fluide et sans galère. Suivez notre guide et vous serez connecté en un rien de temps !
...
Pour vous connecter à votre serveur via le navigateur de serveurs en jeu, ouvrez-le depuis le menu principal. Une fois dans la liste des serveurs, cliquez sur **Navigateur personnalisé**. Utilisez le **champ de recherche** pour chercher votre serveur avec son nom. Sélectionnez-le et cliquez sur **Rejoindre le serveur**.
...
Le serveur peut ne pas apparaître si l’initialisation n’a pas été complétée avec succès. Cela peut être dû par exemple à une mauvaise configuration ou des fichiers corrompus. Vous pouvez généralement trouver plus d’infos dans la console du serveur ou les fichiers logs.
