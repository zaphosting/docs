...
title: "Myth of Empires : Aperçu du Tableau de Bord"
description: "Découvrez comment gérer efficacement votre serveur Myth of Empires et optimiser ses réglages pour une expérience de jeu au top → En savoir plus maintenant"
...
Tu es intéressé par un **Myth of Empires** ou tu en possèdes déjà un, mais tu manques encore d’une vue d’ensemble complète de l’administration et de la localisation de toutes les options nécessaires ? Surtout au début, ça peut être un vrai défi. Mais t’inquiète, on te propose ci-dessous une introduction détaillée à l’administration de ton service, pour que tu aies la meilleure vue d’ensemble possible.

Après avoir parcouru ce guide, tu pourras gérer ton serveur facilement et trouver toutes les options et fonctionnalités rapidement et sans prise de tête. Ici, tu auras un aperçu structuré qui t’aidera pas à pas à te familiariser avec l’administration du serveur et à approfondir tes connaissances de manière ciblée. Comme ça, tu peux t’assurer que ton **Myth of Empires** est parfaitement adapté à tes besoins et que tu tires le meilleur parti des options disponibles.

<YouTube videoId="a-tZnWIpOSg" imageSrc="https://screensaver01.zap-hosting.com/index.php/s/4MxygkgjnLKW5Hw/preview" title="Configure ton serveur MOE en juste une MINUTE !" description="Tu préfères comprendre en voyant les choses en action ? On a ce qu’il te faut ! Plonge dans notre vidéo qui te décompose tout. Que tu sois pressé ou que tu préfères apprendre de façon plus fun et visuelle !" />
...
Dans les **Statistiques**, tu as une vue complète sur les valeurs de charge de ton serveur. Tu y trouveras des infos sur différents aspects comme la charge des slots disponibles, du CPU et de la mémoire. Tu peux consulter ces données sur différentes périodes, par exemple les dernières **60 minutes, 24 heures, 3 jours, 7 jours** ou **30 jours**.

![](https://screensaver01.zap-hosting.com/index.php/s/N8jcmdqqG2Xt4Bk/preview)

Grâce à ces infos, tu as un bon aperçu de l’utilisation de ton serveur. Ces stats sont souvent utiles pour détecter d’éventuels soucis de performance et en cerner les causes. Par exemple, tu peux retracer quand l’utilisation a augmenté et si ça correspond à certains événements ou changements. C’est super important quand des problèmes soudains surviennent, car ça te permet de prendre des mesures ciblées pour dépanner et analyser méthodiquement les causes possibles.
...
En cliquant sur l’image, tu peux créer ta propre boutique pour serveur de jeux avec Tebex. La configuration est rapide et facile, tu seras prêt à vendre tes biens virtuels en un rien de temps.

### Fichiers de Log

Un problème technique survient et tu ne sais pas d’où ça vient ? Une première piste est de vérifier les **Fichiers de Log**. Ces fichiers contiennent plein d’infos sur le serveur, comme les événements en cours, des indices ou des messages d’erreur.

Les messages d’erreur en particulier peuvent aider à identifier et mieux comprendre la cause d’un souci. Souvent, les difficultés peuvent être décelées et résolues en jetant un œil ciblé aux fichiers de log.
...
Le **Journal des Événements** te donne une vue complète de toutes les activités liées à l’administration du serveur. Tu peux y suivre à tout moment quand un serveur a démarré, arrêté ou planté, ainsi que quand des mises à jour, sauvegardes ou nouvelles installations ont été faites.

![](https://screensaver01.zap-hosting.com/index.php/s/xNzsaMbW5BS5KJC/preview)

### Interruptions de Service

Ton serveur a été hors ligne temporairement et tu ne comprends pas pourquoi ? Un coup d’œil dans le journal des événements et les fichiers de log n’a rien donné non plus ? Il se peut que la cause ne vienne pas directement de ton serveur, mais du système hôte sur lequel il tourne.
...
Dans les paramètres, tu as un aperçu des options les plus courantes pour ton serveur, avec une petite explication pour chacune. Cette page est particulièrement recommandée aux débutants en administration de serveur.
...
Tu peux aussi activer les notifications par email pour les mises à jour automatiques, afin d’être informé et toujours au courant des nouveautés.

### Tâches Planifiées

Crée des tâches planifiées qui s’exécutent automatiquement à des moments définis pour automatiser différents processus sur ton serveur. Tu peux choisir si une tâche doit s’exécuter une seule fois ou se répéter. Les types de tâches supportés incluent démarrer, arrêter ou redémarrer des services, redémarrer le serveur s’il est en ligne, créer des sauvegardes ou exécuter des commandes personnalisées.
...
Avec les paramètres DNS de ZAP, tu peux créer des adresses faciles à retenir pour ton service. Tu peux utiliser un sous-domaine de nos domaines ou créer une entrée DNS sur tes propres domaines pour avoir une adresse qui te correspond mieux.
...
**WinSCP :** WinSCP est à la base prévu pour SCP et SFTP (les versions sécurisées du protocole FTP), mais il gère aussi le FTP classique. Le programme est plus complet que FileZilla et convient aux utilisateurs avancés.
