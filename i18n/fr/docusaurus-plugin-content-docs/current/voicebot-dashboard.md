...
Dans la section **Administration du Produit**, vous trouverez toutes les pages nécessaires à la gestion de votre bot vocal. Les options pour gérer votre bot vocal sont listées en détail ci-dessous.
...
Des fonctionnalités avancées comme déplacer automatiquement les utilisateurs ou jouer un message de bienvenue lorsqu’un utilisateur rejoint le serveur peuvent être activées en configurant les paramètres appropriés. Ces fonctions permettent une gestion plus personnalisée et automatisée du serveur.
