...
title: "Abiotic Factor : Aperçu du Tableau de Bord"
description: "Découvrez comment gérer et optimiser efficacement votre serveur Abiotic Factor avec un aperçu clair de toutes les fonctionnalités et options clés → En savoir plus maintenant"
...
Un problème technique survient et vous ne savez pas d’où ça vient ? Une première stratégie possible est de vérifier les **fichiers journaux**. Ces fichiers contiennent plein d’infos sur le serveur, comme les événements en cours, des indices ou des messages d’erreur.
...
En plus de surveiller les événements, vous pouvez aussi envoyer des commandes directement au serveur via la **Console Live**. C’est super utile pour ajuster des réglages, effectuer certaines actions ou réagir vite aux problèmes qui surviennent.
...
L’éditeur de fichiers de config vous permet d’éditer directement les fichiers de configuration depuis le site, à l’ancienne, sans la vue simplifiée. Cette option vous permet de personnaliser les réglages de votre serveur et de les configurer selon vos préférences perso.

En utilisant l’éditeur, vous avez un accès direct à toutes les options disponibles et pouvez les personnaliser précisément. C’est particulièrement utile pour les utilisateurs avancés qui veulent un contrôle total sur les réglages de leur serveur et savent exactement ce qu’ils veulent modifier.

:::info
Vous avez remarqué que certains de vos changements sont partiellement écrasés ? Sachez que certaines valeurs d’option sont prises depuis la page des paramètres. Si vous voulez faire ces changements directement dans la config, vous devez d’abord désactiver les options concernées sur la page des paramètres.
:::
...
De plus, vous pouvez aussi activer la notification par email pour les mises à jour automatiques afin de rester informé des mises à jour et être toujours à la page.
...
**WinSCP :** WinSCP est en fait prévu pour SCP et SFTP (les descendants chiffrés du protocole FTP) mais peut gérer le FTP aussi. Le programme est plus complet que FileZilla et convient aux utilisateurs avancés.



### Bases de données

En plus de nos produits serveur de jeux, nous proposons aussi 10 bases de données incluses. Ces bases sont nécessaires, par exemple, quand des données doivent être stockées dans une base. Sous **Bases de données**, vous pouvez créer une base et voir les identifiants d’accès. Vous avez aussi la possibilité d’activer l’option de sauvegarde et d’importer des sauvegardes de bases existantes.
...
Les **Actualités** vous donnent un aperçu complet de toutes les annonces et nouveautés liées à votre service. Vous y trouverez des infos importantes sur différents aspects tels que :
...
- Offres spéciales et promotions qui vous donnent des avantages ou réductions exclusives
- Autres infos pertinentes pouvant impacter l’utilisation de votre service

En consultant régulièrement les **Actualités**, vous restez informé et pouvez ajuster votre service pour tirer le meilleur de votre serveur.
