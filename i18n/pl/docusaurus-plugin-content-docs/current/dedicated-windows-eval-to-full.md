...
title: "Serwer dedykowany: Konwersja Windows Server Evaluation na Pełną Wersję"
...
Gdy znasz już edycję docelową i masz ważny klucz produktu, możesz rozpocząć aktualizację. Wartość `<TargetEdition>` musi odpowiadać jednej z obsługiwanych edycji zwróconych przez DISM.

Popularne edycje docelowe to:
- `ServerStandard`
- `ServerDatacenter`
...
Wynik powinien teraz pokazać licencjonowaną edycję, np. `ServerStandard` lub `ServerDatacenter`, co potwierdzi, że konwersja z wersji testowej zakończyła się sukcesem.
