...
title: "VPS: Instalacja Odoo (Open Source ERP i CRM) na Linux"
description: "Dowiedz się, jak zarządzać i automatyzować procesy biznesowe dzięki zintegrowanej platformie ERP i CRM Odoo → Sprawdź teraz"
...
Odoo to modułowa, open-source’owa platforma łącząca funkcje Enterprise Resource Planning (ERP) oraz Customer Relationship Management (CRM). Pozwala firmom zarządzać i automatyzować procesy takie jak księgowość, magazyn, zarządzanie projektami czy sprzedaż z poziomu jednego systemu.
...
Po zakończeniu aktualizacji możesz przejść do instalacji zależności. Odoo będzie uruchamiane na Twojej maszynie w kontenerach Docker, dlatego najpierw musisz zainstalować Docker. Wykonaj polecenia:
...
Domyślnie Odoo działa na hoście na portach 80 (HTTP) i 443 (HTTPS). Skonfiguruj domenę z rekordami DNS wskazującymi na Twój host. Jeśli domena jest zarządzana przez nas, możesz to łatwo zrobić przez opcję [EasyDNS](domain-easydns.md).
...
Przy pracy z wieloma konfiguracjami Docker warto stworzyć przejrzystą strukturę katalogów, aby projekty były odseparowane. Popularnym rozwiązaniem jest utworzenie folderu *docker* w katalogu domowym użytkownika, z podfolderami dla każdej domeny. Dzięki temu na jednym serwerze można hostować wiele projektów bez konfliktów konfiguracji.
...
Dzięki temu masz dedykowane katalogi na konfigurację nginx, certyfikaty SSL, pliki include oraz zawartość specyficzną dla Odoo, jak konfiguracje i dodatki.
...
W katalogu projektu docker utwórz plik `compose.yml` za pomocą `nano compose.yml`. Wklej do niego poniższy kod:
...
Aby nginx i certbot działały poprawnie, musisz odblokować w zaporze sieciowej porty TCP 80 (HTTP) i 443 (HTTPS). Port 80 jest potrzebny certbotowi do weryfikacji HTTP, a port 443 do obsługi szyfrowanego ruchu HTTPS. Jeśli korzystasz z UFW (Uncomplicated Firewall), dodaj reguły poleceniami:
...
Sprawdź status reguł poleceniem `sudo ufw status`, aby potwierdzić, że porty są otwarte. Upewnij się, że inne konfiguracje zapory nie blokują tych portów, bo może to uniemożliwić generowanie certyfikatów lub bezpieczny ruch HTTPS.



### Konfiguracja Nginx

Konfigurację nginx zacznij od utworzenia pliku konfiguracyjnego dla swojej domeny. W katalogu `nginx/conf` stwórz plik o nazwie domeny. Wykonaj:

```
nano nginx/conf/example.com.conf
```

i wklej podstawową konfigurację, zamieniając `example.com` na swoją domenę:
...
Ta konfiguracja pozwala certbotowi przeprowadzić wyzwanie ACME i wygenerować ważne certyfikaty SSL. Dodatkowo przekierowuje cały ruch HTTP na HTTPS.
...
Uruchom poniższe polecenie, aby wygenerować certyfikaty SSL za pomocą certbota. Pamiętaj, aby podać swoją domenę po flagu `-d` oraz zastąpić `user@mail.com` swoim prawidłowym adresem e-mail.

```
sudo docker compose run --rm certbot certonly --webroot --webroot-path=/var/www/certbot -d example.com --email user@mail.com --agree-tos --no-eff-email
```

Następnie wygeneruj dedykowany plik parametrów DH, aby dodatkowo wzmocnić bezpieczeństwo wymiany kluczy kryptograficznych.

```
openssl dhparam -out nginx/ssl/dhparam.pem 2048
```

Teraz utwórz plik `ssl.conf` poleceniem `nano nginx/ssl/ssl.conf` i wklej do niego poniższą zawartość. Pamiętaj, aby zamienić `example.com` na swoją domenę:
...
Edytuj plik konfiguracyjny nginx, który utworzyłeś wcześniej, i zastąp jego zawartość poniższą konfiguracją, aby Twoja strona była dostępna wyłącznie przez HTTPS.
...
server {
    ...
    # Pozwala Certbotowi na dostęp do URL wyzwania
    location /.well-known/acme-challenge/ {
        root /var/www/certbot;
    }
...
Aby zastosować własne ustawienia, możesz utworzyć dedykowany plik konfiguracyjny dla Odoo. Umieść nowy plik pod `config/odoo.conf` i dodaj tam wybrane opcje.
...
Flaga `-i base` musi zostać usunięta z pliku `compose.yml`, bo inaczej przy każdym odtworzeniu kontenera Odoo baza danych będzie od nowa tworzona. Aby ją usunąć, otwórz plik compose poleceniem `nano compose.yml` i zmodyfikuj linię z komendą na:
...
Gratulacje! Udało Ci się zainstalować i skonfigurować Odoo na swoim VPS. Polecamy też zapoznać się z poniższymi materiałami, które mogą pomóc i wesprzeć Cię podczas konfiguracji serwera:
