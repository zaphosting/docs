...
W panelu głównym masz też opcję uruchomienia, zatrzymania lub restartu usługi, co daje pełną kontrolę nad jej działaniem. Ponadto możesz uzyskać dostęp do konsoli na żywo, by otrzymywać informacje w czasie rzeczywistym o stanie serwera i w razie potrzeby podjąć bezpośrednie działania.
...
:::warning
Korzystanie z Menedżera DDoS jest możliwe tylko wtedy, gdy masz własny adres IP lub wykupioną opcję "Przegląd Menedżera DDoS" jako dodatek. Jeśli tych elementów brakuje w Twoim pakiecie, możesz je dodać w każdej chwili poprzez aktualizację.
:::
...
:::warning
Nie rozumiesz znalezionych tam informacji lub nie pomagają? Spokojnie! W takim wypadku jesteśmy dostępni codziennie w supportcie. Po prostu załóż **[Ticket](https://zap-hosting.com/en/customer/support/)** i opisz swój problem. Sprawdzimy to jak najszybciej i pomożemy rozwiązać problem! :)
:::
...
Twój serwer był chwilowo offline i nie wiesz dlaczego? Sprawdzenie dziennika zdarzeń i plików logów nie dało odpowiedzi? Możliwe, że przyczyna problemu nie leży bezpośrednio po stronie Twojego serwera, lecz systemu hostującego, na którym działa.
...
W ustawieniach znajdziesz przegląd najczęściej używanych opcji konfiguracyjnych dla Twojego serwera. Znajduje się tam też krótki opis każdej opcji. Korzystanie z tej strony jest szczególnie polecane dla osób początkujących w administracji serwerem.
...
Dla tej gry oferujemy możliwość łatwej instalacji znanych i popularnych zasobów/pluginów za pomocą jednego kliknięcia w zakładce **Pluginy**. W przeglądzie znajdziesz wybraną zawartość, którą możesz dodać klikając przycisk instalacji. Po restarcie serwera pluginy zostaną zainstalowane, aktywowane i dostępne dla Ciebie.
...
Oprócz serwerów gier oferujemy też 10 wliczonych w cenę baz danych. Bazy te są potrzebne np. wtedy, gdy dane muszą być przechowywane w bazie. W zakładce **Bazy danych** możesz utworzyć bazę i zobaczyć dane dostępowe. Masz też opcję włączenia kopii zapasowej i importu istniejących backupów bazy danych.
