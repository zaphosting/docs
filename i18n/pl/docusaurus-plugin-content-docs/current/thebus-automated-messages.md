...
title: "The Bus: Zautomatyzowane wiadomości serwera – Informuj swoich graczy o nowościach, linkach społecznościowych i nie tylko!"
...
Dzięki **Zaplanowanym zadaniom** wiadomości serwera mogą być automatycznie wysyłane na czacie w grze **The Bus** poprzez wykonywanie zdefiniowanych komend według ustalonego harmonogramu.
...
Aby stworzyć zautomatyzowane wiadomości serwera, otwórz panel główny swojego serwera gier i przejdź do sekcji **Zaplanowane zadania**. Utwórz nowe zaplanowane zadanie i wybierz opcję wykonania **komendy**. W polu komendy wpisz komendę do wysyłania wiadomości obsługiwaną przez Twój serwer gier.

```
<komenda wiadomości> <tekst Twojej wiadomości>
```

Zamień komendę na komendę gry lub rozszerzenia, której chcesz użyć do wywoływania wiadomości, a `<tekst Twojej wiadomości>` na tekst, który mają zobaczyć gracze.
...
Funkcjonalność zautomatyzowanych wiadomości serwera zależy od możliwości gry i dostępnych komend. Podstawowe zaplanowane wiadomości można wdrożyć za pomocą standardowych komend serwera, jednak takie podejście może mieć ograniczenia w zakresie formatowania, warunków czy zaawansowanej personalizacji.
...
Zautomatyzowane wiadomości serwera to niezawodny sposób na utrzymanie graczy na bieżąco i zaangażowanych bez ręcznego wysiłku. Dzięki zaplanowanym zadaniom wykonującym komendy wiadomości, administratorzy serwera mogą zapewnić spójną komunikację na całym serwerze.
