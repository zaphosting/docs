...
Przed konfiguracją **Standard Notes** musisz przygotować swój system. Obejmuje to aktualizację systemu operacyjnego do najnowszej wersji oraz instalację wszystkich wymaganych zależności. Te przygotowania zapewnią stabilne środowisko i pomogą uniknąć problemów podczas lub po instalacji.
...
Po zakończeniu aktualizacji możesz przejść do instalacji zależności. Standard Notes będzie uruchamiany na Twojej maszynie za pomocą zestawu kontenerów Docker, więc najpierw musisz zainstalować Dockera. Wykonaj polecenia:
...
Następnie stwórz plik `.env` w katalogu roboczym i wypełnij go domyślnymi wartościami z przykładowej konfiguracji projektu:
...
Otwórz plik `.env` i upewnij się, że wszystkie klucze w sekcji KEYS są poprawnie ustawione. Wygeneruj losowe wartości dla każdej wymaganej zmiennej środowiskowej poleceniem:
...
Pobierz oficjalną przykładową konfigurację i zapisz ją jako `docker-compose.yml` w swoim folderze roboczym. Ten plik definiuje wszystkie potrzebne usługi kontenerowe dla Standard Notes.
...
## Podłączenie do serwera synchronizacji

Aby założyć konto w Standard Notes, kliknij ikonę avatara w prawym dolnym rogu aplikacji. W wyświetlonym menu wybierz „Utwórz konto”, aby rozpocząć tworzenie nowego profilu użytkownika. Podaj ważny adres e-mail oraz bezpieczne hasło.
...
Gratulacje! Udało Ci się zainstalować i skonfigurować Standard Notes na swoim serwerze dedykowanym. Polecamy też zapoznać się z poniższymi materiałami, które mogą pomóc i wesprzeć Cię podczas konfiguracji serwera:
