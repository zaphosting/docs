...
title: "VPS: Plex installeren op Windows"
description: "Ontdek hoe je je persoonlijke mediatheek moeiteloos beheert en streamt met Plex voor naadloze toegang op elk apparaat → Leer het nu"
...
Op de officiële Plex Media website vind je de actuele Windows-versie van de Plex Media Server om te downloaden. Gebruik de 64-bit variant voor compatibiliteit met alle moderne Windows-edities. Download hier: https://www.plex.tv/media-server-downloads/

![img](https://screensaver01.zap-hosting.com/index.php/s/d3b4mZsiQ4iqXrL/preview)

Na het downloaden kun je de installer starten om de Plex Media Server te installeren. Na installatie regel je de configuratie via de webinterface in je browser, waar je bibliotheken aanmaakt en je mediacollectie beheert.

![img](https://screensaver01.zap-hosting.com/index.php/s/5TnmMeRkdLAt2RJ/download)

Als de setup klaar is, opent de Plex Media Server webinterface automatisch in je standaardbrowser. Je wordt gevraagd in te loggen met een bestaand Plex-account of een nieuw account aan te maken als je er nog geen hebt.

Deze login is nodig om de server aan je persoonlijke account te koppelen, zodat functies als externe toegang, gebruikersbeheer en synchronisatie tussen apparaten mogelijk zijn. Na succesvolle authenticatie zit je in je eigen Plex Media omgeving. Hier kun je bibliotheken maken voor films, series, muziek of foto’s, automatisch metadata ophalen en content delen met andere gebruikers in je lokale netwerk of via internet.

![img](https://screensaver01.zap-hosting.com/index.php/s/HmQPZGsBqxqPHmy/download)

Na de setup kun je ook extern bij je Plex Media Server via de webinterface in een browser via `http://<ip-adres>:32400`. Vervang `<ip-adres>` door het publieke IP-adres van je server.
...
Gefeliciteerd! Je hebt Plex nu succesvol geïnstalleerd en geconfigureerd op je VPS. We raden je ook aan om de volgende bronnen te bekijken, die je extra hulp en tips kunnen geven tijdens het configureren van je server:
