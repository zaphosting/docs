...
Mods installeren op je server zorgt ervoor dat de serveromgeving de aangepaste content laadt, terwijl ze lokaal installeren ervoor zorgt dat je gameclient dezelfde bestanden gebruikt wanneer je verbinding maakt met gemodde servers.
...
```
Upload het gedownloade `.zip` mod-bestand naar deze map. Zodra de upload klaar is, pak je de inhoud van het archief direct uit in de `Paks`-map. Nadat de bestanden zijn uitgepakt, ga je terug naar je serverbeheer en start je de server opnieuw op.
