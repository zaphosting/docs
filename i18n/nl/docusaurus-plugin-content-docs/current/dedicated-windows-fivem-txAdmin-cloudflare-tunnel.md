...
Om een Cloudflare Tunnel op Windows te gebruiken heb je een Windows Dedicated Server nodig, een werkende txAdmin installatie, je txAdmin poort en een domein dat al gekoppeld is aan je Cloudflare account. Als je domein nog niet gekoppeld is aan Cloudflare, volg dan eerst onze [Cloudflare Setup](domain-cloudflare-setup.md) gids.
