...
description: "Ontdek hoe je je eigen FiveM Dedicated Server op een VPS installeert en configureert voor soepel multiplayer gamen → Leer het nu"
sidebar_label: FiveM
services:
  - vserver
---
...
Heb je een VPS en wil je je eigen FiveM Dedicated Server service met txAdmin installeren en draaien? Dan ben je hier aan het juiste adres! Hieronder leggen we alle stappen uit die je nodig hebt om het te installeren en configureren, en waar je op moet letten.


## Voorbereiding

Om een FiveM server op te zetten, zijn er een aantal voorbereidingen nodig die je moet doen voordat je echt kunt beginnen met het installeren van de FiveM Dedicated Server.



### Database opzetten

Wil je resources gebruiken en installeren die een database vereisen, dan heb je een extra **database server** nodig. Er zijn verschillende manieren om zo’n server op te zetten. In onze [MySQL Installatie](vserver-windows-installmysql.md) gids laten we je zien hoe je je eigen **database server** op je **VPS** installeert.
...
Tijdens de Recipe Deployer moet je de laatste benodigde gegevens invullen voordat de server echt kan starten. Vul je eerder aangemaakte FiveM licentiesleutel in het veld **License Key** in. Klik daarna op **Show/Hide Database Options (Advanced)** en vul de inloggegevens van je database server in. In dit geval is alleen het eerder ingestelde wachtwoord voor de **root** gebruiker nodig. Alle andere instellingen kunnen ongewijzigd blijven. Controleer je invoer en klik vervolgens op **Run Recipe** om het proces te starten.
...
Deze commando’s maken automatisch firewallregels aan die nodig zijn om je FiveM server publiek toegankelijk te maken.
...
Je hebt nu succesvol een FiveM Dedicated Server service geïnstalleerd en geconfigureerd op je VPS. Voor verdere vragen of hulp kun je altijd contact opnemen met onze support, die dagelijks voor je klaarstaat! 🙂
