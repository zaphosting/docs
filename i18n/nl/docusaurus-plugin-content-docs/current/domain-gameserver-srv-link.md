...
## Introductie
Je kunt je domein helemaal doorsturen of alleen een subdomein naar je game server. Dit is handig omdat mensen dan via een domein verbinding kunnen maken met je game server, in plaats van ingewikkelde IP-adressen met alleen cijfers te moeten onthouden. In deze gids leggen we uit hoe je een domein instelt om door te sturen naar je game server via DNS-records.
...
Voor het doorsturen van het IP-adres maak je óf een subdomein aan dat doorstuurt naar het IP-adres van de game server, óf je stuurt het hele domein door zonder subdomein. Dit is voldoende als de game server de standaardpoort van het spel gebruikt. Als de game server een andere poort gebruikt dan de standaardpoort, heb je een extra **SRV-record** nodig om het subdomein door te sturen naar de game server.
...
Als protocol geef je het internetprotocol op dat gebruikt wordt voor de verbinding. Hier zijn **UDP** en **TCP** beschikbaar. Welk protocol gebruikt wordt, hangt ook af van de game. De specificatie van het protocol begint ook altijd met een **underscore** en is óf **_udp** óf **_tcp**.

## Voorbereiding

Om je domein door te sturen naar een game server zonder een subdomein aan te maken, open je eerst je domein in het dashboard en ga je naar het DNS-beheer in het menu aan de linkerkant.
...
Eerst moet je het IP en de poort van de Minecraft server weten waar je naartoe wilt doorsturen. Met deze info volg je de volgende stappen.
...
Als je dit opslaat, wordt het record zo weergegeven:

![](https://screensaver01.zap-hosting.com/index.php/s/eFLm8oqbo4cLtn8/preview)

### SRV Record aanmaken

Nu moet je een SRV-record aanmaken, dit gebruik je voor het doorsturen van de poort als je niet de standaardpoort hebt.
Klik daarvoor weer op **Nieuw record**.

Bij **Naam** vul je het protocol en het verbindingsdomein in. De domeinnaam die je hier invult, gebruik je later om verbinding te maken met de server. Als **Type** kies je in het dropdownmenu **SRV**:

![](https://screensaver01.zap-hosting.com/index.php/s/pH9F5kZins8wHn4/preview)

Bij **Waarde** vul je drie waarden in: eerst de prioriteit (standaard 0), dan de poort van de game server (in ons voorbeeld `25500`) en als laatste de domeinnaam van het zojuist aangemaakte **A-record**.
...
:::important
Aan het einde van de invoer moet een punt staan, zonder die punt probeert het systeem het domein erachter te plakken.
:::
...
Eerst moet je het IP en de poort van de FiveM server weten waar je naartoe wilt doorsturen. Met deze gegevens volg je de volgende stappen.
...
Bij **Waarde** vul je drie waarden in: eerst de prioriteit (standaard 0), dan de poort van de game server (in ons voorbeeld `30300`) en als laatste de domeinnaam van het zojuist aangemaakte **A-record**.
