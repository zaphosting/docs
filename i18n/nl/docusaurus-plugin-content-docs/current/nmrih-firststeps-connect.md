...
## Introductie
Weet je niet zeker hoe je verbinding maakt met je **No More Room in Hell**-server of wat je nodig hebt om te beginnen? Geen stress, wij fixen het voor je! We nemen je stap voor stap mee – van de benodigde tools en info tot het daadwerkelijke verbindingsproces, inclusief belangrijke tips voor een soepele en probleemloze connectie. Volg onze gids en je bent zo verbonden!
...
<TabItem value="connect_solution_server_browser_steam" label="Serverbrowser (Steam)" default>

Om via de Steam-serverbrowser verbinding te maken met je server, open je de serverbrowser vanuit Steam. Klik bovenin de Steam-navigatie op `Beeld` en daarna op de sectie `Game Server` om deze te openen.

Zoek je server of voeg ‘m toe via de Steam-browser met het `IP-adres` en de `poort` van je gameserver. Selecteer hiervoor het tabblad `Favorieten` in de serverlijst en klik onderaan op het plusje. Er opent een pop-up waarin je je servergegevens invult.
