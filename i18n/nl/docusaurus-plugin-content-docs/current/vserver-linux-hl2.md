...
description: "Ontdek hoe je een Half-Life 2 dedicated server op je Linux VPS installeert voor een soepele game-ervaring → Leer het nu"
...
Heb even geduld terwijl de download loopt, dit kan even duren bij grotere games. Als het gelukt is, zie je een succesbericht verschijnen.
...
Om andere spelers op je server te laten joinen, moet je een Game Server Login Token (GSLT) genereren en toevoegen. Deze token authenticatieert je server bij Steam. Maak een GSLT aan via http://steamcommunity.com/dev/managegameservers met game ID 232370, dat is Half-Life 2.

Zodra je de token hebt, voeg je deze toe aan je server launch parameters met `+sv_setsteamaccount <TOKEN>`.

## Server starten & verbinden

Tijd om je server te starten. Ga naar de hoofd game directory en voer het volgende commando uit:
```
