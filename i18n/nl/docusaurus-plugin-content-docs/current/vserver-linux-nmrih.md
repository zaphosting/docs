...
description: "Ontdek hoe je een No More Room In Hell dedicated server installeert op je Linux VPS voor soepel gamehosting → Leer het nu"
...
Wees geduldig terwijl de download voltooid wordt, dit kan even duren bij grotere games. Als het gelukt is, verschijnt er een succesmelding.

## Configuratie

Op dit punt is de setup van je No More Room In Hell server afgerond. Je kunt de server verder configureren door het launch-bestand aan te passen.
...
Om andere spelers op je server te laten joinen, moet je een Game Server Login Token (GSLT) genereren en toevoegen. Deze token authenticatieert je server bij Steam. Ga naar http://steamcommunity.com/dev/managegameservers en maak een token aan met game ID 317670, dat hoort bij No More Room In Hell.
