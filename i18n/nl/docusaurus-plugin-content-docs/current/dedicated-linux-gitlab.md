...
title: "Dedicated Server: GitLab installeren op Linux"
description: "Ontdek hoe je GitLab op Linux installeert om DevOps workflows te stroomlijnen en team samenwerking te verbeteren → Leer het nu"
...
GitLab is een complete DevOps-platform waarmee teams samen aan code kunnen werken, workflows automatiseren en de volledige softwareontwikkelingscyclus efficiënt beheren. In deze handleiding leggen we uit hoe je GitLab installeert op een Linux-server.
...
| Overige          | Redis 7.x+, Sidekiq 7.3.2+, Prometheus 2.54.1+               |

:::info
Voor de meest accurate en actuele specificaties, raadpleeg de officiële [Hardware Requirements](https://docs.gitlab.com/ee/install/requirements.html) documentatie van GitLab.
:::

## Voorbereiding

Je moet verbinding maken via een SSH-client om GitLab op je Linux-server te installeren. Bekijk onze [Eerste toegang (SSH)](vserver-linux-ssh.md) gids voor meer info hierover.
...
Update de pakketlijst naar de nieuwste versie en installeer het OpenSSH Server pakket samen met de benodigde prerequisites met het volgende commando. Hiermee wordt je webinterface voor GitLab gehost.
...
Als je e-mailnotificaties wilt gebruiken, installeer dan Postfix met:
...
Installeer het OpenSSH Server pakket samen met de benodigde prerequisites met het volgende commando. Hiermee wordt je webinterface voor GitLab gehost.

```
sudo zypper install curl openssh perl
```

Zorg er vervolgens voor dat de OpenSSH daemon is ingeschakeld met de volgende commando’s:
...
Zorg dat de firewall de benodigde toegang toestaat, als je `firewalld` gebruikt.
...
Als dat zo is, open dan de vereiste poorten (standaard 80 en 443):
...
:::info
Tijdens het installeren van Postfix kan een configuratiescherm verschijnen. Kies in dat geval 'Internet Site' en druk op enter. Gebruik de externe DNS van je Linux-server als 'mail name' en druk op enter. Als er meer schermen verschijnen, druk dan steeds op enter om de standaardinstellingen te accepteren.

Wil je een andere oplossing gebruiken voor e-mails, sla deze stap dan over en [configureer een externe SMTP-server](https://docs.gitlab.com/omnibus/settings/smtp) nadat GitLab is geïnstalleerd, volgens de officiële GitLab handleiding.
:::
...
Om alles werkend te krijgen, moet je een aantal aanpassingen doen in het configuratiebestand. Open het GitLab configuratiebestand met je favoriete teksteditor. We gebruiken hier `nano` als voorbeeld.
...
Daarnaast raden we aan je e-mailadres in te vullen bij `letsencrypt['contact_emails']`. Zo kan Let's Encrypt je notificaties sturen en contact opnemen over de automatische en gratis SSL-certificaten.

:::note
Je hebt een domein nodig om een gratis SSL-certificaat van Let's Encrypt te krijgen. Je kunt er geen aanvragen voor een IP-adres.
:::
...
# letsencrypt['auto_renew_minute'] = nil # Moet een nummer of cron-expressie zijn, indien gespecificeerd.
...
Bij de eerste keer inloggen verschijnt de loginpagina.

![](https://screensaver01.zap-hosting.com/index.php/s/E46E3qS32GKFfk3/preview)

Voor admin toegang log je in als root gebruiker met gebruikersnaam `root`.
...
Voer de gebruikersnaam en het wachtwoord in op de loginpagina om voor het eerst in te loggen in je GitLab dashboard. Je hebt nu toegang tot je eigen GitLab panel op je Linux-server.
...
Gefeliciteerd, je hebt GitLab succesvol geïnstalleerd! Heb je nog vragen of problemen? Neem gerust contact op met onze support, we helpen je elke dag graag verder!
