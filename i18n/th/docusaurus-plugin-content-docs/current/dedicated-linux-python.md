...
ลินุกซ์ดิสโทรส่วนใหญ่จะมาพร้อม Python ติดตั้งไว้แล้ว แต่เวอร์ชันอาจจะไม่อัปเดต หรือระบบอาจติดตั้งมาโดยไม่มีแพ็กเกจบางตัว คุณสามารถเช็คว่ามี Python ติดตั้งอยู่หรือไม่ (`python3 --version`) และรันคำสั่งต่อไปนี้เพื่ออัปเดตหรือทำการติดตั้ง runtime
...
การรันคำสั่ง `python3` จะเปิด Python interpreter ขึ้นมา คุณสามารถเขียนโค้ด Python ที่ถูกต้องหลัง `>>>` แล้วกด `Enter` เพื่อรันโค้ดนั้น เมื่อเสร็จแล้วสามารถปิด interpreter ได้โดยพิมพ์ `exit()` ในคอนโซล
