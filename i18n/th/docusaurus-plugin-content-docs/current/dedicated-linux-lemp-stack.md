...
ในคู่มือนี้ เราจะใช้ Ubuntu เป็นดิสโทร Linux คำสั่งจะเหมือนกันสำหรับ Debian และน่าจะคล้ายกันสำหรับดิสโทรอื่น ๆ แต่ไวยากรณ์คำสั่งอาจแตกต่างกันเล็กน้อย ตรวจสอบให้แน่ใจว่าคุณติดตั้งระบบปฏิบัติการแล้วและเชื่อมต่อกับเซิร์ฟเวอร์ผ่าน SSH
...
การติดตั้งสามารถแบ่งออกเป็นแต่ละส่วนประกอบหลักของ LEMP เริ่มจากเซิร์ฟเวอร์เว็บ Nginx ตามด้วยฐานข้อมูล MySQL และสุดท้าย PHP ตลอดการติดตั้ง เราจะตั้งค่าเว็บไซต์ทดสอบที่เขียนด้วย PHP และเข้าถึงฐานข้อมูล MySQL สุดท้ายคำขอเว็บแต่ละรายการจะถูกประมวลผลและให้บริการผ่านเซิร์ฟเวอร์เว็บ Nginx
...
:::tip
คุณสามารถดูโปรไฟล์ที่มีได้โดยรันคำสั่ง `ufw app list` ในตัวอย่างนี้ การใช้ `Nginx Full` หมายความว่าจะสร้างกฎทั้ง HTTP (พอร์ต 80) และ HTTPS (พอร์ต 443)
:::

คุณควรเห็นกฎ `Nginx` และ `Nginx (v6)` พร้อมสถานะ `ALLOW` ซึ่งยืนยันว่าไฟร์วอลล์พร้อมใช้งานแล้ว คุณจะเห็นกฎอื่น ๆ ที่ตั้งไว้ก่อนหน้านี้ รวมถึงกฎ SSH ด้วย
...
หลังจากเสร็จแล้ว แนะนำให้รันสคริปต์ติดตั้งแบบปลอดภัยเพื่อให้เซิร์ฟเวอร์ MySQL ของคุณปลอดภัยขึ้น ซึ่งเป็นตัวเลือกแต่แนะนำอย่างยิ่ง รันด้วยคำสั่ง `sudo mysql_secure_installation`
...
:::tip ส่วนขยาย PHP
สำหรับการใช้งานขั้นสูง คุณอาจต้องการส่วนขยาย PHP เพิ่มเติมเพื่อฟังก์ชันเสริม ดูรายการได้โดยรันคำสั่ง `apt search php- | less`
...
เมื่อติดตั้งส่วนประกอบ LEMP ครบแล้ว เราจะสร้างเว็บไซต์ทดสอบเพื่อแสดงว่า LEMP stack ทำงานร่วมกันอย่างไรเพื่อสร้างโซลูชันเว็บไซต์ไดนามิกที่ดี นี่เป็นตัวเลือกแต่ช่วยให้เข้าใจวิธีใช้เครื่องมือเหล่านี้ตั้งค่าเว็บไซต์ของคุณเอง
...
เราจะใช้โดเมนทดสอบ `zapdocs.example.com` ตลอดคู่มือ เพราะในโลกจริงคุณน่าจะใช้โดเมน คุณ **ต้อง** ตั้งค่าเรคคอร์ด DNS ประเภท `A` สำหรับโดเมนที่ชี้ไปยังที่อยู่ IP ของเซิร์ฟเวอร์ ถ้าต้องการความช่วยเหลือ ดูที่ [คู่มือระเบียนโดเมน](domain-records.md)
...
โดยทั่วไปบนเซิร์ฟเวอร์เว็บ ไฟล์และข้อมูลเว็บไซต์จะเก็บไว้ในไดเรกทอรี `/var/www` โดยปกติ Nginx จะมาพร้อมไดเรกทอรี `html` ที่มีหน้าเริ่มต้น เพื่อให้ทุกอย่างเป็นระเบียบโดยเฉพาะเมื่อโฮสต์หลายเว็บไซต์บน Nginx เดียวกัน เราแนะนำให้ตั้งแต่ละเว็บไซต์ในโฟลเดอร์แยกต่างหาก
...
จากนั้นสร้างไฟล์คอนฟิก server block ใหม่ในไดเรกทอรี `sites-available` สำหรับโดเมนและโฟลเดอร์นี้
```
...
สำคัญมากที่ต้องเปลี่ยน `[your_phpversion]` เป็นเวอร์ชัน PHP ที่ติดตั้งอยู่ ตรวจสอบได้โดยรัน `php -v` ซึ่งจะแสดงเวอร์ชัน เช่น: `PHP 8.3.6 (cli) (built: Mar 19 2025 10:08:38) (NTS)`

ในตัวอย่างนี้ ให้ใส่ `8.3` เป็นเวอร์ชันหลัก เช่นบรรทัดนี้จะเป็น: `fastcgi_pass unix:/var/run/php/php8.3-fpm.sock;`
:::

ไฟล์ server block ใหม่นี้จัดการคำขอพอร์ต 80 (HTTP) และตรวจสอบว่าคำขอตรงกับ `server_name` ที่ระบุ คือโดเมนของคุณ นอกจากนี้ยังชี้ไปที่โฟลเดอร์ `/var/www/[your_domain]` ที่สร้างไว้เพื่อให้บริการไฟล์
...
ตอนนี้คุณตั้งค่า Nginx ผ่าน server block และโฟลเดอร์เอกสารแล้ว ถึงเวลาสร้างเว็บไซต์จริง ๆ โฟลเดอร์ยังว่างอยู่ ดังนั้นจะไม่มีอะไรให้บริการ เราจะสร้างเว็บไซต์รายการสิ่งที่ต้องทำเล็ก ๆ ตามที่กล่าวไว้สำหรับโดเมนนี้

##### เตรียมฐานข้อมูล

เริ่มต้นด้วยการสร้างฐานข้อมูลและตารางเพื่อเก็บรายการแต่ละรายการ เข้าสู่ระบบ MySQL
```
sudo mysql -u root
```
...
เมื่อฐานข้อมูลพร้อมและผู้ใช้ตั้งค่าแล้ว ออกจาก MySQL ด้วยคำสั่ง `quit`

##### ไฟล์เว็บไซต์ PHP

ส่วนสุดท้ายของตัวอย่างนี้คือการตั้งค่าไฟล์เว็บไซต์ PHP สำหรับหน้า to-do จะสร้างไฟล์ `index.php` ใหม่ในไดเรกทอรี `/var/www/[your_domain]` ที่สร้างไว้ก่อนหน้า เปิด nano editor เพื่อสร้างไฟล์นี้
```
sudo nano /var/www/[your_domain]/index.php
```

ด้านล่างนี้เป็นโค้ดตัวอย่างง่าย ๆ ที่คุณสามารถวางลงใน nano editor สำหรับหน้า to-do พื้นฐานที่ดึงรายการจากฐานข้อมูล ส่วน PHP แรกสร้างการเชื่อมต่อ MySQL
...
คุณตั้งค่าเว็บไซต์ทดสอบรายการสิ่งที่ต้องทำที่ใช้ทุกส่วนของ LEMP stack สำเร็จแล้ว!
