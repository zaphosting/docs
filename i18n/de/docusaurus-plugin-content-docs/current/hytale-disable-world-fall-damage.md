...
Um den Fallschaden zu deaktivieren, öffne die `config.json` der Welt, die du unter `universe/worlds/<world_name>/config.json` findest. Suche die Einstellung, die den Fallschaden steuert, meist als Boolean-Flag wie `IsFallDamageEnabled` dargestellt, und setze sie auf `false`:
