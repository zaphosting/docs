...
Du willst deinen Voicebot mit deinem TeamSpeak-Server verbinden, hast aber keine Ahnung, wie das geht? Kein Stress, wir erklären dir jetzt Schritt für Schritt, wie das funktioniert!
...
Jetzt kannst du unter **Server** einen TeamSpeak auswählen, wenn dieser auch bei deinem Account gebucht wurde. Falls der Server extern gehostet wird, kannst du dort auch **Custom Teamspeak Server** auswählen.
...
Da dein Discord-Server keine direkte IP-Adresse oder Port hat, sind ein paar Schritte nötig, bevor du deinen Bot auf einem Discord-Server nutzen kannst.


### Erstellen einer Discord-Anwendung

Als erstes musst du eine Discord-Anwendung erstellen. Deinem Bot wird diese zugewiesen und er agiert unter deren Namen. Um so eine Anwendung zu erstellen, geh bitte auf [das Discord Developer Portal](https://discord.com/developers/applications/).
...
Dann erstellst du die Anwendung mit **Create**. Für die nächsten Schritte notiere dir bitte die Application ID:
...
Beim Erstellen der Anwendung wird automatisch ein Bot angelegt. Den Token bekommst du ganz easy, indem du auf **Reset Token** klickst. Dann erhältst du einen User-Token, den du dir unbedingt notieren solltest:
...
Um den Bot einzuladen, musst du die zuvor notierte Client ID an der passenden Stelle in den folgenden Link einfügen, dann öffnest du den Link im Browser und fügst den Bot zu deinem Discord-Server hinzu. So sieht das Beispiel aus:
```
