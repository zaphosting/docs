...
Bevor du Coolify installierst, verbinde dich mit deinem Server und stelle sicher, dass das Betriebssystem aktuell ist. Du musst dich per SSH mit Root-Zugriff verbinden. Falls du Hilfe brauchst, haben wir eine Anleitung zum [SSH-Erstzugang](vserver-linux-ssh.md).
...
Gehe ins DNS-Management deines Domain-Anbieters und füge zwei neue `A`-Einträge hinzu, die auf deinen Server zeigen. Wenn deine Domain bei ZAP-Hosting registriert ist, hilft dir unsere [Domain Records](domain-records.md) Anleitung beim Einrichten:
