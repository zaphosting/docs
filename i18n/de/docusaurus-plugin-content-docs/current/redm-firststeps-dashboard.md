...
title: "RedM: Dashboard Übersicht"
description: "Entdecke, wie du deinen RedM Gameserver effizient verwaltest und optimierst mit einer klaren Übersicht der wichtigsten Admin-Funktionen → Jetzt mehr erfahren"
...
Das **Dashboard** deines Servers ist das Herzstück der Administration. Hier werden alle grundlegenden und wichtigen Informationen zu deinem Service übersichtlich dargestellt. Dazu gehören zum Beispiel Infos zum Serverstandort, Status, IP: Port, genutztem Speicher, laufendem Spiel, Servername, Map und die Anzahl der verfügbaren Slots. Außerdem wird die Auslastung von CPU, Speicher und RAM angezeigt.
...
Mit Tebex hast du die Möglichkeit, deinen Gameserver zu monetarisieren, indem du virtuelle Güter und Ränge gegen echtes Geld verkaufst. Es ist ein mächtiges und zuverlässiges Tool, das speziell für Gameserver-Admins entwickelt wurde, die ihren Spielern ein besseres Spielerlebnis bieten wollen.
...
Die **Live-Konsole** bietet dir eine Echtzeitansicht aller aktuellen Ereignisse auf deinem Server. Hier siehst du Infos zu den Aktivitäten der Spieler sowie mögliche Fehlermeldungen und Systemnachrichten. Diese Übersicht ermöglicht dir, einen umfassenden Blick darauf zu bekommen, was auf deinem Server passiert, und Probleme frühzeitig zu erkennen.
...
In diesem Bereich findest du eine Übersicht der Unterbrechungshistorie des Hostsystems, auf dem dein Gameserver-Service läuft. Mehr Infos und die aktuellste Version unseres Service-Status findest du auf unserer neuen [Statusseite](https://status.zap-hosting.com/).
...
Unter den Einstellungen findest du eine Übersicht der gängigsten Konfigurationsoptionen für deinen Server. Diese sind auch mit einer kurzen Erklärung versehen. Die Nutzung der Einstellungsseite empfehlen wir besonders Einsteigern in die Serveradministration.
...
- Grundeinstellungen (Servername, Projektname, Projektbeschreibung und mehr)
- EasyAdmin (Sprache, Menü-Button, Discord Webhook, Report- und Admin-Befehle)
- RedM Einstellungen (Koordinatenanzeige, PVP)
- Ladebildschirm (YouTube Video ID)
...
Für dieses Spiel bieten wir die Möglichkeit, bekannte und beliebte Ressourcen/Plugins mit nur einem Klick unter **Ressourcen** einfach zu installieren. In der Übersicht findest du eine Auswahl des verfügbaren Contents, den du per Klick auf den Installationsbutton hinzufügen kannst. Nach einem Neustart des Servers werden diese Ressourcen installiert, aktiviert und stehen dir zur Verfügung.

![](https://screensaver01.zap-hosting.com/index.php/s/bP48gnjNPdSkA9q/preview)

Mit dieser Funktion kannst du den Inhalt deines Gameservers schnell und einfach erweitern, ohne selbst Anpassungen vornehmen zu müssen. Das spart Zeit und reduziert das Risiko von Fehlern oder Kompatibilitätsproblemen.
...
Mit dem **FTP-Browser** bekommst du alle nötigen Infos, um eine Verbindung zum FTP-Zugang herzustellen. Der FTP-Zugang ermöglicht dir direkten Zugriff auf die Dateien deines Gameservers. Für die optimale Nutzung empfehlen wir ein externes FTP-Programm wie **FileZilla** oder **WinSCP**. Wenn du noch nicht vertraut mit FTP bist, kann dir ein Blick in die [Zugriff via FTP](gameserver-ftpaccess.md) Anleitung helfen.
