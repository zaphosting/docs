...
title: "HumanitZ: Automatisierte Server-Nachrichten – Halte deine Spieler über News, Social Links und mehr auf dem Laufenden!"
...
Mit **Geplanten Tasks** werden Server-Nachrichten automatisch im **HumanitZ** Ingame-Chat verschickt, indem vordefinierte Befehle nach einem festgelegten Zeitplan ausgeführt werden.
...
Ist der Task einmal eingerichtet, läuft er automatisch im Hintergrund und zeigt die Nachricht allen verbundenen Spielern an. So ist eine konstante Kommunikation sichergestellt, ohne dass dein Team ständig online sein muss.
...
Ersetze den Befehl durch den Game-Befehl oder den Befehl einer Erweiterung, mit dem du Nachrichten auslösen möchtest, und `<dein Nachrichtentext>` durch die Nachricht, die deine Spieler sehen sollen.

Anschließend kannst du festlegen, wie oft die Nachricht gesendet werden soll – z. B. alle paar Minuten, stündlich oder zu bestimmten Tageszeiten. Für detaillierte Anleitungen zum Erstellen geplanter Tasks schau dir die [Geplante Tasks](gameserver-scheduled-tasks.md) Anleitung an.
...
Automatisierte Server-Nachrichten sind ein zuverlässiger Weg, um deine Spieler ohne manuellen Aufwand informiert und engagiert zu halten. Durch geplante Tasks, die Nachrichtenbefehle ausführen, stellst du eine konstante Kommunikation auf deinem Server sicher.

Ist alles eingerichtet, laufen die automatisierten Nachrichten eigenständig und können einfach wiederverwendet werden, indem du nur den spiel-spezifischen Nachrichtenbefehl und Text anpasst.
