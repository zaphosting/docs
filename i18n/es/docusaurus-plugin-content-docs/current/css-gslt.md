---
id: css-gslt
title: "Counter-Strike: Source: Crear un GSLT (Steam Game Server Login Token)"
description: "Entiende cómo autenticar servidores de juegos con el GSLT de Steam para listado público y protección VAC → Aprende más ahora"
...
El **GSLT (Game Server Login Token)** es un token de acceso requerido por Steam/Valve para ejecutar servidores dedicados de ciertos juegos. Está vinculado a una cuenta de Steam y asegura que el servidor esté correctamente autenticado.
...
Una vez dentro, tienes la opción de crear un nuevo token usando el formulario que aparece en la página. En el campo **App ID**, introduce el ID de la aplicación `220`, que corresponde a **Counter-Strike: Source**. Asegúrate de ingresar el ID correcto, ya que el token no funcionará con un valor incorrecto.
...
:::info Operar múltiples servidores de Counter-Strike: Source
Si operas varios servidores de **Counter-Strike: Source**, necesitarás un GSLT para cada servidor. <u>**No**</u> es posible operar varios servidores de juegos con un solo GSLT.
:::
