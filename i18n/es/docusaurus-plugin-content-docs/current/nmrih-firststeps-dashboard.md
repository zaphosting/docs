...
title: "No More Room In Hell: Vista general del panel"
description: "Descubre cómo gestionar eficientemente tu servidor de No More Room In Hell con una vista clara de funciones y opciones → Aprende más ahora"
sidebar_label: Panel
services:
  - gameserver-nmrih
---
...
¿Tu servidor estuvo offline temporalmente y no sabes por qué? ¿Revisaste el registro de eventos y los archivos de registro y no encontraste nada? Puede que la causa no esté directamente en tu servidor, sino en el sistema host donde se ejecuta.
...
En esta sección encontrarás un historial de interrupciones del sistema host donde corre tu servidor de juegos. Más info y el estado actualizado de nuestros servicios lo tienes en nuestra nueva [página de estado](https://status.zap-hosting.com/).
...
:::warning
¿Has notado que algunos cambios se sobrescriben parcialmente? Ten en cuenta que ciertos valores se toman de la página de configuraciones. Si quieres cambiar esos valores directamente en el config, primero debes desactivar las opciones correspondientes en la página de configuraciones.
:::

### Juegos

En **Juegos** siempre puedes cambiar la variante del juego actual o instalar otro juego en general. Los juegos con el mismo precio por slot se pueden seleccionar directamente en **Juegos disponibles**. Para otros juegos, primero debes ajustar el precio por slot. Más info en la guía de [Cambio de juego](gameserver-gameswitch.md).
...
Para este juego ofrecemos la posibilidad de instalar plugins populares y conocidos con un solo clic en **Plugins**. En la vista general encontrarás una selección del contenido disponible que puedes añadir pulsando el botón de instalación. Tras reiniciar el servidor, estos recursos/plugins se instalarán, activarán y estarán listos para ti.
...
Además de nuestros servidores de juegos, ofrecemos 10 bases de datos incluidas. Estas bases de datos son necesarias, por ejemplo, cuando se deben almacenar datos en una base. En **Bases de datos** puedes crear una base y ver las credenciales de acceso. También puedes activar la opción de copia de seguridad e importar backups existentes.
...
Por defecto, ofrecemos 10 GB de espacio en el servidor de almacenamiento para backups en nuestros productos de servidores de juegos. Si necesitas más espacio, puedes ampliarlo pagando un extra. Así te aseguras de tener siempre espacio suficiente para tus backups y acceso a copias antiguas si las necesitas.

## Enlaces adicionales

En la sección de **Enlaces adicionales** encontrarás más links relevantes para tu producto.
