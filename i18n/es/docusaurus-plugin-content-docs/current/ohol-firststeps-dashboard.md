...
title: "One Hour One Life: Resumen del panel"
description: "Descubre cómo gestionar y optimizar eficientemente tu servidor de One Hour One Life para el mejor rendimiento y experiencia de usuario → Aprende más ahora"
sidebar_label: Panel
services:
  - gameserver-ohol
---
...
En la sección **Administración del producto** encontrarás todas las páginas necesarias para gestionar tu servidor. Las opciones para administrar tu servidor se detallan a continuación. 
...
:::warning
El uso del Gestor de DDoS solo es posible si tienes una IP propia o la opción adicional "Resumen Gestor de DDoS" contratada. Si estos componentes faltan en tu paquete actual, puedes añadirlos en cualquier momento actualizando.
:::
...
¿Tu servidor estuvo offline temporalmente y no sabes por qué? ¿Miraste el registro de eventos y los archivos de registro y no encontraste nada? Puede que la causa del problema no esté directamente en tu servidor, sino en el sistema host donde se ejecuta tu servidor.
...
En esta sección encontrarás un historial de interrupciones del sistema host donde corre tu servicio de servidor de juegos. Más información y la versión más actualizada de nuestro estado de servicio la tienes en nuestra nueva [página de estado](https://status.zap-hosting.com/).
...
Además, puedes activar la función de notificación por correo para actualizaciones automáticas y así estar siempre informado y actualizado.



### Tareas programadas

Crea tareas programadas que se ejecutan automáticamente en horarios definidos para automatizar varios procesos en tu servidor. Puedes configurar si una tarea se ejecuta una vez o se repite según sea necesario. Los tipos de tareas soportados incluyen iniciar, detener o reiniciar servicios, reiniciar el servidor si está online, crear copias de seguridad o ejecutar comandos personalizados.
...
**FileZilla:** FileZilla es probablemente el cliente FTP más sencillo y conocido. Solo instálalo, ábrelo y en la barra superior introduce tus datos de acceso que encuentras un poco más arriba en esta página. Tras un clic rápido en "Conectar" estarás directamente conectado al servidor y podrás gestionar los archivos.
...
En la configuración puedes especificar si las copias se crean automáticamente y si esto debe hacerse diariamente o semanalmente. Además, puedes decidir si se deben eliminar las copias más antiguas si es necesario, por ejemplo, cuando el espacio de almacenamiento disponible se vuelve limitado.
...
La sección **Preguntas frecuentes** ofrece un resumen de las preguntas más comunes de nuestros clientes. Aquí encontrarás respuestas útiles e información detallada sobre varios temas para ayudarte rápida y eficientemente.
...
### Noticias
La sección **Noticias** te ofrece una visión completa de todos los anuncios y novedades relacionadas con tu servicio. Aquí encontrarás información importante sobre varios aspectos como:
