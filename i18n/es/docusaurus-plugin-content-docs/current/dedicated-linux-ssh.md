...
| Cliente SSH | Sistemas operativos soportados | Código abierto |                           Descargar                           |
| :---------: | :-----------------------------: | :------------: | :----------------------------------------------------------: |
...
A continuación se explica la conexión usando el cliente SSH Putty. En la ventana de configuración, se introduce la dirección IP del servidor y el puerto SSH 22 para el **nombre de host**. Después, se puede iniciar la conexión con el botón **Open**.
...
:::info
Al conectarte al servidor por primera vez, Putty mostrará un mensaje de seguridad informando que no hay una clave de host guardada en el registro. Puedes confirmar con **Yes** si no quieres que este mensaje aparezca cada vez.
:::
...
| :------: | :--------------------------------------: | :----------------------------: |
| useradd  |          Crear un usuario nuevo           | useradd [opciones] [usuario]  |
...
| Comando |                         Descripción                         | Sintaxis                                       |
| :-----: | :---------------------------------------------------------: | ---------------------------------------------- |
|   top   | Vista general de carga, procesos y otra info (similar al Administrador de tareas de Windows) | top                                          |
|   df    |            Mostrar uso de almacenamiento (particiones)            | df -h                                        |
|   du    |          Mostrar consumo de almacenamiento (directorios)           | du -sh *                                     |
|  free   | Uso de memoria del sistema. Dividido en RAM y espacio SWAP. | free                                         |
