...
## Introducción
¿Tienes un VPS o servidor dedicado Windows y quieres instalar el servicio de servidor dedicado de ARK: Survival Ascended? Estás en el lugar correcto. En esta guía, te explicaremos paso a paso cómo instalar este servicio en tu servidor.
...
Estos comandos crearán automáticamente las reglas de firewall necesarias para que tu servidor Ark: Survival Ascended sea accesible públicamente.

</TabItem>

<TabItem value="windefender" label="Vía Windows Defender">

Usa la función de búsqueda de Windows para abrir **Configuración de Firewall de Windows con Seguridad Avanzada**. Puede que tengas que pulsar en **Configuración avanzada** para abrir la ventana necesaria si abres la página base del Firewall de Windows.

![](https://screensaver01.zap-hosting.com/index.php/s/ZT2WzELZSBPrFsB/preview)

Debes crear nuevas reglas para tu servidor Ark: Survival Ascended. Para ello, haz clic en las reglas entrantes y salientes como se describe a continuación y añádelas para los siguientes protocolos y puertos:
...
Para este punto, ya has terminado la configuración de tu servidor dedicado Ark: Survival Ascended. Puedes realizar configuraciones adicionales del servidor a través de dos archivos de configuración que se encuentran dentro del directorio de tu servidor.
...
Desde el 18 de noviembre de 2023, los propietarios de servidores deben instalar manualmente la lista de revocación de certificados que se puede obtener en https://dev.epicgames.com/ (o directamente desde http://crl.r2m02.amazontrust.com/r2m02.crl). El archivo r2m02.crl descargado debe instalarse (clic derecho sobre el archivo) seleccionando Colocar todos los certificados en la siguiente tienda y eligiendo Autoridades de certificación raíz de confianza. Puede ser necesario reiniciar el sistema.
...
:::info
Debes instalar [Microsoft Visual C++ 2015 Redistributable](https://learn.microsoft.com/en-us/cpp/windows/latest-supported-vc-redist?view=msvc-170#visual-studio-2015-2017-2019-and-2022) en tu VPS Windows antes de intentar iniciar el servidor si aún no lo tienes, ya que es una dependencia. Tu servidor puede no arrancar si no está instalado.
:::

Una vez que lo tengas instalado en tu VPS, puedes proceder a iniciar el servidor ejecutando el archivo **start-ark.bat** que creaste anteriormente.

Esto abrirá la consola del servidor en un símbolo del sistema y comenzará el proceso de arranque. Si todo ocurre como se espera, tu servidor será visible en la lista de servidores. Alternativamente, podrás conectarte directamente abriendo la consola dentro del juego y ejecutando `open [tu_dirección_ip]:7777`.
