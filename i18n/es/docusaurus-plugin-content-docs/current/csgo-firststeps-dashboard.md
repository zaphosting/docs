...
title: "Counter-Strike Global Offensive: Vista general del panel"
description: "Descubre cómo gestionar y optimizar eficientemente tu servidor de juegos de Counter-Strike: Global Offensive para el mejor rendimiento y control → Aprende más ahora"
sidebar_label: Panel
services:
  - gameserver
---
...
:::warning
El uso del Gestor de DDoS solo es posible si se ha contratado una IP propia o la opción adicional "Resumen del Gestor de DDoS". Si estos componentes faltan en tu paquete actual, puedes añadirlos en cualquier momento actualizando.
:::

### Tienda Tebex

Con Tebex tienes la posibilidad de monetizar tu servidor de juegos vendiendo bienes virtuales y rangos por dinero real. Es una herramienta potente y fiable diseñada específicamente para administradores de servidores de juegos que quieren ofrecer una mejor experiencia a sus jugadores.

![](https://screensaver01.zap-hosting.com/index.php/s/TbwafKd3Kmq4WrS/preview)

Tebex es fácil de usar y ofrece numerosas funciones para personalizar y optimizar tu tienda. Puedes adaptar el diseño, añadir métodos de pago e incluso lanzar campañas de marketing para llegar a más jugadores.

Al hacer clic en la imagen, puedes crear tu propia tienda para servidor de juegos con Tebex. La configuración es rápida y sencilla, así que estarás listo para vender tus bienes virtuales en poco tiempo.
...
:::warning
¿La información encontrada no ayuda o no la entiendes? ¡No te preocupes! En ese caso, también estamos disponibles diariamente en soporte. Solo crea un **[Ticket](https://zap-hosting.com/en/customer/support/)** y explícanos tu problema. Lo revisaremos lo antes posible y te ayudaremos a solucionarlo. :)
:::
...
¿Tu servidor estuvo temporalmente offline y no sabes por qué? ¿Mirar el registro de eventos y los archivos de registro no dio información? Podría ser que la causa del problema no esté directamente en tu servidor, sino en el sistema host donde se ejecuta tu servidor.
...
En esta sección encontrarás un historial de interrupciones del sistema host donde corre tu servicio de servidor de juegos. Más información y la versión más reciente del estado de nuestros servicios la puedes encontrar en nuestra nueva [página de estado](https://status.zap-hosting.com/).
...
En configuraciones encontrarás un resumen de las opciones más comunes para tu servidor. También incluyen una breve explicación. El uso de esta página es especialmente recomendado para quienes son nuevos en la administración de servidores.
...
El editor de archivos de configuración te permite editar los archivos directamente desde la web de forma clásica, sin la vista simplificada. Esta opción te permite personalizar las configuraciones de tu servidor según tus preferencias personales.

Usando el editor, tienes acceso directo a todas las opciones disponibles y puedes personalizarlas con precisión. Esto es especialmente útil para usuarios avanzados que quieren control total sobre las configuraciones y saben exactamente qué cambios desean hacer.
...
En **Juegos** siempre tienes la posibilidad de cambiar la variante del juego de tu juego actual o instalar otro juego en general. Los juegos con el mismo precio por slot se pueden seleccionar directamente bajo **Juegos disponibles**. Para todos los demás juegos, primero se requiere ajustar el precio por slot. Más información sobre esto la encontrarás en la guía [Cambio de juego](gameserver-gameswitch.md).
...
Además, puedes activar la función de notificación por correo electrónico para actualizaciones automáticas y así mantenerte informado sobre las actualizaciones y estar siempre al día.

### Plugins

Para este juego, ofrecemos la posibilidad de instalar fácilmente recursos/plugins conocidos y populares con un solo clic bajo **Plugins**. En la vista general encontrarás una selección del contenido disponible que puedes añadir haciendo clic en el botón de instalación. Tras reiniciar el servidor, estos recursos/plugins se instalarán, activarán y estarán disponibles para ti.

![](https://screensaver01.zap-hosting.com/index.php/s/DoL7miMAmj83CDr/preview)

Usando esta función, puedes ampliar rápida y fácilmente el contenido de tu servidor de juegos sin tener que hacer ajustes por tu cuenta. Esto ahorra tiempo y reduce el riesgo de errores o problemas de compatibilidad.

### Steam Workshop

Si buscas nuevo contenido para mejorar tu experiencia de juego, Steam Workshop ofrece una amplia selección de mods y mapas para varios juegos. Para usar una colección de mods o mapas en tu juego, solo necesitas ingresar el ID de la Colección de Steam Workshop. Luego, deberás reiniciar tu servidor para que el nuevo contenido se cargue. Una vez hecho esto, podrás usar el nuevo contenido en tu servidor y añadir más variedad y diversión a tu juego.
...
Con la configuración DNS de ZAP puedes crear direcciones fáciles de recordar para tu servicio. Puedes usar un subdominio de uno de nuestros dominios o crear una entrada DNS en tus propios dominios y así tener una dirección que se asocie mejor contigo como persona.

![](https://screensaver01.zap-hosting.com/index.php/s/tM5FGTHRSbZA5rN/preview)

## Herramientas

En la sección **Herramientas** de tu panel de navegación encontrarás varios subítems que te ayudarán a gestionar tu servidor. Estos incluyen **Explorador FTP**, **Bases de datos** y **Copias de seguridad**. A continuación, recibirás una introducción detallada a estas áreas.

### Explorador FTP

Usando el **Explorador FTP** obtendrás toda la información necesaria para establecer una conexión al acceso FTP. El acceso FTP te permite acceso directo a los archivos de tu servidor de juegos. Recomendamos usar un programa FTP externo como **Filezilla** o **WinSCP** para un uso óptimo. Si no estás familiarizado con cómo usar FTP, puede ayudarte echar un vistazo a la guía [Acceso vía FTP](gameserver-ftpaccess.md).
...
En la configuración puedes especificar si las copias de seguridad deben crearse automáticamente y si esto debe hacerse diariamente o semanalmente. Además, puedes decidir si se deben eliminar las copias más antiguas si es necesario, por ejemplo cuando el espacio de almacenamiento disponible se vuelve limitado.

![](https://screensaver01.zap-hosting.com/index.php/s/nm9za5LTMEcNsga/preview)

Por defecto, proporcionamos 10 GB de espacio de almacenamiento en el servidor de almacenamiento para copias de seguridad en nuestros productos de servidor de juegos. Si necesitas más espacio, puedes ampliarlo por un costo adicional. Así te aseguras de tener siempre suficiente espacio para tus copias y poder acceder a copias antiguas si es necesario.
