...
title: "Open.mp: Vista general del panel"
description: "Descubre cómo gestionar y optimizar eficientemente tu servidor Open.mp con una vista clara de las funciones y controles clave → Aprende más ahora"
sidebar_label: Panel
services:
  - gameserver-openmp
---
...
¿Tu servidor estuvo offline temporalmente y no sabes por qué? ¿Mirar el registro de eventos y los archivos de registro no dio información? Podría ser que la causa del problema no esté directamente en tu servidor, sino en el sistema host donde se ejecuta tu servidor.
...
En esta sección encontrarás un historial de interrupciones del sistema host donde corre tu servicio de servidor de juegos. Más información y la versión más actualizada del estado de nuestros servicios la encuentras en nuestra nueva [página de estado](https://status.zap-hosting.com/).
...
En configuraciones encontrarás un resumen de las opciones más comunes para tu servidor. También incluyen una breve explicación. El uso de la página de configuraciones es especialmente recomendado para quienes recién empiezan en la administración de servidores.
...
- Configuraciones básicas: nombre del juego Sagegame, nombre del servidor, descripción del servidor, contraseña de admin
...
En **Versiones** tienes la posibilidad de ver y gestionar la versión actual de tu servidor de juegos. Aquí puedes actualizar a la última versión en cualquier momento o activar actualizaciones automáticas para asegurarte de que tu servidor esté siempre al día.
...
**FileZilla:** FileZilla es el cliente FTP más fácil y conocido. Solo instala, inicia y escribe tus datos de acceso en la barra superior, que encuentras en esta página un poco más arriba. Tras un clic rápido en "Conectar", entras directamente al servidor y puedes gestionar los archivos.

**WinSCP:** WinSCP está pensado originalmente para SCP y SFTP (los descendientes cifrados del protocolo FTP) pero también puede manejar FTP. El programa es más completo que FileZilla y es apto para usuarios avanzados.
...
En **Copias de seguridad** tienes la opción de crear copias de seguridad manuales y automáticas para tu servidor y su base de datos asociada. Esto facilita recuperar tus datos en caso de problemas o pérdida de datos.
...
Por defecto, ofrecemos 10 GB de espacio de almacenamiento en el servidor de almacenamiento para copias de seguridad para nuestros productos de servidor de juegos. Si necesitas más espacio, se puede ampliar por un costo adicional. Así te aseguras de tener siempre suficiente espacio para tus copias y poder acceder a copias antiguas si es necesario.
