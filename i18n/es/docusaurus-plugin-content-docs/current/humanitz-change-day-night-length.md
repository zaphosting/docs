...
La duración del día y la noche se controla en el archivo `GameServerSettings.ini`. Puedes acceder a este archivo desde el panel de control de tu servidor en **Configs**. Dentro del archivo de configuración, localiza los siguientes parámetros:
...
¡Felicidades! Modificando los valores `DayDur` y `NightDur` en el `GameServerSettings.ini`, has ajustado con éxito el ciclo de día y noche en tu servidor de HumanitZ. Si tienes más preguntas o necesitas ayuda, no dudes en contactar a nuestro equipo de soporte, ¡disponible todos los días para ayudarte! 🙂
