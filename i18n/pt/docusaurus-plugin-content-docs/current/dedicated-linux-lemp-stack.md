...
description: "Descubra como configurar uma stack LEMP para hospedar sites PHP dinâmicos em servidores Linux com exemplos práticos → Saiba mais agora"
...
Se estiver usando outro firewall, garanta que a porta 80 (HTTP) esteja liberada. Você pode aprender mais sobre firewalls em Linux no nosso guia [Gerenciar Firewall](vserver-linux-firewall.md).
...
Usaremos o domínio de teste `zapdocs.example.com`. No mundo real, você provavelmente usaria um domínio. Você **deve** criar um registro DNS do tipo `A` para o domínio apontando para o endereço IP do seu servidor. Se precisar de ajuda, veja nosso guia [Registros de Domínio](domain-records.md).
