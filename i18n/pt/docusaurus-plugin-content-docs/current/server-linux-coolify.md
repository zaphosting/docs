...
Embora o Coolify seja leve no núcleo, o uso de recursos pode aumentar bastante dependendo da quantidade de aplicações, builds Docker e bancos rodando ao mesmo tempo. Recomendamos os seguintes requisitos de hardware para hospedar o Coolify no seu VPS/servidor dedicado.
...
Antes de instalar o Coolify, conecte-se ao seu servidor e garanta que o sistema operacional está atualizado. Você deve acessar seu servidor via SSH com privilégios root. Temos um guia sobre [Acesso Inicial SSH](vserver-linux-ssh.md) caso precise de ajuda nessa etapa.
...
O Coolify oferece um script de instalação rápido e automatizado. Para garantir a segurança, vamos usar variáveis de ambiente para pré-definir a conta root do administrador. Isso evita acessos não autorizados durante a configuração.
...
Após a instalação, o próximo passo é acessar o dashboard pelo navegador. Digite o endereço IP do seu servidor seguido da porta 8000: `http://[seu_ip_do_servidor]:8000`

Como você já passou as credenciais via comando de instalação, verá direto a tela de login, sem o assistente inicial.
...
Depois de logar no dashboard do Coolify, a primeira coisa a fazer é configurar um domínio personalizado. Acessar o Coolify pelo IP (`http://[seu_ip_do_servidor]:8000`) não é seguro.

Conectar um domínio permite que o Coolify gere automaticamente certificados SSL gratuitos para seu dashboard e todas as aplicações futuras.
...
Para garantir segurança com certificado SSL na sua instância Coolify, atribua um domínio a ela. Vá em **Configurações** no menu lateral e encontre o campo **Domínio da Instância**. Digite seu domínio com o prefixo `https://` (ex: `https://coolify.[seu_dominio]`). Clique em **Salvar** para finalizar.
...
Depois disso, você poderá acessar a aba **Projetos** e começar a fazer deploy da sua primeira aplicação!
...
Parabéns! Você instalou o Coolify no seu servidor e já pode começar a deployar seus projetos na hora. Também recomendamos dar uma olhada nos recursos abaixo, que podem te ajudar ainda mais durante a configuração do seu servidor.
