...
A maioria das distros Linux já vem com Python pré-instalado, porém a versão pode não estar atualizada ou o sistema pode ter sido instalado sem alguns pacotes. Você pode checar se o Python está instalado (`python3 --version`) e rodar os comandos abaixo para atualizar ou instalar o runtime.
...
Quando você escreve um programa Python, pode precisar instalar pacotes externos via pip. Eles podem ser instalados globalmente e ficar disponíveis para todos os scripts `.py` ou você pode criar um ambiente virtual (venv).

### Criando o venv

Primeiro, navegue até a pasta onde quer criar seu venv usando `cd` e, quando estiver lá, rode `python3 -m venv .` que vai instalar os arquivos necessários no local atual.
