...
title: "VPS: Configuração de IP (Ubuntu/Debian)"
description: "Aprenda como configurar um endereço IP estático em um servidor Linux rodando Ubuntu ou Debian → Saiba mais agora"
...
Configurar endereços IP no seu VPS Linux permite que você controle como seu servidor se comunica dentro da rede. Isso inclui atribuir endereços IP, definir o gateway padrão e configurar servidores DNS necessários para a conectividade de rede.

:::warning Servidor inacessível por falta de conexão com a internet?
Caso seu servidor fique inacessível devido a uma configuração de rede incorreta ou falta de conexão com a internet, o acesso remoto via SSH pode não funcionar mais. Use o [console VNC](vserver-vnc.md) na interface web para conectar ao seu servidor e corrigir a configuração de rede.
:::
...
Instalações Ubuntu usam o **Netplan** para configuração de rede. Conecte-se ao seu VPS via SSH e localize o arquivo de configuração do Netplan no diretório `/etc/netplan/`. Abra o arquivo de configuração:
...
O Debian geralmente usa o arquivo de configuração de interfaces de rede ao invés do Netplan. Conecte-se ao seu VPS via SSH e abra o arquivo de configuração de interfaces de rede:
...
Se o endereço correto aparecer na saída, a configuração do IP estático foi aplicada com sucesso. Se o novo endereço IP não aparecer, verifique novamente o arquivo de configuração e certifique-se de que as alterações foram salvas corretamente antes de reaplicar a configuração com:
...
Parabéns! Você configurou com sucesso o endereço IP no seu servidor Linux. Para dúvidas ou ajuda, não hesite em contatar nosso time de suporte, que está disponível diariamente para te ajudar! 🙂
