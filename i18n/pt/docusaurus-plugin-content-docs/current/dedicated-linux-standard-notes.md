...
O software precisa que todas as dependências estejam instaladas e que rode em um sistema operacional suportado. Confira se seu servidor atende a esses requisitos antes de continuar:

**Dependências:** `Docker (Engine 26+ e Compose)`

**Sistema Operacional:** Última versão do Ubuntu/Debian com suporte ao Docker 26+

Garanta que todas as dependências estejam instaladas e que o sistema operacional esteja na versão correta para evitar problemas de compatibilidade durante a instalação do Standard Notes.



## Preparação

Antes de configurar o **Standard Notes**, você precisa preparar seu sistema. Isso inclui atualizar o sistema operacional para a versão mais recente e instalar todas as dependências necessárias. Essas etapas garantem um ambiente estável e ajudam a evitar problemas durante ou depois da instalação.
...
Garantir a segurança do banco de dados é outro passo essencial. Senhas padrão devem ser trocadas por strings fortes e geradas aleatoriamente. Você pode criar uma senha segura com:
...
Essas mudanças garantem que sua instalação não só funcione, mas também esteja segura e adaptada ao seu ambiente.



## Conectando ao Servidor de Sincronização

Para configurar sua conta no Standard Notes, clique no ícone do avatar no canto inferior direito do app. No menu que abrir, selecione “Criar Conta” para começar a criar seu perfil. Informe um e-mail válido e uma senha segura.

Antes de finalizar, abra a seção “Opções Avançadas”. Em “Servidor de Sincronização”, escolha a opção “Personalizado” e insira o endereço IP e a porta do seu próprio servidor no formato IP:Porta. Assim, suas notas não serão sincronizadas pelo serviço padrão do Standard Notes, mas sim pelo seu servidor self-hosted.
