...
|            Comando            |                         Descrição                         |
| :--------------------------: | :-------------------------------------------------------: |
|  tv_allow_camera_man "0\|1"  |        Espectadores podem assumir como cinegrafistas        |
...
|        tv_name "nome"        | Nome do host CSTV. Define o nome do CSTV como aparece no navegador de servidores e na barra de status |
|       tv_nochat "0\|1"       |           Ativa/Desativa o chat do CSTV           |
|    tv_password "senha"       |       Protege a transmissão CSTV com senha       |
|     tv_record "nome_arquivo" |             Inicia a gravação de uma demo CSTV             |
|           tv_stop            |                 Para a transmissão CSTV                 |
|        tv_stoprecord         |            Para a gravação da demo CSTV             |
...
Demos gravadas manual e automaticamente são armazenadas no diretório principal (/gxxxxx/CS:GO/game/csgo) do servidor CS:GO. Elas podem ser baixadas via o acesso FTP fornecido. Se você ainda não sabe como acessar via FTP, tem instruções aqui: [Acesso via FTP](gameserver-ftpaccess.md).

![](https://screensaver01.zap-hosting.com/index.php/s/w9b4Z7ECoSkSQdT/preview)

Com a reprodução da demo pelo jogo, a demo baixada pode ser selecionada via **Load** e iniciada pelo botão **Play**.
