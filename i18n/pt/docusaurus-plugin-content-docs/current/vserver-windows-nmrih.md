...
title: "VPS: Configuração de Servidor Dedicado No More Room In Hell no Windows"
description: "Aprenda como configurar um servidor dedicado de No More Room In Hell no seu VPS ou servidor dedicado de forma rápida e eficiente → Saiba mais agora"
...
## Introdução
Tem um VPS ou servidor dedicado e quer instalar servidores de jogos nele? Por exemplo, um servidor dedicado de No More Room In Hell? Então você está no lugar certo! A seguir, vamos explicar passo a passo como você pode instalar esse serviço no seu servidor.

<InlineVoucher />

## Preparação

Para configurar um servidor de No More Room In Hell, é necessário o SteamCMD. O SteamCMD é a **versão de linha de comando do cliente Steam**. Essa ferramenta pode ser usada para baixar rápida e facilmente as aplicações de servidor dedicado dos jogos populares da Steam. Você encontra o SteamCMD no site oficial para desenvolvedores da Valve: https://developer.valvesoftware.com/wiki/SteamCMD.
...
Assim que a mensagem **Loading Steam API.... OK** aparecer, o processo foi concluído com sucesso e você pode começar a instalar o servidor de No More Room In Hell.
...
O próximo passo será a instalação. Isso é feito com o comando `app_update 317670`. O App ID **317670** corresponde à aplicação do **Servidor Dedicado de No More Room In Hell**.
...
Neste ponto, você já terminou a configuração do seu servidor de No More Room In Hell. Você pode fazer configurações adicionais editando diretamente o arquivo de inicialização. Vá até o diretório raiz. Dentro dele, navegue até a pasta cfg e abra o arquivo `server.cfg`. Lá você pode editar as opções do servidor.
...
Para que outros jogadores possam entrar no seu servidor, você deve gerar e incluir um Game Server Login Token (GSLT). Esse token autentica seu servidor junto à Steam. Para gerar um GSLT, visite http://steamcommunity.com/dev/managegameservers e crie um token usando o ID do jogo 317670, que corresponde a No More Room In Hell.
...
Parabéns, você instalou e configurou com sucesso o servidor de No More Room In Hell no seu VPS! Para dúvidas ou ajuda, não hesite em contatar nosso time de suporte, que está disponível diariamente para te ajudar! 🙂
