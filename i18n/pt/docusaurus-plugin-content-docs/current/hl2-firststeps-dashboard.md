...
Além de monitorar os eventos, você também pode enviar comandos diretamente ao servidor no **Console ao Vivo**. Isso é útil para ajustar configurações, executar ações específicas ou responder rapidamente a problemas que surgirem.
...
Com o **Navegador FTP**, você obtém todas as informações necessárias para estabelecer conexão via FTP. O acesso FTP permite acesso direto aos arquivos do seu servidor de jogos. Recomendamos usar um programa FTP externo como **Filezilla** ou **WinSCP** para melhor experiência. Se não conhece FTP, vale dar uma olhada no guia [Acesso via FTP](gameserver-ftpaccess.md).
...
**WinSCP:** WinSCP é originalmente para SCP e SFTP (versões criptografadas do FTP), mas também funciona bem com FTP normal. O programa é um pouco mais completo que o FileZilla e é mais indicado para usuários avançados.
...
Nas configurações, você define se os backups devem ser criados automaticamente e se isso deve ser diário ou semanal. Também pode decidir se os backups mais antigos devem ser deletados quando o espaço de armazenamento estiver acabando.
