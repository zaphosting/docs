...
:::warning
Notou que algumas mudanças suas são parcialmente sobrescritas? Fique ligado que certos valores são puxados da página de configurações. Se quiser alterar diretamente no config, primeiro desative as opções correspondentes na página de configurações.
:::
...
Nas configurações, você define se os backups serão automáticos e se serão diários ou semanais. Também pode decidir se os backups mais antigos devem ser deletados quando o espaço disponível ficar limitado.
