...
title: "VPS: Configuração do Servidor Dedicado Half-Life 2 no Linux"
description: "Descubra como configurar um servidor dedicado de Half-Life 2 no seu VPS Linux para uma experiência de jogo sem travas → Saiba mais agora"
...
Neste ponto, você já terminou a configuração do seu servidor Half-Life 2. Pode fazer configurações adicionais editando diretamente o arquivo de inicialização.
...
Para que outros jogadores possam entrar no seu servidor, você precisa gerar e incluir um Game Server Login Token (GSLT). Esse token autentica seu servidor junto ao Steam. Para gerar um GSLT, acesse http://steamcommunity.com/dev/managegameservers e crie um token usando o ID do jogo 232370, que corresponde ao Half-Life 2.
