...
title: "CS:GO: Configuração do servidor"
description: "Descubra como configurar servidores de Counter-Strike com gamemodes, mapgroups e tokens GSL para uma jogabilidade otimizada → Saiba mais agora"
...
Nas configurações você pode escolher o gamemode que deseja usar. Os comandos desse Config são então carregados conforme a escolha. Eles podem ser gerenciados tanto na administração do servidor de jogos quanto manualmente via FTP em **gXXXXXX/CS:GO/game/csgo/cfg/**.
...
Na página de configurações você pode ajustar definições básicas para o token GSL, tipo de jogo/gamemode, mapgroup e mais.
...
Mapgroups são grupos de mapas que normalmente correspondem a uma certa categoria. Por padrão existem os grupos **mg_active, mg_allclassic**. Opcionalmente você pode criar os seus próprios. Isso deve ser adicionado ao **gamemodes_server.txt**. A estrutura de um mapgroup é simples e se parece com isso:
...
O mapa inicial é usado para determinar qual mapa deve ser carregado durante o processo de inicialização. É importante que o nome do mapa seja escrito completamente e corretamente. A extensão do arquivo não é necessária.
...
Se você quiser usar seu servidor de forma privada ou se quiser que apenas certas pessoas tenham acesso, pode definir uma senha para o servidor. Será solicitado o password ao conectar-se ao servidor.
...
Como descrito no início, você pode definir o gamemode/tipo de jogo que deseja usar na interface na página de configurações. Depois disso, você deve ajustar suas configurações e modificar o arquivo de configuração correspondente. Nos configs do gamemode você pode fazer alterações detalhadas na jogabilidade, comportamento dos bots etc.
...
Esse comando define a quantia de dinheiro que a equipe Terrorista ganha por plantar uma bomba que é desarmada. Isso **se acumula junto com o bônus por perda da rodada**. O valor padrão é $800.



**cash_team_rescued_hostage**
Esse comando define a quantia de dinheiro que **a equipe inteira** ganha quando um jogador resgata um refém.



**cash_team_terrorist_win_bomb**
Esse comando define o dinheiro ganho por cada jogador da equipe Terrorista quando eles vencem uma rodada pela explosão da bomba.



**cash_team_win_by_defusing_bomb**
Esse comando define o dinheiro que a equipe CT ganha por vencer uma rodada desarmando a bomba.



**cash_team_win_by_hostage_rescue**
Esse comando cheat define o dinheiro ganho pelos membros da equipe quando vencem uma rodada resgatando um refém.



**cash_team_win_by_time_running_out_hostage**
Esse comando define o dinheiro ganho pela equipe que protege os reféns se vencerem a rodada não permitindo que nenhum refém seja resgatado dentro do limite de tempo.
...
**mp_afterroundmoney**
Esse comando define quanto dinheiro é pago a cada jogador de cada equipe no final de uma rodada, independentemente de vitória ou derrota. O valor padrão é 0 (o bônus de vitória/derrota é definido por outro comando).
...
Esse comando define se os jogadores devem receber um kit de desarme grátis no início da rodada e, se sim, quem deve receber:
...
Esse comando define a duração em segundos que um jogador tem para escolher um time. Após esse tempo, um time é selecionado automaticamente. A duração padrão é 15 segundos.



**mp_forcecamera**

Esse comando define o alcance que os espectadores têm (ou seja, quem os jogadores podem assistir quando morrem). Veja as opções para o alcance:

- Valor: 0 - Jogadores mortos podem assistir qualquer jogador
- Valor: 1 - Jogadores mortos podem assistir seus companheiros de equipe
...
Esse comando define se um jogador recebe armadura e capacete grátis no início da rodada:
...
Esse comando define o número máximo de rodadas que o servidor irá jogar.



**mp_roundtime**

Esse comando define o número máximo de minutos que uma rodada dura antes de ser vencida/perdida. Se a rodada acabar após esse tempo, a vitória é dada aos contra-terroristas. Os segundos são dados em dígitos decimais, 1.92 = 1m55s.



**mp_solid_teammates**

Esse comando define se você pode colidir com seus companheiros de equipe ou não:
...
Esse comando define quanto dinheiro um jogador recebe no início de um half. O valor padrão é $800.
...
Esse comando define quanto tempo dura o período de aquecimento antes do jogo começar. O tempo é definido em segundos.
