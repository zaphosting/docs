...
description: "Descubra como gerenciar seu servidor de jogos Counter-Strike 1.6 de forma eficiente com uma visão clara dos recursos e opções principais → Saiba mais agora"
...
> ⚠️ Notou que algumas mudanças são parcialmente sobrescritas? Fique ligado que certos valores são puxados da página de configurações. Se quiser alterar direto no config, primeiro desative as opções correspondentes na página de configurações.
...
Nas configurações, você define se os backups são criados automaticamente e se isso ocorre diariamente ou semanalmente. Também pode decidir se os backups mais antigos devem ser deletados quando o espaço de armazenamento ficar limitado.
