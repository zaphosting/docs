...
Antes de instalar o **Plausible**, certifique-se que seu ambiente de hospedagem atende aos seguintes requisitos para garantir uma instalação suave e desempenho ideal.
...
#### Git
Os dados do Plausible serão baixados via GitHub. Para isso, o Git precisa estar instalado. Rode o comando: 
```
...
O Plausible será implantado e executado na sua máquina usando um container Docker. Para isso, o Docker precisa estar instalado. Rode o comando: 
...
Depois, crie um arquivo de ambiente (.env) e defina as variáveis essenciais. O BASE_URL deve apontar para o domínio real que já tenha um registro DNS resolvendo para seu servidor. Você também precisará gerar uma SECRET_KEY_BASE aleatória com pelo menos 64 bytes:
```
touch .env
...
Essas portas padrão permitem que o Let’s Encrypt emita automaticamente certificados TLS. Se você planeja rodar o Plausible atrás de um proxy reverso, pode ajustar esses valores. Além disso, crie um `compose.override.yml` para expor as portas corretas para o container Plausible:
...
Parabéns! Você instalou e configurou com sucesso o Plausible no seu Servidor Dedicado. Também recomendamos dar uma olhada nos recursos abaixo, que podem te ajudar ainda mais durante a configuração do seu servidor:
