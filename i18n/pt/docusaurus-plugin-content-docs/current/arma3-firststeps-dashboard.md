...
Nas configurações, você encontra uma visão geral das opções mais comuns para seu servidor, com explicações rápidas. O uso da página de configurações é especialmente recomendado para quem está começando na administração de servidores.
...
Usando o editor, você tem acesso direto a todas as opções disponíveis e pode ajustá-las com precisão. Isso é especialmente útil para usuários avançados que querem controle total sobre as configurações e sabem exatamente o que querem mudar.
...
Além disso, você pode ativar notificações por e-mail para atualizações automáticas, mantendo-se informado e sempre na última versão.
...
**FileZilla:** FileZilla é provavelmente o cliente FTP mais simples e conhecido. Instale, abra e insira seus dados de acesso na barra superior, que você encontra nesta página um pouco acima. Com um clique rápido em "Conectar", você já estará logado no servidor e pode gerenciar os arquivos.
...
Nas configurações, você define se os backups são criados automaticamente e se isso deve ser diário ou semanal. Também pode decidir se os backups mais antigos devem ser deletados quando o espaço disponível ficar limitado.
