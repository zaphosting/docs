...
title: "Argo: Visão geral do dashboard"
description: "Descubra como gerenciar e otimizar seu servidor Argo de forma eficiente com uma visão clara dos recursos principais e dicas de administração → Saiba mais agora"
...
Com essas informações, você tem uma boa noção do uso do seu servidor. Além disso, essas estatísticas são úteis para identificar possíveis problemas de desempenho e restringir suas causas. Por exemplo, você pode rastrear quando a utilização aumentou e se isso está relacionado a certos eventos ou mudanças. Isso é especialmente importante quando surgem problemas repentinos, pois permite que você tome medidas direcionadas para solucionar e analisar sistematicamente as causas possíveis.
...
:::warning
Percebeu que algumas mudanças suas são parcialmente sobrescritas? Fique ligado que certos valores são puxados da página de configurações. Se quiser alterar diretamente no config, primeiro desative as opções correspondentes na página de configurações.
:::
...
**FileZilla:** FileZilla é provavelmente o cliente FTP mais simples e conhecido. Basta instalar, abrir e inserir seus dados de acesso na barra superior, que você encontra um pouco acima nesta página. Com um clique rápido em "Conectar", você já estará conectado ao servidor e pode gerenciar os arquivos.

**WinSCP:** WinSCP é originalmente para SCP e SFTP (as versões criptografadas do protocolo FTP), mas também funciona muito bem com FTP normal. O programa é um pouco mais complexo que o FileZilla e é mais indicado para usuários avançados.
