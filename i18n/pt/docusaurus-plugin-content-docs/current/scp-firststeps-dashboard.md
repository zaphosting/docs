...
Com essas informações, você tem uma boa noção do uso do seu servidor. Além disso, essas estatísticas são úteis para identificar possíveis problemas de desempenho e entender suas causas. Por exemplo, você pode rastrear quando a utilização aumentou e se isso está relacionado a eventos ou mudanças específicas. Isso é especialmente importante quando surgem problemas repentinos, pois permite que você tome medidas direcionadas para solucionar e analisar sistematicamente as causas.
...
:::warning
As informações encontradas não ajudam ou você não entende? Relaxa! Nesse caso, nosso suporte está disponível diariamente. Basta abrir um **[Ticket](https://zap-hosting.com/en/customer/support/)** e explicar seu problema. Vamos analisar o quanto antes e ajudar você a resolver! :)
:::
...
Além de monitorar os eventos, você também pode enviar comandos diretamente ao servidor pelo **Console ao Vivo**. Isso é útil para ajustar configurações, executar ações específicas ou responder rápido a problemas.
...
Em configurações, você encontra uma visão geral das opções mais comuns para seu servidor, com uma breve explicação. O uso dessa página é especialmente recomendado para quem está começando na administração de servidores.
...
Com as configurações de DNS da ZAP, você cria endereços fáceis de lembrar para seu serviço. Pode usar um subdomínio de um dos nossos domínios ou criar uma entrada DNS no seu próprio domínio, assim tendo um endereço que se conecta melhor com você.
...
**FileZilla:** FileZilla é provavelmente o cliente FTP mais simples e conhecido. Instale, abra e insira seus dados de acesso na barra superior, que você encontra um pouco acima nesta página. Com um clique rápido em "Conectar", você já estará conectado ao servidor e pode gerenciar os arquivos.
...
Nas configurações, você define se os backups são criados automaticamente e se isso ocorre diariamente ou semanalmente. Também pode decidir se os backups mais antigos devem ser apagados quando o espaço de armazenamento estiver limitado.
...
A seção **Notícias** te mantém por dentro de todos os anúncios e novidades relacionadas ao seu serviço. Aqui você encontra informações importantes sobre:
