...
title: "VPS: Configuração do Servidor Dedicado Day of Defeat: Source no Linux"
...
Quando estiver logado, você pode iniciar o processo de instalação com o comando abaixo para instalar facilmente usando o SteamCMD diretamente no usuário `steam`.
```
...
Neste ponto, você já terminou a configuração do seu servidor Day of Defeat: Source. Você pode fazer configurações adicionais editando diretamente o arquivo de inicialização.
...
Para que outros jogadores possam entrar no seu servidor, você precisa gerar e incluir um Game Server Login Token (GSLT). Esse token autentica seu servidor junto ao Steam. Para gerar um GSLT, visite http://steamcommunity.com/dev/managegameservers e crie um token usando o ID do jogo 232290, que corresponde ao Day of Defeat: Source.
