...
title: "No One Survived: Visão geral do dashboard"
description: "Descubra como gerenciar e otimizar seu servidor No One Survived de forma eficiente com uma visão clara dos principais recursos e controles → Saiba mais agora"
...
Nesta seção, você encontra um histórico das interrupções do sistema host onde seu serviço de servidor de jogos está hospedado. Para mais informações e a versão mais atual do status do serviço, confira nossa nova [página de status](https://status.zap-hosting.com/).
...
Usando o editor, você tem acesso direto a todas as opções disponíveis e pode personalizá-las com precisão. Isso é especialmente útil para usuários avançados que querem controle total sobre as configurações do servidor e sabem exatamente o que querem mudar.

:::warning
Percebeu que algumas mudanças são parcialmente sobrescritas? Fique ligado que certos valores de opção são puxados da página de configurações. Se quiser fazer essas mudanças direto no config, primeiro desative as opções correspondentes na página de configurações.
:::
...
Oferecemos 10 GB de espaço para backups no servidor de armazenamento para nossos produtos de servidor de jogos por padrão. Se precisar de mais espaço, é possível fazer upgrade pagando uma taxa. Assim, você garante espaço suficiente para seus backups e ainda pode acessar backups antigos quando precisar.
