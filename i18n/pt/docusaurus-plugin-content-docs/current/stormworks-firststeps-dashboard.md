...
title: "Stormworks: Visão Geral do Dashboard"
description: "Descubra como gerenciar e otimizar seu servidor de jogos Stormworks de forma eficiente com uma visão clara e passo a passo → Saiba mais agora"
...
Com essas informações, você tem uma boa noção do uso do seu servidor. Além disso, essas estatísticas são úteis para identificar possíveis problemas de desempenho e restringir suas causas. Por exemplo, você pode rastrear quando a utilização aumentou e se isso está relacionado a certos eventos ou mudanças. Isso é especialmente importante quando surgem problemas repentinos, pois permite tomar medidas direcionadas para solucionar e analisar sistematicamente as possíveis causas.
...
As informações encontradas lá não ajudam ou você não entende? Relaxa! Nesse caso, nosso suporte está disponível diariamente. Basta abrir um **[Ticket](https://zap-hosting.com/en/customer/support/)** e explicar seu problema pra gente. Vamos analisar o mais rápido possível e ajudar você a resolver! :)
...
- Configurações básicas: Nome do servidor, senha de admin, senha do servidor de jogos, Seed do Savegame, Ilha Base, menu de configurações e mais
...
Percebeu que algumas mudanças suas são parcialmente sobrescritas? Fique ligado que certos valores de opção são puxados da página de configurações. Se quiser alterar esses valores direto no config, primeiro desative as opções correspondentes na página de configurações.
...
Ficando de olho nas **Notícias** regularmente, você fica informado e consegue ajustar seu serviço para aproveitar ao máximo seu servidor.
