...
description: "Descubra como gerenciar e otimizar seu servidor Voyagers of Nera para o melhor desempenho e controle → Saiba mais agora"
...
Com essas informações, você tem uma boa noção do uso do seu servidor. Além disso, essas estatísticas são muito úteis para identificar possíveis problemas de desempenho e restringir suas causas. Por exemplo, você pode verificar quando a utilização aumentou e se isso está relacionado a certos eventos ou mudanças. Isso é especialmente importante quando surgem problemas repentinos, pois permite que você tome medidas direcionadas para resolver e analisar sistematicamente as possíveis causas.
...
:::warning
Percebeu que algumas mudanças suas são parcialmente sobrescritas? Fique ligado que certos valores são puxados da página de configurações. Se quiser alterar direto no config, primeiro desative as opções relevantes na página de configurações.
:::
...
Na seção **Versões**, você pode visualizar e gerenciar a versão atual do seu servidor de jogos. Aqui, você pode atualizar para a versão mais recente a qualquer momento ou ativar atualizações automáticas para garantir que seu servidor esteja sempre atualizado.
