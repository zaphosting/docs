...
title: "Half-Life 1: Visão Geral do Dashboard"
description: "Descubra como gerenciar e otimizar seu servidor de jogos Half-Life 1 de forma eficiente com uma visão clara dos recursos e controles essenciais → Saiba mais agora"
...
Em **Estatísticas**, você obtém uma visão abrangente dos valores de carga do seu servidor. Lá você encontra informações sobre vários aspectos, como a ocupação dos slots disponíveis, CPU e memória. Você pode visualizar esses dados para diferentes períodos, por exemplo, para os últimos **60 minutos, 24 horas, 3 dias, 7 dias** ou **30 dias**.
...
Nesta seção, você encontra um histórico das interrupções do sistema host onde seu servidor de jogos está hospedado. Para mais informações e a versão mais atual do status dos nossos serviços, visite nossa nova [página de status](https://status.zap-hosting.com/).
...
> ⚠️ Percebeu que algumas mudanças suas são parcialmente sobrescritas? Fique ligado que certos valores são puxados da página de configurações. Se quiser alterar diretamente no config, primeiro desative as opções correspondentes na página de configurações.
...
**WinSCP:** WinSCP é originalmente para SCP e SFTP (versões criptografadas do FTP), mas também funciona muito bem com FTP normal. O programa é um pouco mais complexo que o FileZilla e é mais indicado para usuários avançados.
...
Oferecemos documentação extensa para nossos produtos, que serve como base de conhecimento para as dúvidas e questões mais frequentes. Lá você encontra vários guias e ajuda sobre diferentes assuntos que vão te auxiliar no uso e gerenciamento do seu produto.
