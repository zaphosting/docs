...
description: "Descubra como recuperar o acesso ao seu VPS Windows após um bloqueio por senha e minimizar o tempo de inatividade → Saiba mais agora"
...
Pode acontecer rápido... Você tenta fazer login no seu Windows Server, mas sua senha não é mais aceita ou você simplesmente esqueceu. De repente, você fica bloqueado e o acesso a dados e aplicações importantes não é mais possível. Essa situação é obviamente frustrante, mas está longe de ser o fim. Seguindo os passos abaixo, você pode recuperar o acesso ao seu VPS Windows e continuar seu trabalho com o mínimo de interrupção.



## Preparação
Resetar o acesso da conta administrador é feito usando um arquivo ISO do Windows. Portanto, o mesmo arquivo ISO que foi usado originalmente para a instalação do sistema operacional Windows Server deve ser utilizado.
...
Agora é hora de resetar o acesso da conta administrador. Para isso, siga os primeiros passos da instalação novamente. Porém, você só precisa avançar até o início da instalação e então navegar até **Reparar seu computador**.
...
O layout do teclado no console HTML iLO pode ser diferente do seu, fazendo com que certos símbolos estejam em teclas diferentes. Fique atento para executar os comandos corretamente.

:::

Após o reboot do servidor, na tela de login inicial, pressione a combinação de teclas **Win+U**. Na janela do Prompt de Comando, digite o seguinte:
...
Sua senha foi alterada. Porém, você precisa desfazer as alterações nos arquivos do **Utilman**. Para isso, basta reiniciar seu servidor e repetir os passos. No prompt de comando, execute novamente os comandos:
