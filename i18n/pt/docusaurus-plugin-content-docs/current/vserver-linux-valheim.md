---
id: vserver-linux-valheim
title: "VPS: Configuração do Servidor Dedicado Valheim no Linux"
description: "Descubra como configurar um servidor dedicado Valheim no seu VPS Linux para gameplay e gerenciamento de servidor sem complicações → Saiba mais agora"
sidebar_label: Valheim
services:
  - vserver
---

import InlineVoucher from '@site/src/components/InlineVoucher';

## Introdução
Você tem um VPS Linux e quer instalar o serviço de servidor dedicado Valheim nele? Você está no lugar certo. Neste guia, vamos explicar passo a passo como instalar esse serviço no seu servidor Linux usando o SteamCMD. Usaremos o Ubuntu nos exemplos, mas o processo deve ser bem parecido para outras distribuições.

:::tip
Sabia que você pode instalar nossa **Interface ZAP GS/TS3** direto no seu VPS, permitindo configurar serviços de servidor de jogos com integração direta ao seu dashboard ZAP-Hosting, em poucos cliques? Saiba mais sobre a [Interface GS/TS3](vserver-linux-gs-interface.md).
:::

<InlineVoucher />

## Preparação

Para começar, conecte-se ao seu VPS via SSH. Use nosso guia [Acesso Inicial SSH](vserver-linux-ssh.md) se precisar de ajuda para isso.

Você também precisará fazer uma configuração inicial do SteamCMD se for a primeira vez que usa ele no seu servidor Linux. Use nosso guia [Configuração SteamCMD Linux](vserver-linux-steamcmd.md) e certifique-se que o SteamCMD está totalmente configurado antes de continuar.

## Instalação

Comece fazendo login como usuário `steam` e vá para o diretório raiz `home/steam` para manter tudo organizado.
```
sudo -u steam -s
cd ~
```

Com o login feito, você pode iniciar a instalação usando o comando abaixo para rodar o SteamCMD diretamente como usuário `steam`.
```
steamcmd +force_install_dir '/home/steam/Valheim-Server' +login anonymous +app_update 896660 validate +quit
```

Tenha paciência enquanto o download acontece, pode levar um tempo para jogos maiores. Quando terminar, uma mensagem de sucesso aparecerá confirmando.

## Configuração

Nesta etapa, você já terminou a configuração do seu servidor Valheim. Pode fazer ajustes adicionais editando diretamente o arquivo de inicialização.

Vá até o diretório raiz e abra o arquivo `.sh`. Você pode editar os parâmetros aqui.
```
nano /home/steam/Valheim-Server/start_server.sh
```

## Iniciando e conectando ao seu servidor

Agora é hora de iniciar seu servidor. Vá até o diretório principal do jogo e execute o arquivo shell **start_server.sh**.
```
/home/steam/Valheim-Server/start_server.sh
```

Você verá logs aparecerem no seu terminal, indicando que o servidor iniciou com sucesso. Note que a primeira inicialização pode demorar um pouco enquanto tudo é configurado. Alternativamente, você pode se conectar direto usando a barra de busca na lista de servidores e procurando por: `[seu_endereço_ip]:2456`.

:::info
Se não conseguir conectar e receber erros `PlayFab` no console, pode ser necessário desativar o suporte a crossplay para resolver, pois esse é um problema atual na versão Linux. Para isso, rode `nano /home/steam/Valheim-Server/start_server.sh` e remova a flag `-crossplay`.

Se precisar do crossplay, uma solução alternativa é instalar a versão Windows e usar o **Wine** como camada de compatibilidade. Use nosso guia rápido [Configuração da Camada de Compatibilidade Wine](vserver-linux-wine.md) para isso. Quando estiver pronto, instale a versão Windows do servidor Valheim via SteamCMD com:
```
steamcmd +@sSteamCmdForcePlatformType windows +force_install_dir '/home/steam/Valheim-Server' +login anonymous +app_update 896660 validate +quit
```

Depois de instalado, use o comando abaixo para rodar a versão Windows via Wine: `xvfb-run wine /home/steam/Valheim-Server/StartServer.bat`
:::

## Conclusão

Parabéns, você instalou e configurou com sucesso o servidor Valheim no seu VPS! Como próximo passo, recomendamos conferir nosso guia [Configurar Serviço Linux](vserver-linux-create-gameservice.md), que ensina a configurar seu novo servidor dedicado de jogos como um serviço. Isso traz vários benefícios, como inicialização automática do servidor no boot, atualizações automáticas, gerenciamento fácil e acesso a logs, entre outros!

Se tiver dúvidas ou problemas, fale com nosso time de suporte, que está disponível para te ajudar todos os dias!

<InlineVoucher />