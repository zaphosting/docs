...
title: "Enshrouded: Conecte-se ao servidor"
description: "Descubra como se conectar facilmente ao seu servidor Enshrouded e garantir uma experiência de jogo tranquila → Saiba mais agora"
...
Para se conectar ao seu servidor via o navegador de servidores dentro do jogo, abra o jogo. Depois, clique em **Jogar**, selecione **Entrar** na seleção de jogo e então clique em **Buscar por nome do servidor** à direita. Uma janela pop-up vai abrir onde você deve digitar o nome do seu servidor e clicar em buscar. Seu servidor deve aparecer logo em seguida.
...
Para se conectar ao seu servidor via o navegador de servidores do Steam, abra o navegador de servidores pelo Steam. Clique em `Visualizar` no topo da navegação do Steam e depois na seção `Servidor de Jogo` para abrir.

Busque ou adicione seu servidor pelo navegador do Steam usando o `endereço IP` e a `porta` do seu servidor de jogos. Para isso, selecione a aba `Favoritos` na lista de servidores e clique no símbolo de mais na parte inferior. Uma janela pop-up vai abrir onde você deve especificar as informações do seu servidor.
...
Parabéns, se você seguiu tudo direitinho, já deve ter se conectado ao servidor com sucesso. Para dúvidas ou ajuda, não hesite em contatar nosso suporte, que está disponível todos os dias para te ajudar! 🙂
