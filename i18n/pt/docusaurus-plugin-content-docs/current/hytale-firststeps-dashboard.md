...
description: "Descubra como gerenciar e otimizar seu servidor de jogos Hytale para o melhor desempenho e controle → Saiba mais agora"
...
O uso do Gerenciador de DDoS requer um endereço IP dedicado ou a opção adicional "Visão Geral do Gerenciador de DDoS". Se seu pacote atual não incluir esses componentes, você pode adicioná-los a qualquer momento fazendo um upgrade do serviço.
:::
...
Nesta seção, você encontra um histórico das interrupções do sistema host onde seu servidor de jogos está hospedado. Para mais informações e a versão mais atual do status do serviço, acesse nossa nova [página de status](https://status.zap-hosting.com/).
...
O editor de arquivos de configuração permite editar os arquivos diretamente pelo site, no modo clássico, sem a visualização simplificada. Essa opção permite personalizar as configurações do seu servidor conforme suas preferências.
...
:::warning
Percebeu que algumas mudanças são parcialmente sobrescritas? Fique ligado que certos valores são puxados da página de configurações. Se quiser alterar diretamente no config, primeiro desative as opções relevantes na página de configurações.
:::
