...
nginxの設定は、ドメイン用の設定ファイルを作成することから始まります。`nginx/conf`ディレクトリ内に、ドメイン名をファイル名にした新規ファイルを作成してください。例として、`nano nginx/conf/example.com.conf` を実行し、以下の基本設定を追加します。`example.com`は実際のドメイン名に置き換えてください：
