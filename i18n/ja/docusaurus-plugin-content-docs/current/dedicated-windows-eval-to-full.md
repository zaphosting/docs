...
title: "専用サーバー：Windows Server 評価版から正式版への変換"
...
Windows Server 評価版はテスト用に設計されており、使用期限が限られています。同じインストール環境を本番環境で使い続けたい場合、OSを再インストールせずに評価版から正式ライセンス版へ変換することが可能です。
...
すべてのエディション間で変換が可能なわけではありません。現在のインストールに対して変換可能な正式エディションを確認するには、以下のコマンドを実行してください：
...
対象エディションが判明し、有効なプロダクトキーが用意できたら、変換を開始します。`<TargetEdition>` はDISMのターゲットエディション確認で得られたサポートされているエディションのいずれかに合わせてください。
...
サーバー再起動後、管理者権限でコマンドプロンプトまたはPowerShellを開き、以下を実行してください：

```powershell
DISM /Online /Get-CurrentEdition
```

出力に `ServerStandard` や `ServerDatacenter` といった正式版エディションが表示されていれば、評価版からの変換が成功したことを示します。
