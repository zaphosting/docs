...
落下ダメージを無効にするには、該当ワールドのフォルダ内にある`universe/worlds/<world_name>/config.json`を開きます。落下ダメージを制御する設定は通常、`IsFallDamageEnabled`のようなブール値で表されているので、これを`false`に設定してください：
