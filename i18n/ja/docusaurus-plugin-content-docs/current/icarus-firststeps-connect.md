...
title: "Icarus：サーバーへの接続方法"
description: "Icarusサーバーへのスムーズな接続方法をチェックして、快適なゲーム体験を実現しよう → 今すぐ詳しく見る"
...
## はじめに
**Icarus**のゲームサーバーへの接続方法や、始めるために必要なものがわからない？心配しないで！必要なツールや情報から実際の接続手順、スムーズでトラブルフリーな接続を実現するためのポイントまで、全部しっかりサポートするよ。ガイドに沿って進めれば、すぐに接続完了！
...
ゲーム内のサーバーブラウザから接続するには、**サーバーブラウザ**を開こう。**プレイ**をクリックして、まだキャラクターを作成していなければ作成してね。サーバーブラウザに入ったら、上部の検索欄でサーバー名を検索。サーバーを選択して接続しよう。

![img](https://screensaver01.zap-hosting.com/index.php/s/43JboZPQY756SYi/download)

</TabItem>

<TabItem value="connect_solution_server_browser_direct" label="サーバーブラウザの直接接続（ゲーム内）" default>

サーバーリストから探す代わりに、IPアドレスとポートを使って直接サーバーを追加して接続することもできるよ。サーバーリストで**直接接続**をクリックして、情報を入力しよう。
...
サーバーが見えない場合、初期化が正常に完了していない可能性があるよ。設定ミスやファイルの破損が原因かも。詳しくはサーバーコンソールやログファイルをチェックしてみてね。
...
おめでとう！ここまでの手順をしっかりやれば、サーバーへの接続はバッチリなはず。もし質問やサポートが必要なら、いつでも毎日対応しているサポートチームに気軽に連絡してね！🙂
