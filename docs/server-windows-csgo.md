...
description: "Learn how to set up a CS:GO dedicated server on your server quickly and efficiently → Learn more now"
sidebar_label: "CS:GO"
services:
  - vserver
  - dedicated
...
## Introduction
Got a VPS/Dedicated Server and want to install game servers on it? For example an CS:GO Dedicated Server? Then you are at the right place! In the following we will explain step by step how you can install such a service on your server.



## Preparation

For the setup of a CS:GO server the SteamCMD is needed. The SteamCMD is the **command line edition of the Steam client**. This tool can be used to quickly and easily download the VPS/Dedicated Server applications of popular Steam games. You will find the SteamCMD on the official Valve developer website: https://developer.valvesoftware.com/wiki/SteamCMD. 
...
As soon as the message **Loading Steam API.... OK** is displayed, the process has been completed successfully and you can start with the installation of the CS:GO server.
...
The next step will be the installation. This is done with the `app_update 730` command. The App ID **730** stands for the **CS:GO Dedicated Server** application.
...
By this stage, you have finished the setup for your CS:GO server. You can perform further server configuration by directing editing the launch file. Head over to your root directory. Inside this directory navigate to the cfg directory and open the `server.cfg` file. There you can edit server options.
...
For other players to join your server, you must generate and include a Game Server Login Token (GSLT). This token authenticates your server with Steam. To generate a GSLT, visit http://steamcommunity.com/dev/managegameservers and create a token using the game ID **4465480**, which corresponds to the new CS:GO Standalone.
...
Congratulations, you have successfully installed and configured the CS:GO server on your VPS/Dedicated Server! For further questions or assistance, please don't hesitate to contact our support team, which is available daily to assist you! 🙂
