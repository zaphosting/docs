...
description: "Discover how to remotely manage and control Left 4 Dead 2 servers securely without joining the game → Learn more now"
sidebar_label: RCON
services:
  - gameserver-l4d2
---
...
In Left 4 Dead 2, RCON is used to execute server-side commands, such as managing players, changing gameplay settings, or accessing diagnostic outputs. The connection is protected by a password and operates over a specified port, accessible via compatible RCON clients.

A key advantage of RCON is that it enables server management **without having to be connected to the game as a player**. Server administrators can monitor and control Left 4 Dead 2 from external tools, command-line interfaces, or web dashboards, offering flexibility and convenience for remote operation.
...
Once connected via RCON, various administrative and diagnostic commands can be executed on the Left 4 Dead 2 server. The available commands depend on the game engine but typically include actions for player management, status queries, and server control.
