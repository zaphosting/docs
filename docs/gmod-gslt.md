...
description: "Information about the creation and use of GSLT for Garry’s Mod server from ZAP-Hosting"
...
Once logged in, you have the possibility to create a new token using the form displayed on the page. In the **App ID** field, enter the App ID `4000` which belongs to **Garry’s Mod**. Make sure to enter the correct ID, as the token will not work with an incorrect value.
...
:::info Operating multiple Garry’s Mod servers
When operating multiple **Garry’s Mod** servers, you will need a GSLT for each server. It is <u>**not**</u> possible to operate multiple game servers via one single GSLT.
:::
