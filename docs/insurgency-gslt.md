...
description: "Information about the creation and use of GSLT for Insurgency server from ZAP-Hosting - ZAP-Hosting.com documentation"
...
Once logged in, you have the possibility to create a new token using the form displayed on the page. In the **App ID** field, enter the App ID `17700` which belongs to **Insurgency**. Make sure to enter the correct ID, as the token will not work with an incorrect value.
...
:::info Operating multiple Insurgency servers
When operating multiple **Insurgency** servers, you will need a GSLT for each server. It is <u>**not**</u> possible to operate multiple game servers via one single GSLT.
:::
