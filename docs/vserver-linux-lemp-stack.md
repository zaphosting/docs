...
You should see `Nginx` and `Nginx (v6)` rules with `ALLOW` actions set, which confirms that the firewall is ready. You should also other rules that you may have previously setup, including the SSH rule.
