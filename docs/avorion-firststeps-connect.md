...
title: "Avorion: Connect to the server"
description: Information about various ways on how to connect to your Avorion server from ZAP-Hosting - ZAP-Hosting.com Documentation
...
To connect to your server via the in game server browser, open the server browser in the main menu. To do this, click on **Multiplayer** and then select **Browse Servers**. You can use the **search field** to search for your server in the server list using the server name and then establish the connection using the **Join** button. 
...
Instead of searching for the server via the server list, you can also connect directly using the IP address and port. To do this, click on **Multiplayer** and then select **Join via IP**. This opens a window where you have to enter your server information in the IP/Port field. In addition, you must enter a name for the server. Afterwards, click on **Add server** to add the server and establish the connection. 
