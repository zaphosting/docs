...
Additional software is required to connect to the game server for this game. RedM requires that you use their official Client. In order to download the client visit the official [Website](https://redm.net/) of **RedM (Cfx.re)**. 

![img](https://screensaver01.zap-hosting.com/index.php/s/7NRnjo2PdgQPKP3/download)

The `RedM.exe` file will be downloaded. Execute the application afterwards. Once the application has loaded, Windows Explorer will open and you will need to select the directory where the `Red Dead Redemption 2` game and the `RDR2.exe` file are installed on your computer.  
