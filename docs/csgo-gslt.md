---
id: csgo-gslt
title: "CS:GO: Create a GSLT (Steam Game Server Login Token)"
description: "Discover how to authenticate and secure your dedicated game server with a Steam Game Server Login Token for optimal server visibility and protection → Learn mor"
...
Once logged in, you have the possibility to create a new token using the form displayed on the page. In the **App ID** field, enter the App ID `4465480` which belongs to **CS:GO**. Make sure to enter the correct ID, as the token will not work with an incorrect value.
...
:::info Operating multiple CS:GO servers
When operating multiple **CS:GO** servers, you will need a GSLT for each server. It is <u>**not**</u> possible to operate multiple game servers via one single GSLT.
:::
