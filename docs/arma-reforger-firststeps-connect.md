...
description: Information about various ways on how to connect to your Arma Reforger server from ZAP-Hosting - ZAP-Hosting.com Documentation
...
To connect to your server via the in game server browser, open the game. Once in game, click on **Multiplayer** top open the server list. At the top right of the server list, you can now search for the name of your server in the search bar and then select your server and establish the connection. 
...
Instead of searching for the server by name, you can also use the direct connection via the `V` button. Open the direct connection and enter the IP address and port of your server. Confirm the input with the `Enter` button to establish a connection. 
