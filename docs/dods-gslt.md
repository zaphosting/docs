...
Once logged in, you have the possibility to create a new token using the form displayed on the page. In the **App ID** field, enter the App ID `300` which belongs to **Day of Defeat: Source**. Make sure to enter the correct ID, as the token will not work with an incorrect value.
...
:::info Operating multiple Day of Defeat: Source servers
When operating multiple **Day of Defeat: Source** servers, you will need a GSLT for each server. It is <u>**not**</u> possible to operate multiple game servers via one single GSLT.
:::
