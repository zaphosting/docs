...
## Introduction
Do you have a Linux VPS and you want to install the Hytale on it? You are in the right place. In this guide, we will explain the step by step process of installing this service on your Linux server.
...
Before continuing, ensure that Java 25 is installed on your system. You can verify the installation with:
...
Start by creating a dedicated directory for the Hytale server. This keeps all server files organized in one place.
...
The server is started by launching the JAR file and specifying the path to the assets archive. Adjust the path if your assets are stored in a different location.
...
By default, the server listens on UDP port 5520 and binds to all available interfaces. You can change the address and port if required. The server communicates over UDP using QUIC. Make sure your firewall allows incoming UDP traffic on the chosen port by either using Iptables or UFW.
