...
description: "Information about setting up a Day of Defeat: Source Dedicated Server on a Windows VPS from ZAP-Hosting"
...
As soon as the message **Loading Steam API.... OK** is displayed, the process has been completed successfully and you can start with the installation of the Day of Defeat: Source server.
...
The next step will be the installation. This is done with the `app_update 232290` command. The App ID **232290** stands for the **Day of Defeat: Source Dedicated Server** application.
...
By this stage, you have finished the setup for your Day of Defeat: Source server. You can perform further server configuration by directing editing the launch file. Head over to your root directory. Inside this directory navigate to the cfg directory and open the `server.cfg` file. There you can edit server options.
...
For other players to join your server, you must generate and include a Game Server Login Token (GSLT). This token authenticates your server with Steam. To generate a GSLT, visit http://steamcommunity.com/dev/managegameservers and create a token using the game ID 232290, which corresponds to Day of Defeat: Source.
