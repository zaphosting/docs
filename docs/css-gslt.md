...
description: "Information about the creation and use of GSLT for Counter-Strike: Source server from ZAP-Hosting - ZAP-Hosting.com documentation"
...
Once logged in, you have the possibility to create a new token using the form displayed on the page. In the **App ID** field, enter the App ID `220` which belongs to **Counter-Strike: Source**. Make sure to enter the correct ID, as the token will not work with an incorrect value.
...
:::info Operating multiple Counter-Strike: Source servers
When operating multiple **Counter-Strike: Source** servers, you will need a GSLT for each server. It is <u>**not**</u> possible to operate multiple game servers via one single GSLT.
:::
